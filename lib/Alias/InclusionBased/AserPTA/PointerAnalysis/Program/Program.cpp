...
llvm::cl::opt<size_t> MaxIndirectTarget(
    "max-indirect-target",
    llvm::cl::init(
        std::numeric_limits<size_t>::max()), // by default no limitation
    llvm::cl::desc("max number of indirect call target that can be resolved by "
                   "indirect call"));
...
aser::CallSite::resolveTargetFunction(const Value *calledValue) {
  ...
  LOG_WARN("Unable to resolve target function from calledValue: {}",
           *calledValue);
  return nullptr;
}
