 ...
 */
std::unique_ptr<AliasAnalysisWrapper>
AliasAnalysisFactory::create(Module &M, const AAConfig &config) {
 ...
 *
 * Convenience factory method that creates a wrapper with the default
 * configuration (SparrowAA, context-insensitive). This is useful when you don't
 * need specific analysis features.
 *
 * @param M The LLVM module to analyze
 * @return A unique_ptr to the newly created AliasAnalysisWrapper with default
 * config
 *
 * @note Default is SparrowAA with no context sensitivity (fastest option)
 */
std::unique_ptr<AliasAnalysisWrapper>
AliasAnalysisFactory::createAuto(Module &M) {
 ...
 */
std::unique_ptr<AliasAnalysisWrapper>
AliasAnalysisFactory::createSparrowAA(Module &M, unsigned kCFA) {
  if (kCFA == 0) {
    return create(M, AAConfig::SparrowAA_NoCtx());
  } else if (kCFA == 1) {
 ...
 */
std::unique_ptr<AliasAnalysisWrapper>
AliasAnalysisFactory::createAserPTA(Module &M, unsigned kCFA) {
  if (kCFA == 0) {
...
/**
 * @brief Create a TPA (Flow- and Context-Sensitive Semi-Sparse) alias analysis
 * wrapper
 *
 ...
 *       semi-sparse program representation, which may take time for large
 * modules
 * @note TPA supports loading external pointer tables from configuration files
 */
std::unique_ptr<AliasAnalysisWrapper>
AliasAnalysisFactory::createTPA(Module &M, unsigned kCFA) {
  if (kCFA == 0) {
...
std::unique_ptr<AliasAnalysisWrapper>
AliasAnalysisFactory::createGPG(Module &M) {
  return create(M, AAConfig::GPG());
}
