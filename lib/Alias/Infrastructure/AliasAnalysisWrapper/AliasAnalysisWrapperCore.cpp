 ...
 *
 * This function implements a conservative merging strategy for combining
 * results from multiple alias analysis backends. The strategy prioritizes
 * precision:
 * - If any backend can prove NoAlias, the result is NoAlias (most precise)
 * - If any backend can prove MustAlias (and none prove NoAlias), the result is
 * MustAlias
 ...
 *       to MayAlias as a safe default. This should not happen with sound
 * analyses.
 */
static llvm::AliasResult
 ...
 * @param config The alias analysis configuration specifying which
 * implementation to use, context sensitivity settings, and other parameters
 *
 * @note Initialization happens automatically in the constructor. If
 * initialization fails, the wrapper will be marked as uninitialized and queries
...
void AliasAnalysisWrapper::initialize() {
  ...
  auto initAA = [this](auto fn, const char *name) -> bool {
    try {
      ...
      errs() << "AliasAnalysisWrapper: Failed to init " << name << ": "
             << e.what() << "\n";
      return false;
    }
  };
  ...
  case AAConfig::Implementation::SparrowAA: {
    unsigned k = (_config.ctxSens == AAConfig::ContextSensitivity::KCallSite)
                     ? _config.kLimit
                     : 0;
    _initialized = initAA(
        [this, k] {
          _andersen_aa = std::make_unique<AndersenAAResult>(
              *_module, makeContextPolicy(k));
        },
  ...
  case AAConfig::Implementation::TPA: {
    _initialized = initAA(
        [this] {
          ...
          } else if (_config.ctxSens ==
                     AAConfig::ContextSensitivity::KCallSite) {
          ...
          _tpa_program = std::make_unique<tpa::SemiSparseProgram>(
              builder.runOnModule(*_module));
          // Create and run the pointer analysis
          _tpa_aa = std::make_unique<tpa::SemiSparsePointerAnalysis>();
          // Try to load external pointer table (optional, won't fail if not
          // found) Check LOTUS_CONFIG_DIR first, then fallback to
          // config/ptr.spec
          std::string specPath;
          ...
          // Use a simple file existence check - if file doesn't exist, analysis
          // will still work
          std::error_code EC;
  ...
  case AAConfig::Implementation::GPG:
    _initialized = initAA(
        [this] {
          ...
          config.field_sensitive = _config.fieldSensitive;
          _gpg_aa =
              std::make_unique<lotus::gpg::GPGAnalysisEngine>(*_module, config);
          _gpg_aa->run();
        },
        "GPG");
    break;
  ...
  case AAConfig::Implementation::UnderApprox:
    _initialized = initAA(
        [this] {
          _underapprox_aa =
              std::make_unique<UnderApprox::UnderApproxAA>(*_module);
        },
        "UnderApprox");
    break;
  ...
  case AAConfig::Implementation::CFLAnders:
    _initialized = initAA(
        [this] {
          _tli = std::make_unique<llvm::TargetLibraryInfoWrapperPass>(
              Triple(_module->getTargetTriple()));
  ...
  case AAConfig::Implementation::Combined: {
    ...
    bool andersenInitialized = initAA(
        [this] {
          _andersen_aa = std::make_unique<AndersenAAResult>(
              *_module, makeContextPolicy(0));
        },
        "Andersen(NoCtx)");
    bool dyckInitialized = initAA(
        [this] {
          _dyck_aa = std::make_unique<DyckAliasAnalysis>();
          _dyck_aa->runOnModule(*_module);
        },
