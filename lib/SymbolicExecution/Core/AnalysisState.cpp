...
#include "SymbolicExecution/Core/AnalysisDriver.h"
#include "SymbolicExecution/Core/MemoryAPI.h"
...
PropertyValuePtr AnalysisState::computeOffsets(GEPOperator *GEP) {
  ...
  // In LLVM GEP semantics the first index scales the base pointer by the size
  // of the source element type; only later indices descend into that type.
  ...
  for (Value *index_operand : GEP->indices()) {
    ...
    Type *element_type = current_type;
    if (!IsFirstIndex) {
...
void AnalysisState::transfer(Instruction *Inst, AnalysisDriver &Driver) {
  ...
  for (unsigned Idx = 0; Idx < Inst->getNumOperands(); ++Idx) {
    ...
    if (!isa<Instruction>(Opnd)) {
      if (auto *OpndNode = Graph->findNode(Opnd))
        initSymbol(OpndNode);
      auto *Stripped = Opnd->stripPointerCasts();
      if (auto *StrippedNode = Graph->findNode(Stripped))
        initSymbol(StrippedNode);
    }
  }
...
void AnalysisState::processGEP(Instruction *Inst) {
  ...
  // A zero-offset GEP strips to its base pointer, so the destination and the
  // source can denote the same abstract node. Strong-updating a node onto
  // itself would clear the very set being copied from, so skip the rebind.
  if (Dst != SrcPtr) {
    assignPtr(Dst, SrcPtr, OffsetInBits, true);
    if (PointsTo.count(Dst) && PointsTo.at(Dst).empty()) {
