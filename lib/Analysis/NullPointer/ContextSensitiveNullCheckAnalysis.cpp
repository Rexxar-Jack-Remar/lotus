...
static cl::opt<unsigned>
    CSRound("csnca-round", cl::init(2), cl::Hidden,
            cl::desc("# rounds for context-sensitive NCA"));
static cl::opt<unsigned> CSMaxContextDepth(
    "csnca-max-depth", cl::init(3), cl::Hidden,
    cl::desc("Maximum depth of calling context to consider for NCA."));
// Define our own verbose option
static cl::opt<bool>
    CSVerbose("cs-verbose",
              cl::desc("Enable verbose output for context-sensitive analysis"),
              cl::init(false));
// Add option to control per-function statistics
static cl::opt<bool> CSPrintPerFunction(
    "cs-print-per-function",
    cl::desc("Print per-function statistics for context-sensitive analysis"),
    cl::init(false));
...
void ContextSensitiveNullCheckAnalysis::getAnalysisUsage(
  ...
  for (auto &F : M) {
    if (!F.empty()) {
      ...
      if (It == AnalysisMap.end() || !It->second) {
        auto *LNCA =
            new ContextSensitiveLocalNullCheckAnalysis(NFA, &F, EmptyContext);
        LNCA->run();
...
Context ContextSensitiveNullCheckAnalysis::getKLimitedContext(
...
void ContextSensitiveNullCheckAnalysis::buildKLimitedContextMap() {
  ...
  // Build a map from k-limited contexts to all full contexts that share that
  // k-suffix
  for (const auto &Entry : AnalysisMap) {
    ...
    // Add this full context to the set of contexts with the same k-limited
    // version
    std::pair<Function *, Context> key = {const_cast<Function *>(F),
                                          KLimitedCtx};
...
bool ContextSensitiveNullCheckAnalysis::mayNullInAnyMatchingContext(
    Value *Ptr, Instruction *Inst, const Context &KLimitedCtx) {
  ...
  // For sound analysis: if ANY context says it may be null, then the result is
  // may be null
  for (const Context &FullCtx : It->second) {
    ...
    // If this context's analysis says it may be null, then the overall result
    // is may be null
...
bool ContextSensitiveNullCheckAnalysis::mayNull(Value *Ptr, Instruction *Inst,
                                                const Context &Ctx) {
  // First check if the context-sensitive flow analysis says it's not null in
  // this context
  auto *NFA = &getAnalysis<ContextSensitiveNullFlowAnalysis>();
  if (NFA->notNull(Ptr, Ctx)) {
    return false; // If flow analysis proves NOT_NULL in this context, then it's
                  // definitely NOT_NULL
  }
  ...
  // Otherwise, use sound k-limiting: merge results from all contexts with same
  // k-suffix
