...
static cl::opt<int> CSIncrementalLimits(
    "csnfa-limit", cl::init(10), cl::Hidden,
    cl::desc("Determine how many non-null edges we consider a round in "
             "context-sensitive analysis."));

static cl::opt<unsigned> CSMaxContextDepth(
    "csnfa-max-depth", cl::init(3), cl::Hidden,
    cl::desc("Maximum depth of calling context to consider."));

static cl::opt<unsigned>
    CSRound("csnfa-round", cl::init(10), cl::Hidden,
...
ContextSensitiveNullFlowAnalysis::ContextSensitiveNullFlowAnalysis()
    : ModulePass(ID), AAA(nullptr), DAA(nullptr), VFG(nullptr),
      MaxContextDepth(CSMaxContextDepth), OwnsAliasAnalysisAdapter(false) {}
...
void ContextSensitiveNullFlowAnalysis::getAnalysisUsage(
...
bool ContextSensitiveNullFlowAnalysis::recompute(
    std::set<std::pair<Function *, Context>> &NewNonNullFunctionContexts) {
  std::unordered_map<FunctionContextPair, std::set<DyckVFGNode *>>
      PossibleNonNullNodes;
  unsigned K = 0,
...
bool ContextSensitiveNullFlowAnalysis::notNull(Value *Ptr, Context Ctx) const {
  ...
  if (KSuffix.size() > MaxContextDepth) {
    KSuffix.erase(KSuffix.begin(),
                  KSuffix.begin() + (KSuffix.size() - MaxContextDepth));
  }

  // Add all contexts that have the same k-suffix
  for (const auto &Entry : NonNullNodes) {
    if (Entry.first.first != F)
      continue;
    ...
    if (OtherKSuffix.size() > MaxContextDepth) {
      OtherKSuffix.erase(OtherKSuffix.begin(),
                         OtherKSuffix.begin() +
                             (OtherKSuffix.size() - MaxContextDepth));
    }
  ...
  // For a value to be definitely NOT NULL, it must be NOT NULL in ALL matching
  // contexts This is the sound approach for k-limiting
  for (const Context &MatchingCtx : MatchingContexts) {
    ...
    if (!IsNotNullInContext && InstPoint && AAA &&
        !AAA->mayNull(Ptr, InstPoint)) {
      IsNotNullInContext = true;
    }

    if (!IsNotNullInContext) {
      // If it's not definitely NOT NULL in any matching context, we can't
      // guarantee NOT_NULL
      return false;
    }
  }
...
void ContextSensitiveNullFlowAnalysis::add(Function *F, Context Ctx, Value *V1,
                                           Value *V2) {
  if (!V1 || !V1->getType()->isPointerTy())
    return;

  if (!V2) {
    void (ContextSensitiveNullFlowAnalysis::*AddSingle)(
        Function *, Context, Value *) = &ContextSensitiveNullFlowAnalysis::add;
...
void ContextSensitiveNullFlowAnalysis::add(Function *F, Context Ctx,
                                           CallInst *CI, unsigned int K) {
  if (!CI)
    return;
...
Context ContextSensitiveNullFlowAnalysis::extendContext(const Context &Ctx,
                                                        CallInst *CI) const {
