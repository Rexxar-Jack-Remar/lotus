...
ContextSensitiveLocalNullCheckAnalysis::ContextSensitiveLocalNullCheckAnalysis(
  ...
  for (unsigned K = 0; K < F->arg_size(); ++K) {
    ...
    if ((FirstInst && NFA->notNull(Arg, Ctx)) ||
        (FirstInst && NFA->notNull(ArgX, Ctx)))
      continue;
  ...
  for (auto &B : *F) {
    for (auto &I : B) {
      for (unsigned K = 0; K < I.getNumOperands(); ++K) {
        ...
        if (NFA->notNull(OpX, Ctx) || NFA->notNull(Op, Ctx))
          continue;
...
ContextSensitiveLocalNullCheckAnalysis::
    ~ContextSensitiveLocalNullCheckAnalysis() = default;

bool ContextSensitiveLocalNullCheckAnalysis::mayNull(Value *Ptr,
                                                     Instruction *Inst) {
  // not used as a ptr
  ...
  // must be nonnull
  if (NFA->notNull(Ptr, Ctx) || NFA->notNull(NEA.get(Ptr), Ctx))
    return false;
...
void ContextSensitiveLocalNullCheckAnalysis::merge(
    std::vector<Edge> &IncomingEdges, BitVector &Result) {
...
void ContextSensitiveLocalNullCheckAnalysis::transfer(Edge E,
                                                      const BitVector &In,
                                                      BitVector &Out) {
  ...
  case Instruction::ICmp: {
    ...
    if (ICmp->isEquality() && BrNo < 2) {
      ...
      if (Op0->getType()->isPointerTy() && Op1->getType()->isPointerTy()) {
        ...
        if (!ConstNull)
          ConstNull = dyn_cast<ConstantPointerNull>(Op0);
        if (ConstNull) {
