...
DyckGlobalValueFlowAnalysis::DyckGlobalValueFlowAnalysis(
...
void DyckGlobalValueFlowAnalysis::setVulnerabilityChecker(
...
void DyckGlobalValueFlowAnalysis::printOnlineQueryTime(
    llvm::raw_ostream &O, const char *Title) const {
  if (EnableOnlineQuery.getValue()) {
...
DyckGlobalValueFlowAnalysis::getWitnessPath(const Value *From,
...
bool DyckGlobalValueFlowAnalysis::onlineReachability(const Value *Target) {
  for (const auto &Sink : Sinks) {
    ...
    if (onlineBackwardReachability(Sink.first, Target, visited))
      return true;
  }
  return false;
}

bool DyckGlobalValueFlowAnalysis::onlineForwardReachability(
    const Value *V, std::unordered_set<const Value *> &visited) {
...
bool DyckGlobalValueFlowAnalysis::onlineBackwardReachability(
    const Value *V, const Value *Target,
    std::unordered_set<const Value *> &visited) {
  std::queue<const Value *> WorkQueue;
  WorkQueue.push(V);
...
bool DyckGlobalValueFlowAnalysis::cflReachable(const Value *From,
                                               const Value *To) const {
  return cflReachabilityQuery(From, To, true);
}

bool DyckGlobalValueFlowAnalysis::cflBackwardReachable(const Value *From,
                                                       const Value *To) const {
  return cflReachabilityQuery(To, From, false);
}

bool DyckGlobalValueFlowAnalysis::contextSensitiveReachable(
    const Value *From, const Value *To) const {
  return cflReachable(From, To);
}

bool DyckGlobalValueFlowAnalysis::contextSensitiveBackwardReachable(
    const Value *From, const Value *To) const {
  return cflBackwardReachable(From, To);
}

bool DyckGlobalValueFlowAnalysis::performCFLReachabilityQuery(
    const Value *From, const Value *To, bool Forward) const {
  return cflReachabilityQuery(From, To, Forward);
}

bool DyckGlobalValueFlowAnalysis::cflReachabilityQuery(const Value *From,
                                                       const Value *To,
                                                       bool Forward) const {
  std::unordered_set<const Value *> visited;
  ...
  while (!workQueue.empty()) {
    ...
    if (Forward) {
      for (auto edgeIt = currentNode->begin(); edgeIt != currentNode->end();
           ++edgeIt) {
      ...
      for (auto edgeIt = currentNode->in_begin();
           edgeIt != currentNode->in_end(); ++edgeIt) {
