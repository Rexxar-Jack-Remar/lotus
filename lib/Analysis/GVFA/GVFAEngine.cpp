...
int GVFAEngine::reachable(const Value *V, int Mask) {
  ...
  int UncoveredMask =
      (It != ReachabilityMap.end()) ? (Mask & ~(Mask & It->second)) : Mask;
...
bool GVFAEngine::backwardReachableSink(const Value *V) {
  ...
  // The original code used AllBackwardReachabilityMap for this check in
  // detailed mode? Wait, optimizedBackwardRun fills BackwardReachabilityMap.
  // But backwardReachableSink in original code (line 189) used
  // AllBackwardReachabilityMap. Ah, the original code had: backwardReachable ->
  // Checks BackwardReachabilityMap backwardReachableSink -> Checks
  // AllBackwardReachabilityMap This implies Fast mode CANNOT answer
  // backwardReachableSink correctly if it only has bitmasks? Or does
  // BackwardReachabilityMap store Sink IDs? Line 117:
  // BackwardReachabilityMap[V] = 1; -> It just counts *visits*, not masks. So
  // Fast mode just knows "it reaches SOME sink". So backwardReachableSink
  // should return true if backwardReachable returns true?
...
void GVFAEngine::extendSources(
    std::vector<std::pair<const Value *, int>> &Sources) {
  ...
  auto getPredecessors =
      [&PredCache, this](const Value *V) -> const std::vector<const Value *> & {
    ...
    if (auto *Arg = dyn_cast<Argument>(V)) {
      ...
      if (auto *Node = VFG->getVFGNode(const_cast<Value *>(V))) {
        for (auto ItPred = Node->in_begin(); ItPred != Node->in_end();
             ++ItPred) {
...
void GVFAEngine::processCallSite(
    const CallInst *CI, const Value *V, int Mask,
    std::queue<std::pair<const Value *, int>> &WorkQueue) {
  if (auto *F = CI->getCalledFunction()) {
...
void GVFAEngine::processReturnSite(
    const ReturnInst *RI, const Value *V, int Mask,
    std::queue<std::pair<const Value *, int>> &WorkQueue) {
