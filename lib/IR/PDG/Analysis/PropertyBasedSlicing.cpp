...
PropertyBasedSlicing::NodeSet
PropertyBasedSlicing::resolveCriteria(const Module &M,
                                      const PropertySpec &spec) const {
  NodeSet criteria;

  auto addInstructionNode = [&](const Instruction &I) {
    Node *N = _pdg.getNode(const_cast<Instruction &>(I));
    if (N != nullptr)
      criteria.insert(N);
  };

  for (const PropertyRule &rule : spec.rules()) {
    if (rule.kind == PropertyKind::UnreachCall ||
        rule.kind == PropertyKind::Assertions ||
        rule.kind == PropertyKind::CoverageErrorCall) {
      // For UnreachCall/CoverageErrorCall, find calls to target functions
      std::vector<std::string> targets;

      if (rule.kind == PropertyKind::Assertions) {
        targets = {"__assert_fail", "__verifier_error"};
      } else if (rule.target.empty()) {
        if (rule.kind == PropertyKind::UnreachCall)
          targets = {"reach_error", "__verifier_error"};
        else
          targets = {"reach_error"};
      } else {
        std::string targetLower = toLower(rule.target);
        targets.push_back(targetLower);
      ...
      for (const Function &F : M) {
        ...
        for (const BasicBlock &BB : F) {
          for (const Instruction &I : BB) {
            ...
            if (!Callee)
              continue;
            std::string CName = toLower(Callee->getName().str());
    ...
    const bool hasNullDerefMarkers =
        ...
        moduleHasAnyCall(M, {"__INSTR_mark_pointer"});
      ...
      for (const BasicBlock &BB : F) {
        for (const Instruction &I : BB) {
          ...
          case PropertyKind::Termination:
            if (isCallTo(I, {"__INSTR_fail", "__assert_fail",
                             "__VERIFIER_silent_exit", "__VERIFIER_exit",
                             "__INSTR_check_assume"}))
              addInstructionNode(I);
            break;
          case PropertyKind::NullDeref: {
            if (hasNullDerefMarkers) {
              if (isCallTo(I, {"__INSTR_mark_pointer"})) {
                if (const Instruction *Next = I.getNextNode())
                  addInstructionNode(*Next);
                else
                  addInstructionNode(I);
              }
            } else if (isNullDerefInstruction(I)) {
              addInstructionNode(I);
            }
            break;
          }
...
PropertyBasedSlicing::NodeSet
PropertyBasedSlicing::computeBackwardSlice(const Module &M,
                                           const PropertySpec &spec) const {
  const NodeSet criteria = resolveCriteria(M, spec);
  if (criteria.empty())
    return {};
  BackwardSlicing slicer(_pdg);
  std::set<EdgeType> deps = SlicingUtils::getDataDependencyEdges();
  const std::set<EdgeType> ctrl = SlicingUtils::getControlDependencyEdges();
  deps.insert(ctrl.begin(), ctrl.end());
  const std::set<EdgeType> param = SlicingUtils::getParameterDependencyEdges();
  deps.insert(param.begin(), param.end());
  return slicer.computeSlice(criteria, deps);
}

PropertyBasedSlicing::NodeSet
PropertyBasedSlicing::computeForwardSlice(const Module &M,
                                          const PropertySpec &spec) const {
  const NodeSet criteria = resolveCriteria(M, spec);
  if (criteria.empty())
    return {};
  ForwardSlicing slicer(_pdg);
  std::set<EdgeType> deps = SlicingUtils::getDataDependencyEdges();
  const std::set<EdgeType> ctrl = SlicingUtils::getControlDependencyEdges();
  deps.insert(ctrl.begin(), ctrl.end());
  const std::set<EdgeType> param = SlicingUtils::getParameterDependencyEdges();
