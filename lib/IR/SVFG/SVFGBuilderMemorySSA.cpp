...
bool SVFGBuilder::connectCallSiteToCalleeOnTheFly(
  ...
  const bool isDirectEdge =
      (directCallee && directCallee == callee && !directCallee->isDeclaration());
  ...
  for (SVFGNode *actualNode : g->getActualIns(cs)) {
    ...
    for (SVFGNode *formalNode : g->getFormalIns(callee)) {
      ...
      if (edgePts.empty())
        continue;
      if (SVFGEdge *e = g->addEdge(actualIn, formalIn, SVFGEdgeK::CallAIn, cs, edgePts)) {
        newEdges.push_back(e);
        created = true;
      }
    }
  ...
  for (SVFGNode *actualNode : g->getActualOuts(cs)) {
    ...
    for (SVFGNode *formalNode : g->getFormalOuts(callee)) {
      ...
      if (edgePts.empty())
        continue;
      if (SVFGEdge *e = g->addEdge(formalOut, actualOut, SVFGEdgeK::RetAOut, cs, edgePts)) {
        newEdges.push_back(e);
        created = true;
      }
    }
