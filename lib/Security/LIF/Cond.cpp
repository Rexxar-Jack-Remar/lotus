...
OutMap
lotus::lif::transform::allocOut(llvm::Function &F, const LoopWrapper &LW,
...
lotus::lif::transform::bindIn(llvm::BasicBlock &BB, llvm::Instruction *Before,
                              const OutMap OM, const LoopWrapper &LW,
                              const llvm::DenseSet<llvm::Value *> &Tainted,
  ...
  for (auto *Pred : predecessors(&BB)) {
    ...
    auto *PTy = P->getType();
    auto *AllocaP =
        new llvm::AllocaInst(PTy, 0, "pred.alloca", EntryInsertionPoint);

    // The initial value indicates that the edge Pred -> BB
    ...
    // Always spill and reload branch predicates. Partial linearization may
    // redirect edges so a direct use no longer dominates the rewritten
    // successor block.
    new llvm::StoreInst(InitVal, AllocaP, EntryInsertionPoint);
    new llvm::StoreInst(P, AllocaP, Pred->getTerminator());

    P = new llvm::LoadInst(PTy, AllocaP, "pred.load", BB.getFirstNonPHI());
...
lotus::lif::transform::bindOut(llvm::BasicBlock &BB, llvm::Value *OutPtr,
                               llvm::Value *FrozenPtr,
...
lotus::lif::transform::bindAll(llvm::Function &F, const OutMap OM,
                               const LoopWrapper &LW,
                               const llvm::DenseSet<llvm::Value *> &Tainted,
                               const llvm::DominatorTree &DT) {
  InMap IM(F.size());
...
llvm::Value *lotus::lif::transform::fold(const Incoming &In,
