...
lotus::lif::transform::IsochronousPass::run(llvm::Module &M,
  ...
  for (auto It = RPOT.begin(), End = RPOT.end(); It != End; ++It) {
    ...
    llvm::Function *NewF =
        augmentInterface(*F, WrappedTypes, UnwrappedTypes, IsTaintedCallee);

    // If F is untouched, keep using it; otherwise, replace with NewF:
    if (NewF) {
      if (TaintedCallees.contains(F))
        TaintedCallees.insert(NewF);
      for (size_t Idx = 0; Idx < F->arg_size(); Idx++) {
        auto *Arg = F->getArg(Idx);
        if (Tainted.contains(Arg))
          Tainted.insert(NewF->getArg(Idx));
      }
      Replace[F] = NewF;
      F = NewF;
    }

    WrappedFuncs[F] = std::make_unique<FuncWrapper>(wrapFunc(*F, Tainted, FAM));
    ...
    for (auto &MaybeTaintedBB : FW->F) {
      if (!Tainted.contains(&MaybeTaintedBB))
        continue;
