...
LenMap lotus::lif::transform::inferLength(
    ...
    auto normalizeLengthValue = [Int64Ty, Zero64](
                                    llvm::Value *Length,
                                    llvm::Instruction *Before) {
        if (!Length) return static_cast<llvm::Value *>(Zero64);
        if (Length->getType() == Int64Ty) return Length;
    ...
    for (auto &BB : F) {
        for (auto &I : BB) {
            ...
            if (Call && llvm::isAllocationFn(Call, TLI)) {
                llvm::Value *Length = Call->arg_empty() ? Zero64
...
llvm::SelectInst *lotus::lif::transform::ctsel(
...
void lotus::lif::transform::rewriteLoad(
...
void lotus::lif::transform::rewriteStore(
...
llvm::Value *lotus::lif::transform::rewriteExitingPredicate(
...
std::vector<llvm::BasicBlock *> lotus::lif::transform::influenceRegion(
...
void lotus::lif::transform::rewritePhis(
    ...
    for (auto &BB : FW->F) {
        ...
        for (auto &Phi : BB.phis()) {
            ...
            for (auto *From : Workset) {
                ...
                for (auto *It = UnlinkedIncs.begin(); It != UnlinkedIncs.end();
                     ++It) {
                    auto [OldFrom, Val] = *It;
        ...
        for (auto *Phi : Rewritten) Phi->eraseFromParent();
    }
}

std::vector<llvm::BasicBlock *> lotus::lif::transform::compactOrder(
    FuncWrapper *FW, llvm::FunctionAnalysisManager &FAM
) {
...
void lotus::lif::transform::plinearize(
    FuncWrapper *FW, llvm::FunctionAnalysisManager &FAM
) {
...
void lotus::lif::transform::rewriteFunc(
    FuncWrapper *FW, LenMap &LM, llvm::FunctionAnalysisManager &FAM
) {
...
FuncWrapper lotus::lif::transform::wrapFunc(
