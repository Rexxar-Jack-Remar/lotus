...
LenMap lotus::lif::transform::inferLength(
    llvm::Function &F,
  ...
  auto normalizeLengthValue = [Int64Ty, Zero64](llvm::Value *Length,
                                                llvm::Instruction *Before) {
    if (!Length)
      return static_cast<llvm::Value *>(Zero64);
    if (Length->getType() == Int64Ty)
      return Length;
    if (Length->getType()->isIntegerTy())
      return static_cast<llvm::Value *>(llvm::CastInst::CreateIntegerCast(
          Length, Int64Ty, false, "alloc.length", Before));
    return static_cast<llvm::Value *>(Zero64);
  };
  ...
  for (auto &Arg : F.args()) {
    ...
    auto FieldName = (Arg.hasName() ? Arg.getName().str() : "arg") + ".field";
    ...
    for (size_t Idx = 0; Idx < StructTy->getNumElements(); Idx++) {
      ...
      auto *LenPtr = llvm::GetElementPtrInst::CreateInBounds(
          ...
          FieldName + std::to_string(Idx) + ".length.ptr", InsertionPoint);
  ...
  for (auto &BB : F) {
    for (auto &I : BB) {
      ...
      if (Call && llvm::isAllocationFn(Call, TLI)) {
        llvm::Value *Length =
            Call->arg_empty() ? Zero64 : Call->getArgOperand(0);
        if (auto *Callee = Call->getCalledFunction()) {
          auto Name = Callee->getName();
          if (Name.equals("calloc") && Call->arg_size() >= 1)
            Length = Call->getArgOperand(0);
          else if (Name.equals("realloc") && Call->arg_size() >= 2)
            Length = Call->getArgOperand(1);
        }
        LM[Call] = makeLength({normalizeLengthValue(Length, &I)});
        continue;
      }
      ...
      assert(GEP->getNumIndices() >= 2);
      uint64_t FieldIdx =
          llvm::cast<llvm::ConstantInt>(GEP->getOperand(2))->getZExtValue();

      auto *GEPPtrOp = GEP->getPointerOperand();
      if (ValGEP) {
        LM[GEPPtrOp]->at(FieldIdx) =
            LM[ValGEP->getPointerOperand()]->at(FieldIdx);
...
llvm::SelectInst *lotus::lif::transform::ctsel(llvm::Value *Cond,
                                               llvm::Value *VTrue,
                                               llvm::Value *VFalse,
                                               llvm::Instruction *Before) {
    ...
    if (GEPSrcTy->isArrayTy()) {
      ...
      for (size_t Idx = 1; Idx < NumIndices; Idx++) {
        ...
        auto *IsIdxSafe = llvm::ICmpInst::Create(
            llvm::Instruction::ICmp, llvm::ICmpInst::ICMP_SLT, IdxVal, Length,
            "idx" + std::to_string(Idx) + ".safe", Before);
        IsSafe = llvm::BinaryOperator::CreateAnd(IsSafe, IsIdxSafe, "", Before);
      }
      ...
      auto *IsIdxSafe = llvm::ICmpInst::Create(
          llvm::Instruction::ICmp, llvm::ICmpInst::ICMP_SLT, GEP->getOperand(1),
          Length, "idx0.safe", Before);
...
void lotus::lif::transform::rewriteLoad(llvm::LoadInst &Load,
                                        llvm::AllocaInst *Shadow, LenMap &LM,
                                        llvm::Value *Cond) {
  Load.setOperand(
...
void lotus::lif::transform::rewriteStore(llvm::StoreInst &Store,
                                         llvm::AllocaInst *Shadow, LenMap &LM,
                                         llvm::Value *Cond) {
...
llvm::Value *
lotus::lif::transform::rewriteExitingPredicate(llvm::Instruction &P,
                                               llvm::PHINode &Phi) {
...
lotus::lif::transform::influenceRegion(llvm::BasicBlock *BB,
...
static void applyRewriteRules(FuncWrapper *FW, LenMap &LM,
  ...
  auto *ShadowTy = llvm::IntegerType::get(
      FW->F.getContext(),
      FW->F.getParent()->getDataLayout().getPointerSizeInBits());
  ...
  for (auto &MaybeTaintedBB : FW->F) {
    if (!FW->Tainted.contains(&MaybeTaintedBB))
      continue;
    ...
    for (auto *BB : influenceRegion(TaintedBB, PDom)) {
      ...
      InsertionPoint = InsertionPoint->getNextNode();
      auto *OutVal = new llvm::LoadInst(OutPtr->getAllocatedType(), OutPtr, "",
                                        InsertionPoint);

      FW->Skip.insert(OutVal);

      for (auto &I : *BB) {
        if (FW->Skip.contains(&I) || In.empty())
          continue;
...
void lotus::lif::transform::rewritePhis(FuncWrapper *FW,
                                        llvm::FunctionAnalysisManager &FAM) {
  ...
  for (auto &BB : FW->F) {
    ...
    for (auto &Phi : BB.phis()) {
      ...
      for (auto *From : Workset) {
        ...
        for (auto *It = UnlinkedIncs.begin(); It != UnlinkedIncs.end(); ++It) {
          auto [OldFrom, Val] = *It;
          if (!isReachableFrom(From, OldFrom))
            continue;
          ...
          if (llvm::isa<llvm::Constant>(Val) || DT.dominates(OldFrom, From)) {
          ...
          auto *MaybeUndef =
              llvm::PHINode::Create(Val->getType(), llvm::pred_size(From),
                                    "incval.phi", From->getFirstNonPHI());

          auto *InstVal = llvm::cast<llvm::Instruction>(Val);
          for (auto *Pred : llvm::predecessors(From)) {
            MaybeUndef->addIncoming(DT.dominates(InstVal, Pred)
                                        ? Val
      ...
      for (auto [OldFrom, Val] : UnlinkedIncs) {
        auto *Cond = llvm::dyn_cast<llvm::Instruction>(FW->IM[&BB][OldFrom]);
        if (Cond && (!InsPoint || InsPoint->comesBefore(Cond)))
          InsPoint = Cond->getNextNode();
      }
      if (!InsPoint)
        InsPoint = BB.getFirstNonPHI();

      for (auto [OldFrom, Val] : UnlinkedIncs) {
        llvm::Value *Cond = FW->IM[&BB][OldFrom];

        // FIXME: Val might not have been defined here.
        PhiSelect = llvm::SelectInst::Create(Cond, Val, PhiSelect, "phi.fold",
                                             InsPoint);
      }
    ...
    for (auto *Phi : Rewritten)
      Phi->eraseFromParent();
  }
}

std::vector<llvm::BasicBlock *>
lotus::lif::transform::compactOrder(FuncWrapper *FW,
                                    llvm::FunctionAnalysisManager &FAM) {
...
void lotus::lif::transform::plinearize(FuncWrapper *FW,
                                       llvm::FunctionAnalysisManager &FAM) {
  ...
  for (auto *BB : CompactOrder) {
    ...
    // algorithm):
      ...
      auto *Br = llvm::cast<llvm::BranchInst>(BB->getTerminator());
      auto *NewBr = llvm::BranchInst::Create(Next);
      llvm::ReplaceInstWithInst(Br, NewBr);
    }
...
void lotus::lif::transform::rewriteFunc(FuncWrapper *FW, LenMap &LM,
                                        llvm::FunctionAnalysisManager &FAM) {
...
extendStruct(llvm::LLVMContext &Ctx, llvm::StructType *StructTy,
             ...
             llvm::DenseMap<llvm::Type *, llvm::Type *> &UnwrappedTypes) {
  auto TypeName = (StructTy->hasName() ? StructTy->getName().str() : "struct") +
                  ".extended.ty";
...
llvm::Function *lotus::lif::transform::augmentInterface(
    llvm::Function &F, llvm::DenseMap<llvm::Type *, llvm::Type *> &WrappedTypes,
    llvm::DenseMap<llvm::Type *, llvm::Type *> &UnwrappedTypes,
  ...
  if (auto *const G = M->getGlobalVariable("llvm.global.annotations")) {
    ...
    for (const auto &Element : Array->operands()) {
      ...
      Annotation.drop_front(std::string("nowrap:").size()).split(Indices, ",");
  ...
  for (size_t I = 0; I < F.arg_size(); I++) {
    ...
    auto *ExtStructTy =
        extendStruct(Ctx, StructTy, WrappedTypes, UnwrappedTypes);
  ...
  if (ModifiedArgIdx.empty() && !IsTaintedCallee)
    return nullptr;
  ...
  for (auto Idx : ModifiedArgIdx) {
    ...
    for (auto &Use : Arg->uses()) {
      ...
      auto *StructGEP =
          llvm::GetElementPtrInst::Create(NewArgElTy, NewArg, Indices, "", GEP);
      ...
      // we add a second GEP:
      auto *UnwrapLoad = new llvm::LoadInst(StructGEP->getResultElementType(),
                                            StructGEP, "", GEP);

      auto *UnwrapGEP = llvm::GetElementPtrInst::CreateInBounds(
          UnwrapLoad->getType()->getPointerElementType(), UnwrapLoad,
          {Zero, Zero}, Name + ".field" + std::to_string(Idx) + ".unwrapped",
          GEP);
...
void lotus::lif::transform::fixCallSites(
  ...
  for (auto It = OldF->use_begin(); It != End; ++It) {
    ...
    for (size_t I = 0; I < Call->arg_size(); I++) {
      ...
      auto *Alloca = new llvm::AllocaInst(NewArgElTy, 0, Name + ".wrapped",
                                          InsertionPoint);
      ...
      if (!StructTy) {
        auto *GEP = llvm::GetElementPtrInst::CreateInBounds(
            NewArgElTy, Alloca, {Zero, Zero}, Name + ".unwrapped", Call);
      ...
      for (size_t Idx = 0; Idx < StructTy->getNumElements(); Idx++) {
        ...
        if (!WrappedTypes.count(FieldTy)) {
          auto *To = llvm::GetElementPtrInst::CreateInBounds(
              NewArgElTy, Alloca, {Zero, FIdx}, FieldName + ".to", Call);
          new llvm::StoreInst(new llvm::LoadInst(FieldTy, From, "", Call), To,
                              Call);
          continue;
        }
        ...
        auto *To = llvm::GetElementPtrInst::CreateInBounds(
            NewFieldElTy, AllocaField, {Zero, Zero}, FieldName + ".to", Call);

        new llvm::StoreInst(new llvm::LoadInst(FieldTy, From, "", Call), To,
                            Call);

        auto *WrappedTo = llvm::GetElementPtrInst::CreateInBounds(
            NewArgElTy, Alloca, {Zero, FIdx}, FieldName + ".wrapped.to", Call);
...
FuncWrapper
lotus::lif::transform::wrapFunc(llvm::Function &F,
