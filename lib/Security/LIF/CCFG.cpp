...
CCFG::Node *lotus::lif::transform::CCFG::Node::make(llvm::BasicBlock *BB,
...
CCFG::ChildrenTy
lotus::lif::transform::CCFG::Node::successors(const llvm::LoopInfo &LI,
                                              bool Invalidate) {
  if (!Invalidate && !this->Successors.empty())
  ...
  if (std::holds_alternative<llvm::BasicBlock *>(this->Val)) {
    ...
    for (auto *Succ : llvm::successors(BB)) {
      ...
      if (L && L->isLoopLatch(BB) && Header == Succ)
        continue;
...
llvm::BasicBlock *lotus::lif::transform::CCFG::Node::asBasicBlock() {
...
std::vector<CCFG::Node *>
lotus::lif::transform::CCFG::postOrder(Node *Entry, const llvm::LoopInfo &LI) {
...
std::vector<CCFG::Node *>
lotus::lif::transform::CCFG::compactOrder(Node *Entry, const llvm::LoopInfo &LI,
...
void lotus::lif::transform::CCFG::prettyPrint(
  ...
  for (size_t Idx = 0; Idx < PrintHeader.size(); Idx++)
    llvm::errs() << "=";
  llvm::errs() << "\n";
}
