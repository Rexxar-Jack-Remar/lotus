...
const char *keyToString(SifaStats::Key k) {
  switch (k) {
#define SIFA_KEY(x) case SifaStats::Key::x: return #x;
...
#define SIFA_KEY(x) if (s == #x) return SifaStats::Key::x;
  SIFA_KEY(OVERALL_TIME)
  SIFA_KEY(ICFG_INTERPRETER_ENTERED_PROCEDURES)
  SIFA_KEY(DAG_INTERPRETER_EARLY_EXIT_QUERIES_NONTRIVIAL)
  SIFA_KEY(DAG_INTERPRETER_EARLY_EXITS)
...
std::vector<std::string> SifaStats::getKeys() const {
  ...
  for (int i = 0; i < static_cast<int>(Key::FLUID_ABSTRACTIONS_APPLIED) + 1; ++i) {
...
void SifaStats::increment(Key k, std::uint64_t by) {
  counters_[k] += by;
}

void SifaStats::add(Key k, std::uint64_t summand) {
  counters_[k] += summand;
}
