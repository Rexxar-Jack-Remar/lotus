    ...
    for (const auto& kv : state.getPostHeap().getEdges()) {
        ...
        if (summary_it != summary_->getPostHeap().getEdges().end()) {
            for (const auto& edge_kv : kv.second) {
                if (summary_it->second.find(edge_kv.first) != summary_it->second.end()) {
                    // Edge exists in both - keep it
                    intersection.getPostHeap().addEdge(kv.first, edge_kv.first, edge_kv.second);
                }
            }
        }
    }
...
NonDisjunctiveSummary NonDisjunctiveSummary::join(const NonDisjunctiveSummary& s1, const NonDisjunctiveSummary& s2) {
    if (s1.isEmpty()) {
        return s2.clone();
    }
    if (s2.isEmpty()) {
    ...
    return NonDisjunctiveSummary(std::make_unique<AbductiveDomain>(domain.getSummary()->clone()));
}
