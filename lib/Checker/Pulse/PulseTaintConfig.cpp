...
bool TaintConfig::isSink(const llvm::Function* func, std::vector<TaintKind>& sink_kinds) const {
    sink_kinds.clear();
...
std::vector<TaintKind> TaintConfig::isSanitizer(const llvm::Function* func) const {
    std::vector<TaintKind> kinds;
...
const std::vector<SinkPolicy>& TaintConfig::getSinkPolicies(const TaintKind& sink_kind) const {
...
std::vector<TaintKind> TaintConfig::getAllocationSourceKinds(const std::string& class_name) const {
...
void TaintConfig::initializeDefault() {
    if (default_config_) return;
    
    default_config_ = new TaintConfig();
    
    // Common taint sources
    ProcedureMatcher read_matcher(ProcedureMatcher::Type::ProcedureNameRegex, "read|fread|recv");
    ProcedureUnit read_source(read_matcher, {TaintKind::UserInput()}, ProcedureTarget::ReturnValue);
    default_config_->addSourceProcedure(read_source);
    
    ProcedureMatcher getenv_matcher(ProcedureMatcher::Type::ProcedureName, "getenv");
    ProcedureUnit getenv_source(getenv_matcher, {TaintKind::Environment()}, ProcedureTarget::ReturnValue);
    default_config_->addSourceProcedure(getenv_source);
    
    // Common taint sinks
    ProcedureMatcher system_matcher(ProcedureMatcher::Type::ProcedureName, "system");
    ProcedureUnit system_sink(system_matcher, {TaintKind::UserInput()}, ProcedureTarget::AllArguments);
    default_config_->addSinkProcedure(system_sink);
    
    ProcedureMatcher exec_matcher(ProcedureMatcher::Type::ProcedureNameRegex, "exec|popen");
    ProcedureUnit exec_sink(exec_matcher, {TaintKind::UserInput()}, ProcedureTarget::AllArguments);
    default_config_->addSinkProcedure(exec_sink);
    
    // Common sanitizers
    ProcedureMatcher sanitizer_matcher(ProcedureMatcher::Type::ProcedureNameRegex, "strlen|atoi|strtol");
    ProcedureUnit sanitizer_unit(sanitizer_matcher, {TaintKind::Unknown()}, ProcedureTarget::AllArguments);
    default_config_->addSanitizerProcedure(sanitizer_unit);
    
    // Sink policies
    SinkPolicy command_injection(1, "Command injection");
    command_injection.source_kinds = {TaintKind::UserInput(), TaintKind::Network()};
