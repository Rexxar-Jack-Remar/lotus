...
#include "Checker/Pulse/PulseExecutionDomain.h"
...
std::map<std::string, std::chrono::steady_clock::time_point> PulseLogger::timers_;
std::map<std::string, unsigned> PulseLogger::counters_;

void PulseLogger::setLevel(LogLevel level) {
...
void PulseLogger::log(LogLevel level, const std::string& prefix, const std::string& msg) {
...
std::string PulseLogger::formatLocation(const llvm::BasicBlock* BB) {
    ...
    if (name.empty()) {
        ...
        oss << "BB#" << std::distance(BB->getParent()->begin(), 
...
void PulseLogger::logFunction(const llvm::Function* F, const std::string& action) {
...
void PulseLogger::logBlock(const llvm::BasicBlock* BB, const std::string& action) {
...
void PulseLogger::logInstruction(const llvm::Instruction* I, const std::string& action) {
...
void PulseLogger::logBug(OperationResult kind, const llvm::Instruction* loc, const std::string& details) {
    std::ostringstream oss;
    oss << "BUG DETECTED: ";
...
void PulseLogger::printStats() {
    if (current_level_ >= LogLevel::Info) {
        ...
        oss2 << "Analysis: " << functions_analyzed << " functions analyzed, "
             ...
             << summaries_applied << " summaries applied, "
             << joins_performed << " joins performed, "
