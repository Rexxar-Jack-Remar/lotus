...
AbductiveDomain SpecializationManager::applySpecialization(const AbductiveDomain& astate,
    ...
    for (const auto& kv : key.dynamic_types) {
        ...
        // 2. Add type constraint to formula using PulseArithmetic.and_dynamic_type_is
        // For now, we track it in need_dynamic_type_specialization
...
/**
 * Helper: initialize heap path in domain (similar to Infer's initialize_heap_path)
 */
...
/**
 * Helper: prune equalities for a list of values (similar to Infer's prune_eq_list_values)
 */
static AbductiveDomain pruneEqListValues(const std::vector<AbstractValue>& values,
