...
SummaryEntry::SummaryEntry(std::unique_ptr<AbductiveDomain> pre,
    ...
    : pre_(std::move(pre)),
      pre_formula_(std::move(pre_formula)),
      post_(std::move(post)),
      post_formula_(std::move(post_formula)),
      return_value_(ret_val),
      latent_issue_(std::move(latent_issue)) {
}
...
PulseSummary::PulseSummary(const llvm::Function* F,
    ...
    pre_post_list_.emplace_back(
        std::move(pre), std::move(pre_formula),
        std::move(post), std::move(post_formula),
        ret_val);
}

llvm::Optional<AbstractValue> PulseSummary::getFormalAV(const llvm::Value* formal) const {
...
PulseSummary PulseSummary::join(const PulseSummary& s1, const PulseSummary& s2) {
...
void SummaryManager::storeSummary(const llvm::Function* F, PulseSummary summary) {
    summaries_.erase(F);
