...
void PulseModels::registerStandardModels() {
    // Memory allocation
    models_["malloc"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelMalloc(call, state, pred);
    };
    models_["calloc"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelCalloc(call, state, pred);
    };
    models_["free"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFree(call, state, pred);
    };
    ...
    models_["fclose"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFileClose(call, state, pred);
    };
    models_["open"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFileOpen(call, state, pred);
    };
    models_["close"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFileClose(call, state, pred);
    };
    models_["fdopen"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFileOpen(call, state, pred);
    };
    models_["opendir"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFileOpen(call, state, pred);
    };
    models_["closedir"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFileClose(call, state, pred);
    };
    
    // String operations
    models_["strcpy"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelStringCopy(call, state, pred);
    };
    models_["strncpy"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelStringCopy(call, state, pred);
    };
    models_["strdup"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelStrdup(call, state, pred);
    };
    models_["strchr"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelStrchr(call, state, pred);
    };
    models_["strstr"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    ...
    // I/O operations
    models_["read"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelRead(call, state, pred);
    };
    models_["write"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelWrite(call, state, pred);
    };
    models_["fread"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFread(call, state, pred);
    };
    models_["fwrite"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFwrite(call, state, pred);
    };
    models_["fgets"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelFgets(call, state, pred);
    };
    models_["gets"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelGets(call, state, pred);
    };
    
    // Locking
    models_["pthread_mutex_lock"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
...
void PulseModels::registerCppModels() {
    ...
    // std::unique_ptr
    models_["_ZNSt10unique_ptrI*E7releaseEv"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelStdUniquePtrRelease(call, state, pred);
    };
    models_["_ZNSt10unique_ptrI*E5resetE*"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelStdUniquePtrReset(call, state, pred);
    };
    models_["_ZNSt10unique_ptrI*E3getEv"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
        return modelStdUniquePtrGet(call, state, pred);
    };
    models_["_ZNSt10unique_ptrI*ED1Ev"] = [this](PulseChecker&, const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
...
ModelResult PulseModels::modelMalloc(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    auto* astate = state.getAstate();
    if (!astate) return ModelResult::success({state});
...
ModelResult PulseModels::modelFree(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
...
ModelResult PulseModels::modelRealloc(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
...
ModelResult PulseModels::modelFileOpen(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    auto* astate = state.getAstate();
    if (!astate) return ModelResult::success({state});
...
ModelResult PulseModels::modelFileClose(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 1) return ModelResult::success({state});
...
ModelResult PulseModels::modelLock(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    // Simplified lock model: check if already locked?
    // For now just no-op or state update
    return ModelResult::success({state});
}

ModelResult PulseModels::modelUnlock(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 1) return ModelResult::success({state});
...
ModelResult PulseModels::modelStringCopy(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 2) return ModelResult::success({state});
    ...
    if (dest_opt && src_opt) {
        ...
        AbstractValue dummy = factory_.createFresh(call); // Dummy value representing the string content
...
ModelResult PulseModels::modelStdVectorPushBack(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
...
ModelResult PulseModels::modelStdVectorAccess(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    // vector::operator[]
    // Check bounds?
    return ModelResult::success({state});
}

ModelResult PulseModels::modelStdVectorData(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    return ModelResult::success({state});
}

ModelResult PulseModels::modelStdStringCStr(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    return ModelResult::success({state});
}

ModelResult PulseModels::modelSocket(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    return modelFileOpen(call, state, pred); // Treat socket like file for now
}

ModelResult PulseModels::modelTaintSource(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred, const std::string& kind) {
    // TODO: Mark return value as tainted
    return ModelResult::success({state});
}

ModelResult PulseModels::modelTaintSink(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred, const std::string& kind) {
    // TODO: Check if args are tainted
    return ModelResult::success({state});
}

// C++ Smart Pointers (reusing existing implementations in header, just ensuring they are linked)
// ... (The header implementation is sufficient if inline, otherwise move here)
// Since I removed them from header to avoid redefinition if I included it, I should move them here if they were inline.
// But wait, the previous `Read` showed them as member functions in `PulseModels.h` but implemented in `PulseModels.cpp`?
// No, the previous `Read` of `PulseModels.cpp` (before I overwrote it) showed them implemented there.
// I have preserved them in my new `PulseModels.cpp`.

ModelResult PulseModels::modelStdUniquePtrRelease(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 1) return ModelResult::success({state});
    auto* astate = state.getAstate();
    auto this_ptr_opt = ops_.eval(*astate, call->getArgOperand(0), call, pred);
    if (!this_ptr_opt) return ModelResult::success({state});
    ...
    AbstractValue null_val = factory_.getOrCreate(llvm::Constant::getNullValue(call->getType()));
...
ModelResult PulseModels::modelStdUniquePtrReset(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 1) return ModelResult::success({state});
    auto* astate = state.getAstate();
    auto this_ptr_opt = ops_.eval(*astate, call->getArgOperand(0), call, pred);
    if (!this_ptr_opt) return ModelResult::success({state});
    ...
    if (call->arg_size() > 1) {
        ...
        if (val_opt) new_addr = *val_opt;
        else new_addr = Address(factory_.createFresh(call));
    } else {
        AbstractValue null_val = factory_.getOrCreate(llvm::Constant::getNullValue(llvm::PointerType::get(call->getContext(), 0)));
...
ModelResult PulseModels::modelStdUniquePtrGet(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 1) return ModelResult::success({state});
    auto* astate = state.getAstate();
    auto this_ptr_opt = ops_.eval(*astate, call->getArgOperand(0), call, pred);
    if (!this_ptr_opt) return ModelResult::success({state});
...
ModelResult PulseModels::modelStdUniquePtrDtor(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 1) return ModelResult::success({state});
    auto* astate = state.getAstate();
    auto this_ptr_opt = ops_.eval(*astate, call->getArgOperand(0), call, pred);
    if (!this_ptr_opt) return ModelResult::success({state});
...
ModelResult PulseModels::modelCalloc(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    auto* astate = state.getAstate();
    if (!astate) return ModelResult::success({state});
...
ModelResult PulseModels::modelStrdup(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 1) return ModelResult::success({state});
    
    auto* astate = state.getAstate();
    auto src_opt = ops_.eval(*astate, call->getArgOperand(0), call, pred);
    if (!src_opt) return ModelResult::success({state});
...
ModelResult PulseModels::modelStrchr(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 2) return ModelResult::success({state});
    
    auto* astate = state.getAstate();
    auto str_opt = ops_.eval(*astate, call->getArgOperand(0), call, pred);
    if (!str_opt) return ModelResult::success({state});
...
ModelResult PulseModels::modelStrstr(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 2) return ModelResult::success({state});
...
ModelResult PulseModels::modelMemcpy(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 3) return ModelResult::success({state});
...
ModelResult PulseModels::modelRead(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 3) return ModelResult::success({state});
...
ModelResult PulseModels::modelWrite(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 3) return ModelResult::success({state});
...
ModelResult PulseModels::modelFread(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 4) return ModelResult::success({state});
...
ModelResult PulseModels::modelFwrite(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 4) return ModelResult::success({state});
    ...
    if (stream_opt && ptr_opt) {
        ...
        if (stream_read.first == OperationResult::Success && ptr_read.first == OperationResult::Success) {
...
ModelResult PulseModels::modelFgets(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 3) return ModelResult::success({state});
...
ModelResult PulseModels::modelGets(const llvm::CallInst* call, ExecutionDomain& state, const llvm::BasicBlock* pred) {
    if (call->arg_size() < 1) return ModelResult::success({state});
