...
static bool materializePreFromAddress(
    ...
    for (const auto& edge_kv : pre_it->second) {
        ...
        const Address* caller_target = caller_astate.getPostHeap().findEdge(caller_addr, access);
        if (!caller_target) {
            ...
            if (!materializePreFromAddress(ops, factory, callee_pre, formal_target.addr,
                                         fresh_target, caller_astate, substitution, visited)) {
                return false;
            }
        } else {
            // Edge exists - check if we need to update substitution
            AbstractValue caller_target_av = caller_astate.getCanonical(caller_target->addr);
            AbstractValue formal_target_canon = callee_pre->getCanonical(formal_target.addr);
            ...
            if (!substitution.substitute(formal_target_canon)) {
                ...
                // Recursively materialize from target
                if (!materializePreFromAddress(ops, factory, callee_pre, formal_target.addr,
                                             caller_target_av, caller_astate, substitution, visited)) {
                    return false;
                }
            }
        }
...
static bool materializePreFromFormal(
    ...
    auto stack_it = std::find_if(pre_stack.begin(), pre_stack.end(),
        [&](const auto& kv) {
    ...
    return materializePreFromAddress(ops, factory, pre, formal_addr_in_pre,
                                    actual_canon, caller_astate, substitution, visited);
}
 ...
 */
static void applyPostCondition(
    const AbductiveDomain* callee_post,
    const AbductiveDomain* callee_pre,
    AbductiveDomain& caller_astate,
    Substitution& substitution,
    AbstractValueFactory& factory) {
    ...
    for (const auto& kv : callee_post->getPostHeap().getEdges()) {
        ...
        AbstractValue caller_from = caller_from_opt ? *caller_from_opt : factory.createFresh();
        if (!caller_from_opt) {
        ...
        for (const auto& edge_kv : kv.second) {
            ...
            // Substitute target
            AbstractValue formal_target_canon = callee_post->getCanonical(formal_target.addr);
            auto caller_target_opt = substitution.substitute(formal_target_canon);
            AbstractValue caller_target = caller_target_opt ? *caller_target_opt : factory.createFresh();
            ...
            // Add or update edge
            caller_astate.getPostHeap().addEdge(caller_from, access, caller_target_addr);
        }
    }
...
std::vector<ExecutionDomain> PulseChecker::applySummaryImproved(
    ...
    for (const auto& entry : summary.getPrePostList()) {
        ...
        // We need to extract captured variables from it
        // Infer handles this via CapturedFormalActualLength contradiction
        ...
        if (CI->arg_size() > 0) {
            ...
            if (first_arg->getType()->isPointerTy()) {
                ...
                if (closure_opt) {
                    ...
                    // This is simplified - full implementation would traverse closure struct
                    AbstractValue closure_addr = new_astate->getCanonical(closure_opt->addr);
        ...
        // (In full implementation, we'd extract captured formals from callee signature)
        if (is_closure_call && !captured_actuals.empty()) {
            // Simplified: assume captured formals match if closure structure exists
            // Full implementation would:
            // 1. Extract captured formals from callee signature (from function attributes/metadata)
        ...
        for (const auto& Arg : callee->args()) {
            ...
            if (formal_av_opt) {
                ...
                if (!materializePreFromFormal(ops_, factory_, pre, summary, &Arg, actual_addr,
                                              *new_astate, substitution)) {
                    entry_failed = true;
                    break;
                }
            }
        ...
        PulseFormula callee_pre_formula = entry.getPreFormula().applySubstitution(substitution);
        
        // Enhanced contradiction detection using new module
        auto contradiction_opt = checkContradiction(
            caller_formula, callee_pre_formula, formal_to_actual_map, actual_to_formals_map);
        ...
        PulseFormula test_merge = PulseFormula::merge(caller_formula, callee_pre_formula);
        ...
        PulseFormula merged_pre = PulseFormula::merge(caller_formula, callee_pre_formula);
        ...
        if (merged_post.isConsistent()) {
            new_astate->setPathFormula(std::make_unique<PulseFormula>(std::move(merged_post)));
        }
        ...
        if (entry.getLatentIssue()) {
            ...
            AbstractValue caller_addr = caller_addr_opt ? *caller_addr_opt : factory_.createFresh(CI);
            if (!caller_addr_opt) {
            ...
            if (LatentIssue::isManifest(*new_astate)) {
                ...
                latent_issues_.emplace_back(latent.diagnostic,
                                            LatentIssue::issueKindFromResult(latent.diagnostic),
                                            caller_addr, CI, issue_trace.clone());
