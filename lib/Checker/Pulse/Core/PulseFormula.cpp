...
bool PulseFormula::hasNullConstraints() const {
    return !null_values_.empty();
}
    ...
    if (has_bounds) {
        ...
        switch (constraint.kind) {
            case ConstraintKind::LessEqual:
                if (min_value > rhs) return false;  // Contradiction
                break;
            case ConstraintKind::Less:
                if (min_value >= rhs) return false;
                break;
            case ConstraintKind::GreaterEqual:
                if (max_value < rhs) return false;
                break;
            case ConstraintKind::Greater:
                if (max_value <= rhs) return false;
                break;
            case ConstraintKind::Equal:
                if (min_value > rhs || max_value < rhs) return false;
                break;
        }
    }
...
bool PulseFormula::checkSatisfiability() const {
    ...
    if (linear_constraints_.empty() && lower_bounds_.empty() && upper_bounds_.empty() &&
        ...
        non_null_values_.empty() && arithmetic_ops_.empty()) {
        return true;
    }

    // Quick consistency check: if we have both null and non-null for same value, it's UNSAT
    for (AbstractValue null_val : null_values_) {
    ...
    try {
        ...
        for (const auto& neq : disequalities_) {
            ...
            AbstractValue rep2 = const_cast<PulseFormula*>(this)->findRep(neq.second);
...
void PulseFormula::propagateBounds() {
    ...
    for (const auto& eq : equalities_) {
        ...
        if (lower1 != lower_bounds_.end() || lower2 != lower_bounds_.end()) {
            ...
            if (lower1 != lower_bounds_.end()) new_lower = std::max(new_lower, lower1->second);
            if (lower2 != lower_bounds_.end()) new_lower = std::max(new_lower, lower2->second);
        ...
        if (upper1 != upper_bounds_.end() || upper2 != upper_bounds_.end()) {
            ...
            if (upper1 != upper_bounds_.end()) new_upper = std::min(new_upper, upper1->second);
            if (upper2 != upper_bounds_.end()) new_upper = std::min(new_upper, upper2->second);
...
bool PulseFormula::addArithmeticOperation(AbstractValue result, AbstractValue op1, 
                                          AbstractValue op2, const std::string& op) {
    if (op == "*") return addMultiply(result, op1, op2);
    if (op == "/") return addDivide(result, op1, op2);
    if (op == "%") return addModulo(result, op1, op2);
    if (op == "&") return addBitwiseAnd(result, op1, op2);
    if (op == "|") return addBitwiseOr(result, op1, op2);
    if (op == "^") return addBitwiseXor(result, op1, op2);
    if (op == "<<") return addLeftShift(result, op1, op2);
    if (op == ">>") return addRightShift(result, op1, op2);
