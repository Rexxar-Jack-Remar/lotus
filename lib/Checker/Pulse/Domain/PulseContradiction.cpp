...
llvm::Optional<Contradiction> checkAliasingWithAllAliases(
    ...
    const std::map<AbstractValue, std::set<AbstractValue>>& actual_to_formals_map,
    const PulseFormula& callee_pre_formula) {
    return checkAliasingWithAllAliases(formal_to_actual_map, actual_to_formals_map, 
                                       callee_pre_formula, nullptr);
}

/**
 * Check for DynamicTypeNeeded contradiction
 * Returns map of heap paths to abstract values that need dynamic type specialization
 */
...
llvm::Optional<Contradiction> checkCapturedFormalActualLength(
    ...
    if (captured_formal_count != captured_actual_count) {
        return Contradiction::makeCapturedFormalActualLength(
            captured_formal_count, captured_actual_count, captured_formals, captured_actuals);
    }
 ...
 */
llvm::Optional<Contradiction> mergeContradictions(
    const llvm::Optional<Contradiction>& c1,
    ...
    if (cont1.kind == cont2.kind) {
        switch (cont1.kind) {
            case ContradictionKind::AliasingWithAllAliases: {
                // Merge alias classes
                std::vector<std::vector<HeapPath>> merged_classes = cont1.alias_classes_with_paths;
            ...
            case ContradictionKind::DynamicTypeNeeded: {
                ...
                merged_paths.insert(cont2.dynamic_type_paths.begin(), cont2.dynamic_type_paths.end());
                return Contradiction::makeDynamicTypeNeeded(merged_paths);
            }
            case ContradictionKind::PathCondition: {
                // Merge conflicting constraints
                std::vector<std::string> merged_conflicts = cont1.path_condition.conflicting_constraints;
                merged_conflicts.insert(merged_conflicts.end(),
    ...
    // Different kinds: prioritize DynamicTypeNeeded > AliasingWithAllAliases > others
    if (cont1.kind == ContradictionKind::DynamicTypeNeeded) return c1;
    if (cont2.kind == ContradictionKind::DynamicTypeNeeded) return c2;
    if (cont1.kind == ContradictionKind::AliasingWithAllAliases) return c1;
    if (cont2.kind == ContradictionKind::AliasingWithAllAliases) return c2;
...
llvm::Optional<Contradiction> checkContradictionWithCallState(
    ...
    auto aliasing_contradiction = checkAliasingWithAllAliases(
        formal_to_actual_map, actual_to_formals_map, callee_pre_formula, call_state);
    if (aliasing_contradiction) {
        return aliasing_contradiction;
    }
