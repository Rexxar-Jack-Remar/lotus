...
ConcurrencyAnalysisDumper::ConcurrencyAnalysisDumper(
    ...
    m_mhpAnalysis(mhpAnalysis),
    m_locksetAnalysis(locksetAnalysis),
    m_escapeAnalysis(escapeAnalysis),
...
std::string ConcurrencyAnalysisDumper::improveLocation(
...
void ConcurrencyAnalysisDumper::dumpText(raw_ostream& os, DebugInfoAnalysis& debugInfo) const {
    os << "\n";
    os << "========================================\n";
...
mhp_done:
    ...
    for (Function& func : m_module) {
        ...
        for (inst_iterator I = inst_begin(func), E = inst_end(func); I != E; ++I) {
            ...
            // Show lock sets for: lock operations, memory accesses, and function entries
            ...
            if (!mayLocks.empty()) {
                ...
                for (LockID lock : mayLocks) {
                    ...
                    if (lockName.empty()) {
                        lockName = "lock@" + std::to_string(reinterpret_cast<uintptr_t>(lock));
                    }
                    os << lockName;
                    first = false;
                }
    ...
    for (Function& func : m_module) {
        ...
        for (inst_iterator I = inst_begin(func), E = inst_end(func); I != E; ++I) {
            ...
            if (auto* load = dyn_cast<LoadInst>(inst)) {
                ...
                if (m_escapeAnalysis->isEscaped(ptr) && shownValues.insert(ptr).second) {
                    if (escapedCount++ >= maxEscapedToShow) {
                        os << "... (showing first " << maxEscapedToShow << " escaped values)\n";
                        goto escape_done;
                    }
...
escape_done:
    ...
    for (const auto& pair : threadInstructions) {
        ...
        if (pair.second.size() > sampleSize) {
            os << "    ... (" << (pair.second.size() - sampleSize) << " more instructions)\n";
        }
    }
    os << "\n";
        ...
        for (inst_iterator I = inst_begin(func), E = inst_end(func); I != E; ++I) {
            ...
            if (m_threadAPI->isTDAcquire(inst) || m_threadAPI->isTDRelease(inst)) {
                if (lockOpCount++ >= maxLockOpsToShow) {
                    os << "... (showing first " << maxLockOpsToShow << " lock operations)\n";
                    goto lockops_done;
                }
                ...
                std::string loc = improveLocation(inst, debugInfo, funcName);
                std::string src = debugInfo.getSourceCodeStatement(inst);
                std::string opType = m_threadAPI->isTDAcquire(inst) ? "ACQUIRE" : "RELEASE";
...
void ConcurrencyAnalysisDumper::dumpJSON(raw_ostream& os, DebugInfoAnalysis& debugInfo) const {
    os << "{\n";
    ...
    for (size_t i = 0; i < allInsts.size(); ++i) {
        ...
        for (size_t j = i + 1; j < allInsts.size(); ++j) {
            ...
            ++mhpPairCount;
                ...
                os << "        \"type\": \"" << (isa<LoadInst>(inst1) ? "load" : isa<StoreInst>(inst1) ? "store" : "other") << "\"\n";
                os << "      },\n";
                os << "      \"instruction2\": {\n";
...
mhp_json_done:
    ...
    for (Function& func : m_module) {
        ...
        for (inst_iterator I = inst_begin(func), E = inst_end(func); I != E; ++I) {
            ...
            if (m_threadAPI->isTDAcquire(inst) || m_threadAPI->isTDRelease(inst)) {
                if (lockOpCount++ >= maxLockOpsToShow) goto lockops_json_done;
        ...
        for (inst_iterator I = inst_begin(func), E = inst_end(func); I != E; ++I) {
            ...
            if (auto* load = dyn_cast<LoadInst>(inst)) {
                ...
                if (m_escapeAnalysis->isEscaped(ptr) && shownValues.insert(ptr).second) {
                    ...
                    std::string varName = debugInfo.getVariableName(ptr);
                    if (varName.empty()) {
                        varName = "value@" + std::to_string(reinterpret_cast<uintptr_t>(ptr));
                    }
