...
static cl::OptionCategory SaberCategory("Saber Options",
                                        "Options for Saber source-sink checker");
...
cl::opt<unsigned> SaberMaxForwardItems(
    ...
    cl::desc("Max (node,context) items per source in forward traversal (0 = no limit); safety cap only"),
    cl::init(0u),
    cl::cat(SaberCategory));
...
cl::opt<bool> SaberDumpSlice(
    "saber-dump-slice",
    cl::desc("Dump slice (annotate and dump SVFG)"),
    cl::init(false),
    cl::cat(SaberCategory));

cl::opt<bool> SaberValidateTests(
