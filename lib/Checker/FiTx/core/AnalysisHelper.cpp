#include "Checker/FiTx/core/AnalysisHelper.hpp"

#include "Checker/FiTx/core/Utils.hpp"
#include "Checker/FiTx/core/Value.hpp"
...
long arrayElementNum(llvm::GetElementPtrInst *get_element_ptr_inst) {
  ...
  if (num_ops < 2) return framework::Value::kArbitaryArrayElement;

  if (get_element_ptr_inst->getSourceElementType()->isArrayTy()) {
    // Use the last index (array element index); idx_end() is past-the-end.
    llvm::Value *last_idx_val =
        get_element_ptr_inst->getOperand(num_ops - 1);
    if (auto *const_int = llvm::dyn_cast<llvm::ConstantInt>(last_idx_val)) {
      if (const_int->getBitWidth() > 0) return const_int->getSExtValue();
    }
  ...
  if (auto *const_int = llvm::dyn_cast<llvm::ConstantInt>(first_idx_val)) {
    if (const_int->getBitWidth() == 0) return framework::Value::kArbitaryArrayElement;
