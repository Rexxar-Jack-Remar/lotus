// taken from https://github.com/Sap4Sec/path-aware-fuzzing
//===- PathNumbering.cpp --------------------------------------*- C++ -*---===//
//
...
#include "Fuzzing/PathProfiling/PathNumbering.h"
...
BasicBlock *BallLarusNode::getBlock() { return (_basicBlock); }

// Returns the number of paths to the exit starting at the node.
unsigned BallLarusNode::getNumberPaths() { return (_numberPaths); }
...
BallLarusNode::NodeColor BallLarusNode::getColor() { return (_color); }

// Sets the NodeColor used in graph algorithms.
void BallLarusNode::setColor(BallLarusNode::NodeColor color) { _color = color; }

// Returns an iterator over predecessor edges. Includes phony and
// backedges.
BLEdgeIterator BallLarusNode::predBegin() { return (_predEdges.begin()); }

// Returns the end sentinel for the predecessor iterator.
BLEdgeIterator BallLarusNode::predEnd() { return (_predEdges.end()); }

// Returns the number of predecessor edges.  Includes phony and
// backedges.
unsigned BallLarusNode::getNumberPredEdges() { return (_predEdges.size()); }

// Returns an iterator over successor edges. Includes phony and
// backedges.
BLEdgeIterator BallLarusNode::succBegin() { return (_succEdges.begin()); }

// Returns the end sentinel for the successor iterator.
BLEdgeIterator BallLarusNode::succEnd() { return (_succEdges.end()); }

// Returns the number of successor edges.  Includes phony and
// backedges.
unsigned BallLarusNode::getNumberSuccEdges() { return (_succEdges.size()); }
...
void BallLarusNode::removeEdge(BLEdgeVector &v, BallLarusEdge *e) {
  ...
  for (BLEdgeIterator i = v.begin(), end = v.end(); i != end; ++i) {
    if ((*i) == e) {
      v.erase(i);
      break;
    }
...
BallLarusNode *BallLarusEdge::getSource() const { return (_source); }

// Returns the target node of this edge.
BallLarusNode *BallLarusEdge::getTarget() const { return (_target); }

// Sets the type of the edge.
BallLarusEdge::EdgeType BallLarusEdge::getType() const { return _edgeType; }

// Gets the type of the edge.
void BallLarusEdge::setType(EdgeType type) { _edgeType = type; }

// Returns the weight of this edge.  Used to decode path numbers to sequences
// of basic blocks.
unsigned BallLarusEdge::getWeight() { return (_weight); }

// Sets the weight of the edge.  Used during path numbering.
void BallLarusEdge::setWeight(unsigned weight) { _weight = weight; }

// Gets the phony edge originating at the root.
BallLarusEdge *BallLarusEdge::getPhonyRoot() { return _phonyRoot; }
...
BallLarusEdge *BallLarusEdge::getPhonyExit() { return _phonyExit; }
...
BallLarusEdge *BallLarusEdge::getRealEdge() { return _realEdge; }
...
unsigned BallLarusEdge::getDuplicateNumber() { return (_duplicateNumber); }
...
void BallLarusDag::calculatePathNumbers() {
  ...
  while (bfsQueue.size() > 0) {
    ...
    //oprint("calculatePathNumbers on " << node->getName() << "\n");
    ...
    //oprint("prev, new number paths " << prevPathNumber << ", "
    //                  << node->getNumberPaths() << ".\n");

    if (prevPathNumber == 0 && node->getNumberPaths() != 0) {
      //oprint("node ready : " << node->getName() << "\n");
  ...
  if (getenv("PATH_DEBUG") || getenv("PATH_DUMP_NUMBERS"))
    oprint("\t[PN] Number of paths: " << getRoot()->getNumberPaths() << "\n");
}
...
BallLarusNode *BallLarusDag::getRoot() { return _root; }

// Returns the exit node for the DAG.
BallLarusNode *BallLarusDag::getExit() { return _exit; }

// Returns the function for the DAG.
Function &BallLarusDag::getFunction() { return (_function); }
  ...
  if (currentNode->getColor() != BallLarusNode::WHITE) {
    ...
    if (ProcessEarlyTermination) {
      for (BasicBlock::iterator bbCurrent = currentNode->getBlock()->begin(),
                                bbEnd = currentNode->getBlock()->end();
           bbCurrent != bbEnd; bbCurrent++) {
    ...
    auto *terminator = currentNode->getBlock()->getTerminator();
