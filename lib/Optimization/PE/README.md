...
- **Headers**: `include/Optimization/PE/` – LLPE.h, ShadowInlines.h, SharedTree.h, LLPECopyPaste.h
- **Library**: Static library `CanaryPE` built from sources in `lib/Optimization/PE/`
...
The PE library is built via `add_subdirectory(PE)` in `lib/Optimization/CMakeLists.txt`. It is **not** linked into `CanaryOptimization` by default; tools that need partial evaluation should link `CanaryPE` explicitly.
