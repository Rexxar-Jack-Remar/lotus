...
static Constant* getStringPtrArray(std::string& bytes, std::vector<size_t>& lineStarts, Module& M) {
  ...
  Constant* EnvPtrsPtr = ConstantExpr::getGetElementPtr(
			 cast<PointerType>(EnvPtrsG->getType())->getPointerElementType(),
			 EnvPtrsG, gepArgs, 2);
...
void LLPEAnalysisPass::loadArgv(Function* F, std::string& path, unsigned argvIdx, unsigned& argc) {
  ...
  for(unsigned i = 0; i < argc; ++i) {
    ...
    if(lineStarts[i] != -1) {
      ...
      Instruction* argvPtr =
	GetElementPtrInst::Create(cast<PointerType>(Arg->getType())->getPointerElementType(),
				  Arg, gepArg, "argv_ptr", InsertBefore);
  ...
  assert(Arg->getType()->isPointerTy());
  Instruction* argvEndPtr =
    GetElementPtrInst::Create(cast<PointerType>(Arg->getType())->getPointerElementType(),
			      Arg, gepArg, "argv_end_ptr", InsertBefore);
