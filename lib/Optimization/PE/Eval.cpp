...
bool IntegrationAttempt::tryFoldPointerCmp(ShadowInstruction* SI, std::pair<ValSetType, ImprovedVal>* Ops, ValSetType& ImpType, ImprovedVal& Improved, unsigned char* needsRuntimeCheck) {
  ...
  if(op0Arg && op1Arg && (op0Arg == zero || op1Arg == zero)) {
    ...
    Improved = ShadowValue(
	   ConstantFoldCompareInstOperands(CmpI->getPredicate(), op0Arg, op1Arg, *GlobalTD));
  ...
  if(op0 == op1) {
    ...
    ImpType = ValSetTypeScalar;
    Improved.V =
      ShadowValue(ConstantFoldCompareInstOperands(getSignedPred(CmpI->getPredicate()),
						  op0Arg, op1Arg, *GlobalTD));
    return true;
...
void IntegrationAttempt::tryEvaluateResult(ShadowInstruction* SI, 
  ...
  if (const CmpInst *CI = dyn_cast<CmpInst>(I)) {
    ...
    newConst = ConstantFoldCompareInstOperands(CI->getPredicate(), instOperands[0],
					       instOperands[1], *GlobalTD);

  }
  else if(LoadInst *LI = dyn_cast<LoadInst>(I)) {
    newConst = ConstantFoldLoadFromConstPtr(instOperands[0], LI->getType(), *GlobalTD);
  } else
