...
Constant* llvm::getConstAsType(Constant* C, Type* Ty) {
  ...
  (void)Op; // getCastOpcode validates; ConstantExpr::getCast will assert if invalid
...
void IntegrationAttempt::emitTerminator(ShadowBB* BB, ShadowInstruction* I, BasicBlock* emitBB) {
  ...
  if(inst_is<ReturnInst>(I)) {
	...
	if(!retVal) {
	  ...
	  // Create { undef, true }
	  Value* aggTemplate = ConstantStruct::get(retType, undefRet, cast<Constant>(retFlag));
	  // Return { retval, true }
...
static void emitSeekTo(Value* FD, uint64_t Offset, BasicBlock* emitBB) {
  ...
  CallInst* SeekC = CallInst::Create(FT, SeekFn, ArrayRef<Value*>(CallArgs, 3), "", emitBB);
...
bool IntegrationAttempt::emitVFSCall(ShadowBB* BB, ShadowInstruction* I, SmallVector<CommittedBlock, 1>::iterator& emitBBIter) {
...

  {
	...
	if(it->second.isFifo) {
	  ...
	  FunctionType* MemCmpFT = FunctionType::get(GInt32, {GInt8Ptr, GInt8Ptr, GInt64}, false);
	  Value* MemCmpFn = F.getParent()->getOrInsertFunction("memcmp", MemCmpFT).getCallee();
	  Value *CallArgs[] = { readBuffer, checkBuffer, MemcmpSize };
	  Instruction* ReadMemcmp = CallInst::Create(MemCmpFT, MemCmpFn, ArrayRef<Value*>(CallArgs, 3), "", emitBB);
	  ...
	  Value* CheckFn = F.getParent()->getOrInsertFunction("lliowd_ok", CheckFT).getCallee();
	  Value* CheckResult = CallInst::Create(CheckFT, CheckFn, ArrayRef<Value*>(), "readcheck", emitBB);
	...
	Instruction* ReadMemcpy = CallInst::Create(MemCpyFn->getFunctionType(), MemCpyFn, ArrayRef<Value*>(CallArgs, 5), "", emitBB);
  ...
  if((!pass->omitChecks) && I->needsRuntimeCheck == RUNTIME_CHECK_READ_LLIOWD && 
    ...
    FunctionType* CheckFT = FunctionType::get(Int32Ty, false);
    Value* CheckFn = F.getParent()->getOrInsertFunction("lliowd_ok", CheckFT).getCallee();
    Value* CheckResult = CallInst::Create(CheckFT, CheckFn, ArrayRef<Value*>(), VerboseNames ? "readcheck" : "", emitBB);
...
void IntegrationAttempt::emitCall(ShadowBB* BB, ShadowInstruction* I, SmallVector<CommittedBlock, 1>::iterator& emitBBIter) {
  ...
  if(InlineAttempt* IA = getInlineAttempt(I)) {
	...
	if(inst_is<CallInst>(I) || (inst_is<InvokeInst>(I) && !BB->succsAlive[1])) {
	  ...
	  NewI = CallInst::Create(IA->CommitF->getFunctionType(), IA->CommitF, Args, "", emitBB);
	  ...
	  NewI = InvokeInst::Create(IA->CommitF->getFunctionType(), IA->CommitF, invokeNormalDest, exnBlock, Args, "", emitBB);
	...
	if(IA->emittedAlloca) {
	  ...
	  CallInst* SavedPtr = CallInst::Create(StackSave->getFunctionType(), StackSave, ArrayRef<Value*>(), VerboseNames ? "savedstack" : "", FunctionPred->getTerminator());

	  // Restore on successful return
	  CallInst::Create(StackRestore->getFunctionType(), StackRestore, ArrayRef<Value*>(SavedPtr), "", IA->returnBlock);

	  // Restore on failed 
	  if(IA->failedReturnBlock) {
	    CallInst::Create(StackRestore->getFunctionType(), StackRestore, ArrayRef<Value*>(SavedPtr), "", IA->failedReturnBlock->getFirstNonPHI());
...
Constant* llvm::getGVOffset(Constant* GV, int64_t Offset, Type* targetType) {
  ...
  if(Offset == 0)
    ...
    OffsetGV = ConstantExpr::getGetElementPtr(
			      cast<PointerType>(CastGV->getType())->getPointerElementType(),
			      CastGV, OffC);
  }
...
bool IntegrationAttempt::synthCommittedPointer(ShadowValue* I, Type* targetType, ImprovedVal IV, BasicBlock* emitBB, Value*& Result) {
  ...
  if(Base.isGV()) {
    ...
    if(Type* ElTy = XXXFindElementAtOffset(InTy, Offset, GEPIdxs, GlobalTD)) {
      ...
      Result = GetElementPtrInst::Create(InTy, BaseI, GEPIdxs,
					 VerboseNames ? "synthgep" : "", emitBB);
      if((!isa<PointerType>(targetType)) || ElTy != cast<PointerType>(targetType)->getPointerElementType())
    ...
    Value* OffsetI = GetElementPtrInst::Create(
				cast<PointerType>(CastI->getType())->getPointerElementType(),
				CastI, OffsetC, VerboseNames ? "synthgep" : "", emitBB);
...
static Instruction* emitMemcpyInst(Value* To, Value* From, uint64_t Size, BasicBlock* emitBB) {
  ...
  return CallInst::Create(MemCpyFn->getFunctionType(), MemCpyFn, ArrayRef<Value*>(CallArgs, 5), "", emitBB);
...
void IntegrationAttempt::emitChunk(ShadowInstruction* I, BasicBlock* emitBB, SmallVector<IVSRange, 4>::iterator chunkBegin, SmallVector<IVSRange, 4>::iterator chunkEnd, SmallVector<Instruction*, 4>& newInstructions) {
  ...
  if(chunkSize == 1) {
    ...
    if(elSize > 8) {
      ...
      release_assert(newVal->getType() == cast<PointerType>(targetPtrSynth->getType())->getPointerElementType());
...
void InlineAttempt::commitArgsAndInstructions() {
  ...
  if(isCommitted()) {
	...
	targetCallInfo && targetCallInfo->targetStackDepth == (uint32_t)pass->llioPreludeStackIdx) ||
      ...
      FunctionCallee WDInitFC = getGlobalModule()->getOrInsertFunction("lliowd_init", FunctionType::get(Void, false));
