...
cl::opt<std::string> PrefetchFile("input-file",
                                  cl::desc("Specify input filename for mypass"),
...
bool SWPrefetchingLLVMPass::doInitialization(Module &M) {
  if (PrefetchDistanceProviderMode == PrefetchDistanceProvider::Profile) {
    ...
    if (std::error_code EC = ReaderOrErr.getError()) {
      ...
      Ctx.diagnose(DiagnosticInfoSampleProfile(PrefetchFilePath, Msg,
    ...
    for (auto &F : M) {
      const sampleprof::FunctionSamples *SamplesReaded =
