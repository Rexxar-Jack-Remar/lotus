#include "Dataflow/IFDS/Analyses/IFDSSignAnalysis.h"
...
SignAnalysis::FactSet SignAnalysis::call_to_return_flow(
    const llvm::CallBase * /*call*/, const llvm::Instruction *return_site,
    llvm::ArrayRef<const llvm::Function *> callees, const SignFact &fact) {
  (void)return_site;
  (void)callees;
  FactSet result;
  result.insert(fact);
