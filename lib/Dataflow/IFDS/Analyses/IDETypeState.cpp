...
IDETypeState::FactSet IDETypeState::return_flow(
...
IDETypeState::FactSet IDETypeState::call_to_return_flow(
  ...
  bool preserve_call_result =
      call && !call->getType()->isVoidTy() && should_track(call);
    ...
    for (const llvm::Function *callee : callees) {
      ...
      if (preserve_fact) {
        for (int idx : m_description->get_consumer_param_indices(callee_name)) {
          ...
          if (actual == fact || (actual->getType()->isPointerTy() &&
                                 fact->getType()->isPointerTy() &&
...
IDETypeState::EdgeFunction IDETypeState::return_edge_function(
    ...
    const llvm::Instruction *exit_inst, const llvm::Instruction *return_site,
    const Fact &exit_fact, const Fact &ret_fact) {
  (void)call;
  (void)callee;
  (void)exit_inst;
  (void)return_site;
...
IDETypeState::EdgeFunction IDETypeState::summary_edge_function(
    const llvm::CallBase *call, const llvm::Function *callee,
  ...
  auto apply_api_transition = [description = m_description, token,
    ...
    return Value(description->get_next_state(token, current_state, call));
  };

  if (src_fact == zero_fact() &&
      m_description->is_factory_function(callee_name)) {
