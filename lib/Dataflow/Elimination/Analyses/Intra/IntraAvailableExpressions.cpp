...
public:
  explicit ElimAvailableExpressionsProblem(llvm::Function *F,
                                           llvm::AAResults *AA = nullptr,
                                           llvm::DominatorTree *DT = nullptr,
                                           llvm::TargetLibraryInfo *TLI = nullptr,
                                           llvm::MemorySSA *MSSA = nullptr)
      : LLVMIntraEliminationProblem<AvailableExpressionsFact>(F),
        AA(AA), DT(DT), TLI(TLI), MSSA(MSSA) {
    buildUniverse(F);
  }

  AvailableExpressionsFact applyTransfer(const transfer_t &T,
                                         const AvailableExpressionsFact &In)
      const override {
    auto *Inst = T;
  ...
  AvailableExpressionsFact meet(const AvailableExpressionsFact &Lhs,
                                const AvailableExpressionsFact &Rhs)
      const override {
...
runIntraElimAvailableExpressions(llvm::Function *F, llvm::AAResults *AA,
  ...
  IntraEliminationSolver<LLVMEliminationDomain<AvailableExpressionsFact>> Solver(
      Problem, Opts);
  Solver.solve();
