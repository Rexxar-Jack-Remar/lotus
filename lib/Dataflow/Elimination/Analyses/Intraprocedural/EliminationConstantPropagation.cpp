...
ConstantPropagationValue makeConst(const llvm::Constant *Value) {
  ...
  return ConstantPropagationValue::get(
      const_cast<llvm::Constant *>(Value));
}
...
ConstantPropagationValue evalBinaryOp(const llvm::Instruction *Inst,
  ...
  if (isConstRange(Lhs) && isConstRange(Rhs) && Inst != nullptr &&
    ...
    if (Res.isSingleElement()) {
      return makeConst(llvm::ConstantInt::get(Inst->getType(),
                                              *Res.getSingleElement()));
    }
...
public:
  explicit ElimConstantPropagationProblem(llvm::Function *F,
  ...
  ConstantPropagationMap applyTransfer(const transfer_t &T,
                                       const ConstantPropagationMap &In)
      const override {
    auto *Inst = T;
    ...
    if (const auto *Cast = llvm::dyn_cast<llvm::CastInst>(Inst)) {
      ...
      if (Src.isConstantRange() && Cast->getType()->isIntegerTy()) {
        ...
        if (DstTy != nullptr) {
          ...
          if (Res.isSingleElement()) {
            Out[Cast] = makeConst(
                llvm::ConstantInt::get(Cast->getType(),
                                       *Res.getSingleElement()));
          } else {
      ...
      if (auto *Folded = llvm::ConstantFoldUnaryOpOperand(
              Cast->getOpcode(), C, *DL)) {
...
runIntraElimConstantPropagation(llvm::Function *F, llvm::AAResults *AA,
  ...
  return runIntraElimConstantPropagation(F, AA, nullptr, nullptr, nullptr, Opts);
}
