...
bool PrintBlockResultsOpt = false;
bool PrintNewtonProfileOpt = false;
...
void printNewtonProfile(raw_ostream &OS, const npa::Stat &Stats) {
  if (!PrintNewtonProfileOpt || Stats.newton_rounds.empty())
    return;
...
int runNpaLLVM(const std::string &input_filename, raw_ostream &OS,
  ...
  PrintBlockResultsOpt = print_block_results;
  PrintNewtonProfileOpt = print_newton_profile;
  LLVMContext Context;
  SMDiagnostic Err;
  auto M = lotus::dataflow_tool::loadModuleOrReport(input_filename, Context, Err,
                                                    argv0);
  if (!M)
    return 1;
  ...
  const auto *Handler =
      lotus::dataflow_tool::findHandler(analysis, Handlers);
  if (!Handler) {
    errs() << "error: unknown NPA analysis '" << analysis << "'\n";
    return 1;
  }

  if (Handler->ModuleScoped && solver != "newton") {
    errs() << "error: NPA analysis '" << analysis
  ...
  const npa::SolverStrategy Strategy = parseSolverStrategy(solver);
  const npa::LinearStrategy LinearStrategy =
      parseLinearStrategy(linear_solver);
  const npa::NewtonRoundStrategy RoundStrategy =
      parseNewtonRoundStrategy(newton_round);
  OS << "[npa:" << analysis;
  if (Handler->ModuleScoped)
    OS << ":module";
  else
    OS << ":" << solver;
  OS << ":linear=" << linear_solver;
  if (print_newton_profile)
    OS << ":round=" << newton_round;
  OS << "]\n";
  if (Handler->ModuleScoped)
