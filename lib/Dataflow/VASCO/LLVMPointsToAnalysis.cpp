...
PointsToAnalysis::DomainType
PointsToAnalysis::meet(const DomainType &LHS, const DomainType &RHS) {
...
PointsToAnalysis::flowFunction(ContextPtr Context, const NodeType &Node,
  ...
  if (isPointerLike(Node)) {
    OutValue.assign(PointsToValue::forValue(Node), evaluateValue(Node, InValue));
  }
...
void PointsToAnalysis::seedGlobals(DomainType &State) const {
  ...
  for (auto &Global : LLVMProgram->getModule()->globals()) {
    ...
    State.store(MemoryLocation::exact(Object), initializerTargets(Global.getInitializer()),
                true);
  }
}
...
PointsToAnalysis::evaluateValue(const llvm::Value *Value,
  ...
  if (auto *ConstantExpr = llvm::dyn_cast<llvm::ConstantExpr>(Value)) {
    ...
    if (ConstantExpr->getOpcode() == llvm::Instruction::GetElementPtr) {
      ...
      for (const auto &Location : Base) {
        ...
        Result.insert(FieldLocation.value_or(MemoryLocation::summary(Location.Object)));
      }
      return Result;
    }
    return summaryLocation();
  ...
  if (auto *Select = llvm::dyn_cast<llvm::SelectInst>(Value)) {
    ...
    const auto FalseTargets = pointsToSetOfValue(Select->getFalseValue(), State);
...
void PointsToAnalysis::handleStore(const llvm::StoreInst &Store, DomainType &OutValue,
                                   const DomainType &InValue) const {
  const auto StoredTargets = pointsToSetOfValue(Store.getValueOperand(), InValue);
  const auto Locations = pointsToSetOfValue(Store.getPointerOperand(), InValue);
  if (Locations.empty()) {
    return;
  }

  const bool StrongUpdate =
      Locations.size() == 1 && !Locations.begin()->IsSummary &&
...
bool PointsToAnalysis::handleMemoryIntrinsicCall(const llvm::CallBase &Call,
                                                 DomainType &OutValue,
                                                 const DomainType &InValue) const {
  const auto *DL = dataLayout();
...
bool PointsToAnalysis::handleReallocCall(const llvm::CallBase &Call,
  ...
  const auto NewObject = MemoryBlock::heap(&Call, Model.layoutForHeapCall(&Call), ContextId);
  ...
  for (const auto &Old : OldTargets) {
    ...
    OutValue.assign(PointsToValue::forValue(&Call), singletonLocation(NewBlock));
  }
...
void PointsToAnalysis::summarizePointerTargets(const llvm::Value *Pointer,
                                               DomainType &OutValue,
                                               const DomainType &InValue) const {
...
PointsToAnalysis::resolveIndirectTargets(const llvm::CallBase *Call,
  ...
  const auto CalleeTargets = pointsToSetOfValue(Call->getCalledOperand(), State);
...
std::optional<PointsToAnalysis::DomainType>
PointsToAnalysis::processCallTarget(ContextPtr CallerContext, NodeType Call,
                                    const MethodType &Method,
                                    const DomainType &InValue,
  ...
  DomainType EntryValue;
  for (unsigned I = 0; I < CallBase->arg_size() && I < Method->arg_size(); ++I) {
  ...
  if (Program.isPhantomMethod(Method)) {
    for (unsigned I = 0; I < CallBase->arg_size(); ++I) {
      ...
      const auto Targets = pointsToSetOfValue(CallBase->getArgOperand(I), Returned);
      for (const auto &Target : Targets) {
...
PointsToAnalysis::flowCall(ContextPtr Context, NodeType Call,
  ...
  if ((!Targets.has_value() || Targets->empty()) && CallBase->isIndirectCall()) {
    Targets = resolveIndirectTargets(CallBase, InValue);
  }

  if (!Targets.has_value()) {
    this->ContextTransitions.addTransition(typename Base::CallSiteType(Context, Call),
                                          nullptr);
    ...
    for (unsigned I = 0; I < CallBase->arg_size(); ++I) {
      ...
      const auto TargetsForArg = pointsToSetOfValue(CallBase->getArgOperand(I), Unknown);
