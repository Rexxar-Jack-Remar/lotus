...
MemoryLayout MemoryModel::layoutForGlobal(const llvm::GlobalVariable *Global) const {
  if (Global == nullptr) {
    return MemoryLayout::unknown();
  }
...
MemoryLayout MemoryModel::layoutForArgument(const llvm::Argument *Argument) const {
...
MemoryLayout MemoryModel::layoutForReallocCall(const llvm::CallBase *Call,
...
llvm::Type *MemoryModel::inferHeapPointeeType(const llvm::CallBase *Call) const {
...
llvm::Type *
MemoryModel::inferTypedAllocationFromReturnUses(const llvm::CallBase *Call) const {
  if (Call == nullptr) {
    return nullptr;
  }
  ...
  for (const auto *User : Call->users()) {
    ...
    if (auto *Store = llvm::dyn_cast<llvm::StoreInst>(User)) {
      auto *PointerTy =
          llvm::dyn_cast<llvm::PointerType>(Store->getPointerOperand()->getType());
...
llvm::Type *
MemoryModel::nextBitcastPointeeType(const llvm::Instruction *Instruction) const {
...
MemoryModel::constantAllocationSize(const llvm::CallBase *Call) const {
  ...
  auto ConstantArg = [&](unsigned Index) -> std::optional<std::uint64_t> {
    ...
    if (auto *Const = llvm::dyn_cast<llvm::ConstantInt>(Call->getArgOperand(Index))) {
...
MemoryModel::computeOffset(const llvm::Value *PointerOperand,
  ...
  if (auto *GEP = llvm::dyn_cast<llvm::GEPOperator>(PointerOperand)) {
    llvm::APInt OffsetBits(Layout.getIndexTypeSizeInBits(PointerOperand->getType()), 0,
                           true);
