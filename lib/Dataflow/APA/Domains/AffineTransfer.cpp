...
int64_t addWrapped(int64_t lhs, int64_t rhs, unsigned width) {
  return wrapToBitWidth(
      (llvm::APInt(64, lhs, true) + llvm::APInt(64, rhs, true)).getSExtValue(),
      width);
}

int64_t subWrapped(int64_t lhs, int64_t rhs, unsigned width) {
  return wrapToBitWidth(
      (llvm::APInt(64, lhs, true) - llvm::APInt(64, rhs, true)).getSExtValue(),
      width);
}

int64_t mulWrapped(int64_t lhs, int64_t rhs, unsigned width) {
  return wrapToBitWidth(
      (llvm::APInt(64, lhs, true) * llvm::APInt(64, rhs, true)).getSExtValue(),
      width);
}
...
AffineExpr addExpr(AffineExpr lhs, const AffineExpr &rhs, unsigned bitWidth) {
  if (lhs.top || rhs.top)
    return topExpr();
  lhs.constant = addWrapped(lhs.constant, rhs.constant, bitWidth);
  for (const auto &term : rhs.terms)
    lhs.terms[term.first] =
        addWrapped(lhs.terms[term.first], term.second, bitWidth);
...
AffineExpr scaleExpr(AffineExpr expr, int64_t factor, unsigned bitWidth) {
  if (expr.top)
    return topExpr();
  expr.constant = mulWrapped(expr.constant, factor, bitWidth);
  for (auto &term : expr.terms)
    term.second = mulWrapped(term.second, factor, bitWidth);
...
Relation equalityConstraintForExprs(const AffineExpr &lhs,
  ...
  for (const auto &term : rhs.terms)
    coeffs[term.first] = subWrapped(coeffs[term.first], term.second, bitWidth);
  ...
  int64_t constant = subWrapped(rhs.constant, lhs.constant, bitWidth);
...
AffineState materializeAffineExpressionsImpl(const Relation &state) {
      ...
      for (const Row &row : rows) {
        ...
        for (unsigned postCol = 0; postCol < vars; ++postCol) {
          ...
          if (solved[postCol].top) {
            auto &coefficient = expr.terms[vocab->values[postCol]];
            coefficient =
                addWrapped(coefficient, (-(row[postCol] * inv)).getSExtValue(),
                           actualWidth);
            continue;
          }
          expr = addExpr(std::move(expr),
        ...
        for (unsigned preCol = 0; preCol < vars; ++preCol) {
          ...
          if (!preSolved[preCol].top) {
            ...
            auto &coefficient = expr.terms[vocab->values[preCol]];
            coefficient = addWrapped(coefficient, coeffInt, actualWidth);
          }
        }
  ...
  for (const Row &row : rows) {
    ...
    for (unsigned postCol = 0; postCol < vars; ++postCol) {
      if (!row[postCol].isZero()) {
        auto &coefficient = equality.terms[vocab->values[postCol]];
        coefficient = addWrapped(coefficient, row[postCol].getSExtValue(),
                                 equality.bitWidth);
      }
    }
    ...
    for (unsigned preCol = 0; preCol < vars; ++preCol) {
      ...
      if (!preSolved[preCol].top) {
        lhsConstant = addWrapped(
            lhsConstant,
            mulWrapped(coeff, preSolved[preCol].constant, equality.bitWidth),
            equality.bitWidth);
        for (const auto &term : preSolved[preCol].terms)
          equality.terms[term.first] =
              addWrapped(equality.terms[term.first],
                         mulWrapped(coeff, term.second, equality.bitWidth),
                         equality.bitWidth);
      } else {
        auto &coefficient = equality.terms[vocab->values[preCol]];
        coefficient = addWrapped(coefficient, coeff, equality.bitWidth);
      }
    }

    equality.constant = subWrapped(0, lhsConstant, equality.bitWidth);
    ...
    for (const auto &term : equality.terms) {
      ...
      for (const auto &other : equality.terms) {
        ...
        llvm::APInt otherCoeff(width, static_cast<uint64_t>(other.second),
                               true);
        expr.terms[other.first] =
            addWrapped(expr.terms[other.first],
                       (-(otherCoeff * inv)).getSExtValue(), width);
      }
...
public:
  ...
  static bool edgeEntersPhi(const llvm::Instruction &Pred,
    ...
    return Pred.isTerminator() && &Succ.getParent()->front() == &Succ &&
  ...
  Relation edgeTransferRelation(const llvm::Instruction &Term,
    ...
    if (auto *Switch = llvm::dyn_cast<llvm::SwitchInst>(&Term)) {
      ...
      if (!D::isTrackedValue(Switch->getCondition()))
        return D::identity();
      if (Switch->getDefaultDest() == SuccBlock)
        return D::identity();
      Relation cases = D::zero();
      for (const auto &Case : Switch->cases()) {
        if (Case.getCaseSuccessor() == SuccBlock) {
          cases = D::combine(
              cases,
              switchCaseRefinement(
                  Switch->getCondition(),
  ...
  Relation phiTransferForEdge(const llvm::Instruction &PredInst,
    ...
    if (PredBlock == nullptr || SuccBlock == nullptr ||
        !PredInst.isTerminator())
      return D::identity();
    if (&SuccBlock->front() != &SuccInst)
      return D::identity();

    std::vector<std::pair<const llvm::Value *, Relation>> assignments;
    for (auto &Inst : *SuccBlock) {
      ...
      if (auto modeled = booleanPhiRelation(*Phi)) {
        assignments.emplace_back(Phi, *modeled);
        continue;
      }
      assignments.emplace_back(
          Phi, relationForValue(Phi, Phi->getIncomingValueForBlock(PredBlock)));
    }
    return D::parallelAssign(assignments);
  }
  ...
  std::optional<Relation> booleanPhiRelation(const llvm::PHINode &Phi) const {
    ...
    int64_t delta = subWrapped(TrueConst->getSExtValue(),
                               FalseConst->getSExtValue(), width);
  ...
  Relation callEntryTransfer(const llvm::CallBase &Call,
    ...
    for (const auto &formal : Callee.args()) {
      ...
      if (D::isTrackedValue(&formal))
        assignments.emplace_back(
            &formal, relationForValue(&formal, Call.getArgOperand(index)));
      ++index;
    }
    return D::parallelAssign(assignments);
  }
  ...
  buildBitwisePartialRelation(const llvm::BinaryOperator &BinOp) const {
    ...
    auto tryUseExtraKnownBits =
      ...
      if (opcode == llvm::Instruction::And) {
        ...
        AffineExpr expr =
            addConstant(lessExpr, (lowResult - lessLow).getSExtValue(), width);
      ...
      if (opcode == llvm::Instruction::Xor) {
        if (!lowMiddleBitIsOne) {
          ...
          AffineExpr expr = addConstant(
              lessExpr, (lowResult - lessLow).getSExtValue(), width);
          return congruenceAssignmentForExpr(&BinOp, modulusBits, expr);
        }
        llvm::APInt lessLow = lowBitsValue(less.value, knownBits);
        AffineExpr expr = addConstant(
            scaleExpr(lessExpr, -1, width),
            (lessLow + lowResult - llvm::APInt(width, 1).shl(knownBits))
                .getSExtValue(),
            width);
        return congruenceAssignmentForExpr(&BinOp, modulusBits, expr);
      }
  ...
  Relation buildBinaryRelation(const llvm::BinaryOperator &BinOp) const {
    ...
    case llvm::Instruction::Add:
      if (getConstantIntValue(L, lhsConst) && getConstantIntValue(R, rhsConst))
        return D::makeAffineAssignment(
            &BinOp, addWrapped(lhsConst, rhsConst, width), {});
    ...
    case llvm::Instruction::Sub:
      if (getConstantIntValue(L, lhsConst) && getConstantIntValue(R, rhsConst))
        return D::makeAffineAssignment(
            &BinOp, subWrapped(lhsConst, rhsConst, width), {});
      if (getConstantIntValue(R, rhsConst))
        return D::makeAffineAssignment(&BinOp, subWrapped(0, rhsConst, width),
                                       {{L, 1}});
      if (D::isTrackedValue(L) && D::isTrackedValue(R))
        return D::makeAffineAssignment(&BinOp, 0, {{L, 1}, {R, -1}});
      return D::makeForget(&BinOp);
    case llvm::Instruction::Mul:
      if (getConstantIntValue(L, lhsConst) && getConstantIntValue(R, rhsConst))
        return D::makeAffineAssignment(
            &BinOp, mulWrapped(lhsConst, rhsConst, width), {});
...
Relation
AffineTransferBuilder::instructionTransfer(llvm::Instruction &inst) const {
  return TransferBuilderImpl{}.instructionTransfer(inst);
}

Relation AffineTransferBuilder::edgeTransferRelation(
    const llvm::Instruction &term, const llvm::Instruction &succ) const {
  return TransferBuilderImpl{}.edgeTransferRelation(term, succ);
}

Relation
AffineTransferBuilder::phiTransferForEdge(const llvm::Instruction &term,
                                          const llvm::Instruction &succ) const {
  return TransferBuilderImpl{}.phiTransferForEdge(term, succ);
}

Relation
AffineTransferBuilder::callEntryTransfer(const llvm::CallBase &call,
                                         const llvm::Function &callee) const {
  return TransferBuilderImpl{}.callEntryTransfer(call, callee);
}

Relation
AffineTransferBuilder::callReturnTransfer(const llvm::CallBase &call,
                                          const llvm::Function &callee,
                                          llvm::Instruction *exit) const {
  return TransferBuilderImpl{}.callReturnTransfer(call, callee, exit);
}

bool AffineTransferBuilder::instructionHasEffect(
    const llvm::Instruction &inst) {
  return TransferBuilderImpl{}.instructionHasEffect(inst);
}

bool AffineTransferBuilder::edgeHasCondition(const llvm::Instruction &inst) {
  return TransferBuilderImpl{}.edgeHasCondition(inst);
}

bool AffineTransferBuilder::edgeEntersPhi(const llvm::Instruction &pred,
                                          const llvm::Instruction &succ) {
  return TransferBuilderImpl{}.edgeEntersPhi(pred, succ);
}

bool AffineTransferBuilder::isTrackedScalar(const llvm::Value *value) {
  return elimination::isTrackedScalar(value);
}
bool AffineTransferBuilder::isAssumeLikeCall(const llvm::CallBase &call) {
  return elimination::isAssumeLikeCall(call);
}
