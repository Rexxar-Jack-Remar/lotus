/**
 * @file UninitializedVariablesAnalysis.cpp
...
static GenKillTransformer *createUninitTransformer(Instruction *I) {
  ...
  if (auto *AI = dyn_cast<AllocaInst>(I)) {
    ...
    if (Value *rep =
            MemoryObjectFact::getRepresentative(LI->getPointerOperand())) {
      addFlow(rep, LI);
    }
    ...
    // p2 = gep p1
    if (Value *rep =
            MemoryObjectFact::getRepresentative(GEP->getPointerOperand())) {
      addFlow(rep, GEP);
    }
