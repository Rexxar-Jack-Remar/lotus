...
static cl::opt<bool> EnableBreakCritLoops(
    "break-crit-loops",
...
static cl::opt<bool> EnableRemoveConstantExprs(
    ...
    cl::desc("Transform constant expressions to instructions"),
    cl::init(false), cl::cat(OptCat));
...
static cl::opt<bool> EnableRemoveErrorCalls(
    "remove-error-calls",
...
static cl::opt<bool> EnableInstrumentAllocNF(
    "instrument-alloc-nf",
    cl::desc("Replace calls to malloc/calloc with verifier functions (never fails)"),
    cl::init(false), cl::cat(OptCat));
...
static cl::opt<bool> EnableFlattenLoops(
    "flatten-loops",
...
static cl::opt<bool> EnableInstrumentNontermination(
    ...
    cl::desc("Insert trivial checks for state space cycles"),
    cl::init(false), cl::cat(OptCat));
...
static cl::opt<bool> EnableRenameVerifierFuns(
    ...
    cl::desc("Replace calls to verifier functions with calls to named versions"),
    cl::init(false), cl::cat(OptCat));
...
static cl::opt<bool> EnableMarkVolatile(
    "mark-volatile",
...
static cl::opt<bool> EnableFindExits(
    "find-exits",
    cl::desc("Put calls to __VERIFIER_silent_exit into bitcode before any exit"),
    cl::init(false), cl::cat(OptCat));
...
static cl::opt<bool> EnableClassifyLoops(
    "classify-loops",
    cl::desc("Detect what loops are in the program"),
    cl::init(false), cl::cat(OptCat));

static cl::opt<bool> EnableCountInstr(
    "count-instr",
    cl::desc("Print statistics from module"),
    cl::init(false), cl::cat(OptCat));

static cl::opt<bool> EnableGetTestTargets(
    "get-test-targets",
...
int main(int argc, char **argv) {
  ...
  if (!EnableAInline && !EnableIPDSE && !EnableIPRLE && !EnableIPSink &&
      ...
      !EnableRemoveErrorCalls && !EnableInstrumentAlloc && !EnableInstrumentAllocNF &&
      !EnableRemoveInfiniteLoops && !EnableBreakInfiniteLoops && !EnableFlattenLoops &&
  ...
  if (EnableInstrumentAllocNF)
    PM.add(lotus::verification::transform::createInstrumentAllocNeverFailsPass());
  ...
  if (EnableInstrumentNontermination)
    PM.add(lotus::verification::transform::createInstrumentNonterminationPass());
