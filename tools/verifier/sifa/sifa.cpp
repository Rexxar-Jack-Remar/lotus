...
static cl::opt<std::string> InputFilename(cl::Positional, cl::desc("<input bitcode>"), cl::value_desc("bitcode"));

static cl::opt<std::string> FunctionName("function", cl::desc("Function to analyze (default: main)"), cl::value_desc("name"));

static cl::opt<std::string> BlockLabel("block", cl::desc("Block to analyze to (default: return)"), cl::value_desc("label"));
...
static cl::opt<bool> NoValidateSubset("no-validate-subset", cl::desc("Disable IR subset validation"), cl::init(false));

static cl::opt<bool> ListFunctions("list-functions", cl::desc("List functions"), cl::init(false));

static cl::opt<bool> ListBlocks("list-blocks", cl::desc("List basic blocks"), cl::init(false));

static cl::opt<bool> Verbose("verbose", cl::desc("Print detailed state (SymAbs)"), cl::init(false));

static cl::opt<std::string> LogLevel("log-level", cl::desc("none|error|warning|info|progress|debug|trace"), cl::value_desc("level"), cl::init("none"));

static llvm::BasicBlock *findBlock(llvm::Function &F, const std::string &label) {
  for (auto &BB : F)
...
static SifaLogLevel parseLogLevel(const std::string &s) {
  const std::pair<std::string, SifaLogLevel> levels[] = {
      {"error", SifaLogLevel::Error},   {"warning", SifaLogLevel::Warning}, {"info", SifaLogLevel::Info},
      {"progress", SifaLogLevel::Progress}, {"debug", SifaLogLevel::Debug}, {"trace", SifaLogLevel::Trace}};
...
int main(int argc, char **argv) {
  cl::ParseCommandLineOptions(argc, argv, "Sifa - Symbolic Interpretation with Fluid Abstractions\n");
  ...
  Function *targetFunc = FunctionName.empty() ? module->getFunction("main") : module->getFunction(FunctionName);
  if (!targetFunc)
    for (auto &F : *module)
      if (!F.isDeclaration()) {
        targetFunc = &F;
  ...
  if (!targetFunc) {
    errs() << "Error: Function '" << (FunctionName.empty() ? "main" : FunctionName.getValue()) << "' not found\n";
    return 1;
  }

  if (ListFunctions) {
    for (auto &F : *module)
      if (!F.isDeclaration()) outs() << "  " << F.getName() << "\n";
    return 0;
  }
  if (ListBlocks) {
    ...
    if (UseSymAbs) {
      ...
      auto reportSymAbs = [&](const std::string &label, SymAbsState state) {
        if (ReachabilityOnly) {
          ...
          outs() << label << (reachable ? " is reachable\n" : " is not reachable\n");
          return reachable ? 0 : 1;
        }
        if (!state || state->isBottom()) {
          outs() << label << ": " << (!state ? "unreachable (bottom)" : "bottom") << "\n";
          return 0;
        }
      ...
      if (!BlockLabel.empty()) {
        ...
        SymAbsState state = analyzeSymAbsTo(*module, *targetFunc, *targetBlock, options);
        int ret = reportSymAbs("Block " + BlockLabel.getValue(), state);
        if (ret != 0) return ret;
      } else {
        int ret = reportSymAbs("Return", analyzeSymAbsToReturn(*module, *targetFunc, options));
        if (ret != 0) return ret;
      }
    } else {
      bool useOctagon = DomainOpt.getValue().find("Octagon") != std::string::npos;
      unsigned nBlocks = 0;
      for (auto &BB : *targetFunc) (void)BB, ++nBlocks;
      SifaLogger::progress("Analyzing (instruction transfer, no SMT) function '" + targetFunc->getName().str() +
                          "' (" + std::to_string(nBlocks) + " blocks, domain: " + (useOctagon ? "Octagon" : "Interval") + ")");

      if (!BlockLabel.empty() && !findBlock(*targetFunc, BlockLabel.getValue())) {
        errs() << "Error: block '" << BlockLabel.getValue() << "' not found\n";
        return 1;
      }

      auto label = BlockLabel.empty() ? std::string("Return") : "Block " + BlockLabel.getValue();
      ...
      if (useOctagon) {
        OctagonState state = BlockLabel.empty()
            ? analyzeToReturnWithOctagonDomain(*targetFunc, OctagonState(false), sifaOpts)
            : analyzeToWithOctagonDomain(*targetFunc, *findBlock(*targetFunc, BlockLabel.getValue()), OctagonState(false), sifaOpts);
        OctagonDomain dom(nullptr, nullptr);
        reportNative(dom.isBottom(state), state, "octagon");
      } else {
        IntervalState state = BlockLabel.empty()
            ? analyzeToReturnWithIntervalDomain(*targetFunc, IntervalState(false), sifaOpts)
            : analyzeToWithIntervalDomain(*targetFunc, *findBlock(*targetFunc, BlockLabel.getValue()), IntervalState(false), sifaOpts);
