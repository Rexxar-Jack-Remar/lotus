...
- **CMake**: CUDD and WPDS are built via `third-party/CMakeLists.txt`; link targets `CanaryCUDD`, `wpds`, `wpds++` (and interfaces `ewpds`, `wpdsplusplus_util`) as needed. spdlog is header-only; optional target `spdlog::spdlog` exposes include directories. CRAB is configured from `cmake/ConfigureClamCrab.cmake` and is discovered from `third-party/crab/` by default.
