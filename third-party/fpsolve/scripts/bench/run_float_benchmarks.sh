...
for f in `find tests/unit/grammars/float-random -name ${BENCH_PREFIX}'*.cfg'`
do
  for i in {1..5}
  do
    echo $f
...
#    outf=${f}_log_float_noscc_${i}
    ...
    echo $outf
#    echo `timeout 5m ./build/tools/fpsolve -f $f --float -i 10 | grep "Multiplication\|Addition\|Star" | wc -l`' ms' > $outf
    timeout 5m ./build/tools/fpsolve -f $f --float -i 10 -s newtonNumeric > $outf
    if [ $? -ne 0 ]; then
...
for i in {1..5}
do
  for f in `find tests/unit/grammars/float-random -name ${BENCH_PREFIX}*log_float_${i}`
  do
