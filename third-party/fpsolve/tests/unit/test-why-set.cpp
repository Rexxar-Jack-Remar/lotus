/*
 * test-why-set.cpp
 *
 *  Created on: 15.05.2014
 *      Author: schlund
 */


#include "test-why-set.h"
#include "util.h"

void WhySetSemiringTest::SetUp()
{
  std::cout << "Why-Set-SR-Test :" << '\n';
  a = new WhySemiring("a");
  b = new WhySemiring("b");
  c = new WhySemiring("c");
  d = new WhySemiring("d");
}

void WhySetSemiringTest::TearDown()
{
  delete a;
  delete b;
  delete c;
  delete d;
}

TEST_F(WhySetSemiringTest, testSemiring)
{
  generic_test_semiring(*a, *b);
  generic_test_semiring(*a+*c, *b+*d);
}

TEST_F(WhySetSemiringTest, testAddition)
{
  std::cout << "a+b=" <<*a + *b << std::endl;
  EXPECT_TRUE( (*a) + WhySemiring::null() == (*a));
  EXPECT_TRUE( WhySemiring::null() + (*a) == (*a));
}

TEST_F(WhySetSemiringTest, testMultiplication)
{
  std::cout << "a*b=" << *a* *b << std::endl;
  std::cout << "(a+b)*(a+b)=" << ((*a + *b)*(*a + *b)) << std::endl;
  std::cout << "1+(a+b)=" << (WhySemiring::one()+(*a + *b)) << std::endl;
  std::cout << "(a+b)^2=" << pow((*a + *b),2) << std::endl;
}

TEST_F(WhySetSemiringTest, testStar)
{
  WhySemiring r = *a + *b + *c;
  std::cout << "r^3=" << (WhySemiring::one()*r*(r*r)) << std::endl;
  WhySemiring ab = *a * *b;
  WhySemiring ac = *a * *c;
  WhySemiring bc = *b * *c;
  WhySemiring abc = *a * *b * *c;
  EXPECT_TRUE( r.star() == WhySemiring::one() + *a + *b + *c + ab + ac + bc + abc);
}
