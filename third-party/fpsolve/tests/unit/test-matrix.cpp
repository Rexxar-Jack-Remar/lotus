...
{
	// null + matrix = matrix
	EXPECT_TRUE( *null + *fourth == *fourth);
	// matrix + null = matrix
	EXPECT_TRUE( *fourth + *null  == *fourth);
...
{
	// one * matrix = matrix
	EXPECT_TRUE( *one * *fourth == *fourth);
	// matrix * one = matrix
	EXPECT_TRUE( *fourth * *one == *fourth);
	// null * matrix = null
	EXPECT_TRUE( *null * *fourth == *null);
	// matrix * null = null
	EXPECT_TRUE( *fourth * *null  == *null);
...
*/
  ...
  std::vector<unsigned int> seeds{42,23,11805,24890};
    ...
    //std::cout << "starmat:" << std::endl << rec_star;
	  ...
	  EXPECT_TRUE(rec_star2 == fw_star);
	  EXPECT_TRUE(rec_star2 == rec_star);
    ...
    EXPECT_TRUE(test_matrix.solve_LDU(test_vec) == rec_star*test_vec);
	}

  Matrix<Rat> test_matrix2(3, {Rat("1/3"), Rat("0"), Rat("1/5"),
                               Rat("0"), Rat("1/2"), Rat("2/3"),
                               Rat("0"), Rat("5/78"), Rat("1/7")});
  EXPECT_TRUE(test_matrix2.star() == test_matrix2.star2());
  EXPECT_TRUE(test_matrix2.star() == test_matrix2.star3());
...
*/
  ...
  EXPECT_TRUE(test_matrix2.solve_LDU(test_vec2) == test_matrix2.star()*test_vec2);
  EXPECT_TRUE(test_matrix2.solve_LDU(test_vec2) == test_matrix2.star2()*test_vec2);
  EXPECT_TRUE(test_matrix2.solve_LDU(test_vec2) == test_matrix2.star3()*test_vec2);
