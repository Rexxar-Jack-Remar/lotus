...
TEST_F(PolynomialTest, testMultiplication) {
  // 1 * poly = poly
...
TEST_F(PolynomialTest, testJacobian) {
  ...
  EXPECT_TRUE( CommutativePolynomial<TEST_SR>::jacobian(polys, vars) == result );
...
TEST_F(PolynomialTest, testEvaluation) {
  ...
  EXPECT_TRUE( second->eval(values) == (caa + dab) + ebb ||
...
TEST_F(PolynomialTest, testMatrixEvaluation) { }

TEST_F(PolynomialTest, testPolynomialToFreeSemiring) {
...
TEST_F(PolynomialTest, testDerivativeBinomAt) {
  ValuationMap<TEST_SR> values = {
  ...
  /*
  EXPECT_TRUE(first->derivative_binom(dx0).eval(values).string() ==
                 first->DerivativeBinomAt(hdx0, values).string());


  EXPECT_TRUE(first->derivative_binom(dx1).eval(values).string() ==
                 first->DerivativeBinomAt(hdx1, values).string());

  EXPECT_TRUE(first->derivative_binom(dx1).eval(values).string() ==
                 first->DerivativeBinomAt(hdx1, values).string());

  EXPECT_TRUE(second->derivative_binom(dx0).eval(values).string() ==
                 second->DerivativeBinomAt(hdx0, values).string());

  EXPECT_TRUE(second->derivative_binom(dx1).eval(values).string() ==
                 second->DerivativeBinomAt(hdx1, values).string());

  EXPECT_TRUE(second->derivative_binom(dx2).eval(values).string() ==
