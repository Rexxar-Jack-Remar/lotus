...
void Context::print(ostream& in) const {
    ...
    in << "- The polyhedral store:" << '\n';
    in << *polyStore;
    in << '\n';

    in << "- The disequality store:" << '\n';
    in << *LambdaStore;
    in << '\n';

    in << "- The equality expression store:" << '\n';
    vector<Expression>::iterator it;
    for (it = eqExprs->begin(); it < eqExprs->end(); it++)
        in << (*it) << " == 0" << '\n';
    in << '\n';

    in << "- The inequality expression store:" << '\n';
    for (it = ineqExprs->begin(); it < ineqExprs->end(); it++)
        in << (*it) << " >= 0" << '\n';
