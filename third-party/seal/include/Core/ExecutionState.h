...
class ExecutionStateControl {
public:
    static const std::set<BasicBlock *> *ControlPath;
};

class ExecutionState {
private:
    /// the last basic block where this execution state is accessed
    BasicBlock *Block;
...
public:
    ...
    /// get the branch condition, on which we jump from B to B's Kth successor
    z3::expr condition(BasicBlock *B, unsigned K);

    /// set the block to visit
    void setBlock(BasicBlock *B) { this->Block = B; }

    void refine(std::vector<std::vector<unsigned>> &PhiSelectors);

    z3::expr refine(const z3::expr &, std::vector<std::vector<unsigned>> &PhiSelectors);
