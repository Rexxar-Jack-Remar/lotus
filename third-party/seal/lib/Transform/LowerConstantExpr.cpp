        ...
        if (isa<Function>(PossibleCallee)) {
            ...
            for (unsigned K = 0; K < CI->arg_size(); ++K) {
                ...
                if (ArgK->getType() == FuncTy->getParamType(K)) {
                    ...
                    assert(DL.getTypeSizeInBits(ArgK->getType()) == DL.getTypeSizeInBits(FuncTy->getParamType(K)));
                    auto *BCI = BitCastInst::CreateBitOrPointerCast(ArgK, FuncTy->getParamType(K), "", &I);
                    Args.push_back(BCI);
                }
            }
