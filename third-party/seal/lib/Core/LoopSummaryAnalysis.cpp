...
LoopSummaryAnalysis::LoopSummaryAnalysis(SingleLoop *SL, ExecutionState *S, unsigned LID) :
        CurrLoop(SL), TripCount(0), PCIndex(0), LoopAnalysisID(LID) {
    ...
    // a starting state
    auto *StartingState = FSM->newState(LoopAnalysisID);
    StartingState->PostConditions = S->fork();
    FSM->RunningState = StartingState;
...
ExecutionState *LoopSummaryAnalysis::getNextInitialState(ExecutionState *ES) {
    if (FSM) {
        switch (FSM->status()) {
            case LoopSummaryStateMachine::LSSM_Summarizing: {
                if (!ES) {
                    FSM->summarizeRecentNewState();
                }
                auto *NextRunningState = FSM->nextRunningState();
                ExecutionStateControl::ControlPath = &FSM->RunningPath;
                assert(NextRunningState->PostConditions);
                return NextRunningState->PostConditions;
            }
            case LoopSummaryStateMachine::LSSM_Fail2Summarize: {
                delete FSM;
                FSM = nullptr;
                return InitialState;
            }
            case LoopSummaryStateMachine::LSSM_Summarized: {
            }
                break;
        }
    }
...
void LoopSummaryAnalysis::collect(LoopSummaryAnalysis *NestedLSA, ExecutionState *ES) {
    ...
    // todo collect more info from nested loops
}
...
void LoopSummaryAnalysis::collect(BasicBlock *B, ExecutionState *ES, unsigned MergeID) {
    ...
    for (auto &I: *B) {
        ...
        if (Cond) {
            State->update(TripCount, Cond, ES->boundValue(Cond)->value());
        }
    }
    ...
    if (B == CurrLoop->getLatch()) {
        // record the most recent post condition
        assert(!State->PostConditions);
        State->PostConditions = ES->fork();
        FSM->summarizeRecentNewState();
