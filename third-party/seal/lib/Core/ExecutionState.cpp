...
const std::set<BasicBlock *> *ExecutionStateControl::ControlPath = nullptr;
...
//    }
//
//    outs() << B->getName() << " -> " << B->getTerminator()->getSuccessor(I)->getName() << "\n";
//    if (B->getName() == "if.end23" || B->getName() == "if.then35") {
//        outs() << Cond << "\n";
//        if (B->getName() == "if.then35")
//            for (unsigned K = 0; K < PC.size(); ++K) {
//                outs() << "[" << K << "] " << PC[K] << "\n";
//            }
//        outs() << "conflict: " << conflict(Cond) << "\n";
//    }

    if (!LoopExiting && conflict(Cond))
        return nullptr;

    auto Succ = B->getTerminator()->getSuccessor(I);
    if (ExecutionStateControl::ControlPath
        && !ExecutionStateControl::ControlPath->empty()
        && !ExecutionStateControl::ControlPath->count(Succ))
        return nullptr;
...
void ExecutionState::merge(BasicBlock *B, unsigned MergeID, std::vector<ExecutionState *> &ESVec,
    ...
    std::vector<BasicBlock *> PredecesorVec;
    for (unsigned I = 0; I < ESVec.size(); ++I) {
        if (MergeCond[I].is_false()) continue;
        auto *ES = ESVec[I];
        assert(ES);
        PredecesorVec.push_back(ES->Block);
        if (!FirstSet) {
...
//            if (!NewPCExpr.is_true() && !Z3::is_free(NewPCExpr)) PC.push_back(NewPCExpr);
//        }
        auto NewPCExpr = Z3::make_phi(MergeID, MergeCondZ3Vec, MergeCondZ3Vec);
        if (!NewPCExpr.is_true() && !Z3::is_free(NewPCExpr)) PC.push_back(NewPCExpr);
    }

    Z3::bind_phi_id(MergeID, B, PredecesorVec);
...
z3::expr ExecutionState::condition(BasicBlock *B, unsigned K) {
...
bool ExecutionState::conflict(const z3::expr &E) {
    ...
    return std::any_of(PC.cbegin(), PC.cend(), [&E](const z3::expr &V) {
        return Z3::simplify(E, V).is_false() || Z3::make_and(E, V).is_false();
    });
}
...
void ExecutionState::popPC(unsigned Len) {
    while (PC.size() > Len) PC.pop_back();
}
...
static z3::expr refine(const z3::expr &E, std::vector<unsigned> &P, unsigned K) {
    if (Z3::is_phi(E)) {
        auto PhiID = Z3::phi_id(E);
        if (P[K] == PhiID) {
            if (K == P.size() - 2) {
                return E.arg(P[K + 1] == 0 ? 1 : 0);
            } else {
                auto Vec = Z3::vec();
                for (unsigned J = 0; J < E.num_args(); ++J) {
                    auto A = E.arg(J);
                    if (J == P[K + 1]) A = refine(A, P, K + 2);
                    Vec.push_back(A);
                }
                return Z3::make_phi(PhiID, Vec);
            }
        } else {
            auto Vec = Z3::vec();
            for (unsigned J = 0; J < E.num_args(); ++J) {
...
static z3::expr refine(const z3::expr &E, std::vector<std::vector<unsigned>> &PS) {
    auto Ret = E;
    for (auto &P: PS) {
        Ret = refine(Ret, P, 0);
    }
    DEBUG_WITH_TYPE("LoopSummaryState", dbgs() << "[refine] " << E << "\n[refine] \t---> " << Ret << "\n");
    return Ret;
}

void ExecutionState::refine(std::vector<std::vector<unsigned>> &PhiSelectors) {
    if (PhiSelectors.empty()) return;

    for (auto &It: AbsValRevisionMap) {
        auto Key = It.first;
        auto Val = It.second;

        if (isa<ScalarValue>(Key)) {
            auto Orig = Val->value();
            auto Refi = ::refine(Orig, PhiSelectors);
            if (Z3::same(Orig, Refi)) continue;
            getValue(Key, true)->set(Refi);
        }
    }
    // todo may need refine other values
    return;
}

z3::expr ExecutionState::refine(const z3::expr &E, std::vector<std::vector<unsigned>> &PhiSelectors) {
    if (PhiSelectors.empty()) return E;
    return ::refine(E, PhiSelectors);
}
