...
LoopSummaryState::LoopSummaryState(SingleLoop *L, unsigned LID) : CurrLoop(L), LoopAnalysisID(LID) {
    static unsigned X = 0;
    ID = X++;
    LLVM_DEBUG(dbgs() << "[FSM] create state (" << ID << ")\n");
}

LoopSummaryState::~LoopSummaryState() {
    LLVM_DEBUG(dbgs() << "[FSM] delete state (" << ID << ")\n");
...
void LoopSummaryState::update(unsigned int TripCount, Value *Cond, const z3::expr &CondExpr) {
    recentIteration(TripCount, Iterations)->ConditionalValues.insert(std::make_pair(Cond, CondExpr));
}

void LoopSummaryState::update(unsigned TripCount, unsigned MergeID) {
    auto It = recentIteration(TripCount, Iterations);

    if (MergeID < It->MinMergeID) It->MinMergeID = MergeID;
    if (MergeID > It->MaxMergeID) It->MaxMergeID = MergeID;

    if (It->MinMergeID == It->MaxMergeID) {
        // the first time, we call this function
        // MergeID is not actually used
    } else {
        auto Vec = Z3::vec();
        for (auto C: Z3::phi_cond(MergeID)) Vec.push_back(C);
        It->PhiConditions.insert(std::make_pair(MergeID, Vec));
    }
}
...
static z3::expr substitute(const z3::expr &Cond, std::map<unsigned, unsigned> &PhiSelector) {
    if (Cond.is_const()) return Cond;

    auto Decl = Cond.decl();
    if (Z3::is_phi(Cond)) {
        auto PhiID = Z3::phi_id(Cond);
        if (PhiSelector.count(PhiID)) {
            return substitute(Cond.arg(PhiSelector[PhiID]), PhiSelector);
        }
    }
    auto Vec = Z3::vec();
    for (unsigned K = 0; K < Cond.num_args(); ++K) {
        Vec.push_back(substitute(Cond.arg(K), PhiSelector));
    }
    if (Z3::is_phi(Cond)) {
        return Z3::make_phi(Z3::phi_id(Cond), Vec);
    } else if (Cond.is_and()) {
        auto AndResult = Z3::make_and(Vec);
        if (Z3::is_free(AndResult)) return Z3::bool_val(true);
        return AndResult;
    } else if (Cond.is_or()) {
        auto OrResult = Z3::make_or(Vec);
        if (Z3::is_free(OrResult)) return Z3::bool_val(true);
        return OrResult;
    }
    return Decl(Vec);
}

static void repair(std::set<BasicBlock *> &Path, SingleLoop *CurrLoop) {
    auto Latch = CurrLoop->getLatch();
    std::set<BasicBlock *> RealPath;
    std::vector<BasicBlock *> DFSStack;
    if (Path.count(Latch)) DFSStack.push_back(Latch);
    while (!DFSStack.empty()) {
        auto *Top = DFSStack.back();
        DFSStack.pop_back();
        if (RealPath.count(Top)) continue;
        RealPath.insert(Top);

        for (auto It = pred_begin(Top), E = pred_end(Top); It != E; ++It) {
            auto *Pred = *It;
            if (Path.count(Pred)) {
                DFSStack.push_back(Pred);
            }
        }
    }
    assert(RealPath.empty() || RealPath.count(CurrLoop->getHeader()));
    assert(RealPath.empty() || RealPath.count(CurrLoop->getLatch()));
    assert(RealPath.size() <= Path.size());

    Path.swap(RealPath);
}

void LoopSummaryState::repairPath() {
    repair(Path, CurrLoop);
}
...
void intersect(const LoopSummaryState &S1,
    ...
    LLVM_DEBUG(
            dbgs() << "[Intersect] S1: ";
            for (auto *B: S1.Path) dbgs() << B->getName() << ", ";
            dbgs() << "\n";
            dbgs() << "[Intersect] S2: ";
            for (auto *B: S2.Path) dbgs() << B->getName() << ", ";
            dbgs() << "\n";
    );
    ...
    if (!Common.count(S1.CurrLoop->getLatch())) {
        Common.clear();
        P1 = S1.Path;
        P2 = S2.Path;
        LLVM_DEBUG(dbgs() << "[Intersect] Intersection is empty!\n");
        return;
    }
    repair(Common, S1.CurrLoop);
    ...
    while (!DFSStack.empty()) {
        auto *Top = DFSStack.back();
        DFSStack.pop_back();
        if (P1.count(Top)) continue;
        P1.insert(Top);

        bool DistinguishableBlock = false;
        for (auto It = succ_begin(Top), E = succ_end(Top); It != E; ++It) {
            auto *Succ = *It;
            if (!S1.CurrLoop->contains(Succ)) continue;
            if (!S1.Path.count(Succ) && Common.count(Succ)) {
                DistinguishableBlock = true;
                break;
            } else if (S1.Path.count(Succ) && !Common.count(Succ)) {
                DistinguishableBlock = true;
                break;
            }
        }
        if (DistinguishableBlock) {
            ...
            for (auto It = succ_begin(Top), E = succ_end(Top); It != E; ++It) {
                auto *Succ = *It;
                if (S1.Path.count(Succ)) {
                    assert(Common.count(Succ));
                } else {
                    assert(!Common.count(Succ));
                    continue;
                }
    ...
    assert(P1.count(S1.CurrLoop->getHeader()));
    assert(P1.count(S1.CurrLoop->getLatch()));
    if (P1.size() == S1.Path.size()) {
        // no distinguishable block is found
        P1.clear();
    }

    // S2 - Common
    DFSStack.clear();
    DFSStack.push_back(S2.CurrLoop->getHeader());
    while (!DFSStack.empty()) {
        auto *Top = DFSStack.back();
        DFSStack.pop_back();
        if (P2.count(Top)) continue;
        P2.insert(Top);

        bool DistinguishableBlock = false;
        for (auto It = succ_begin(Top), E = succ_end(Top); It != E; ++It) {
            auto *Succ = *It;
            if (!S2.CurrLoop->contains(Succ)) continue;
            if (!S2.Path.count(Succ) && Common.count(Succ)) {
                DistinguishableBlock = true;
                break;
            } else if (S2.Path.count(Succ) && !Common.count(Succ)) {
                DistinguishableBlock = true;
                break;
            }
        }
    ...
    assert(P2.count(S2.CurrLoop->getHeader()));
    assert(P2.count(S2.CurrLoop->getLatch()));
    if (P2.size() == S2.Path.size()) {
        // no distinguishable block is found
        P2.clear();
    }
    ...
    LLVM_DEBUG(
            dbgs() << "[Intersect] P1: ";
            for (auto *B: P1) dbgs() << B->getName() << ", ";
            dbgs() << "\n";
            dbgs() << "[Intersect] CC: ";
            for (auto *B: Common) dbgs() << B->getName() << ", ";
            dbgs() << "\n";
            dbgs() << "[Intersect] P2: ";
            for (auto *B: P2) dbgs() << B->getName() << ", ";
            dbgs() << "\n";
    );
...
void subtract(const LoopSummaryState &S, std::set<BasicBlock *> &P, std::set<BasicBlock *> &R) {
    // S1 - Common
    std::vector<BasicBlock *> DFSStack;
    DFSStack.push_back(S.CurrLoop->getHeader());
    while (!DFSStack.empty()) {
        auto *Top = DFSStack.back();
        DFSStack.pop_back();
        if (R.count(Top)) continue;
        R.insert(Top);

        bool DistinguishableBlock = false;
        for (auto It = succ_begin(Top), E = succ_end(Top); It != E; ++It) {
            auto *Succ = *It;
            if (!S.CurrLoop->contains(Succ)) continue;
            if (P.count(Succ)) assert(S.Path.count(Succ));

            if (!S.Path.count(Succ) && P.count(Succ)) {
                DistinguishableBlock = true;
                break;
            } else if (S.Path.count(Succ) && !P.count(Succ)) {
                DistinguishableBlock = true;
                break;
            }
        }
    ...
    assert(R.count(S.CurrLoop->getHeader()));
    assert(R.count(S.CurrLoop->getLatch()));
    if (R.size() == S.Path.size()) {
        // no distinguishable block is found
        R.clear();
    }
}
...
z3::expr LoopIteration::condition(BasicBlock *B, unsigned K) const {
    ...
    assert(ConditionalValues.count(Val));
    auto CondVar = ConditionalValues.at(Val);
    if (Z3::is_free(CondVar))
        return Z3::free_bool(); // condition that is not interesting

    if (auto *SW = dyn_cast<SwitchInst>(Terminator)) {
        int64_t CaseVar;
        if (Z3::is_numeral_i64(CondVar, CaseVar)) {
            unsigned KK = 2;
            ...
            if (K == 0) {
                ...
                for (unsigned I = 2; I < SW->getNumOperands(); I += 2) {
                    ...
                    auto ThisCond = Z3::ne(CondVar, Z3::bv_val(SWInt, CondVar.get_sort().bv_size()));
                ...
                CaseCond = Z3::eq(CondVar, Z3::bv_val(SWInt, CondVar.get_sort().bv_size()));
            }
            return CaseCond;
        }
    } else if (auto *Br = dyn_cast<BranchInst>(Terminator)) {
        if (Br->isConditional()) {
            auto Cond = Z3::bv_to_bool(CondVar);
