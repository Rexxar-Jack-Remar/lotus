...
\label{Se:Linux}
\WALi requires CMake, a C++ compiler, and Boost. The following commands will
download and extract the \WALi source tree.
...
\noindent This will compile and link the \WALi library and the OpenNWA
command-line tools in the directory \verb!build!.

The \WALi source tree comes bundled with some examples and addons. Selected
examples can be compiled by configuring with
\verb!-DWALI_BUILD_EXAMPLES=ON!, and addons can be compiled with
\verb!-DWALI_BUILD_ADDONS=ON!. The examples include weight domains
(\sectref{WeightDomains})
...
After compiling shared libraries, the relevant CMake build or installation
library directory should be added to the environment variable
\verb!LD_LIBRARY_PATH! or \verb!PATH! for Linux or Cygwin, respectively.
