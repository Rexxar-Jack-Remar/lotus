...
class LegacyPointerAnalysis : public llvm::ModulePass {
 public:
  ...
  LegacyPointerAnalysis(Analysis analysis, ContextSensitivity cs = INSENSITIVE)
      : llvm::ModulePass(ID), analysis_(analysis), cs_(cs) {}

  void setAnalysis(Analysis a) { analysis_ = a; }
  void setContextSensitivity(ContextSensitivity cs) { cs_ = cs; }
  void setSignatures(const std::string &sig) { signatures_ = sig; }
  void setCheckAssertions(bool check) { check_assertions_ = check; }
  void setDebugDir(const std::string &dir) { debug_dir_ = dir; }
 ...
 private:
  ...
  llvm::Optional<Analysis> analysis_;
  llvm::Optional<ContextSensitivity> cs_;
  llvm::Optional<std::string> signatures_;
  llvm::Optional<bool> check_assertions_;
  llvm::Optional<std::string> debug_dir_;
};
