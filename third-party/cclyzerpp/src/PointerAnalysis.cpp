...
auto LegacyPointerAnalysis::runOnModule(llvm::Module &mod) -> bool {
  const Analysis chosen_analysis =
      analysis_.hasValue() ? *analysis_ : datalog_analysis.getValue();
  const ContextSensitivity chosen_cs =
      cs_.hasValue() ? *cs_ : context_sensitivity.getValue();
  const std::string chosen_dir_str =
      debug_dir_.hasValue() ? *debug_dir_ : datalog_debug_dir_option.getValue();
  const bool chosen_debug =
      datalog_debug_option.getValue() || debug_dir_.hasValue();
  const bool chosen_check =
      check_assertions_.hasValue() ? *check_assertions_
                                   : datalog_check_assertions_option.getValue();
  const std::string chosen_sigs =
      signatures_.hasValue() ? *signatures_ : signatures.getValue();

  const fs::path output_dir = fs::path(chosen_dir_str);
  ...
  if (chosen_sigs != "") {
    signatures_path = llvm::Optional<fs::path>(fs::path(chosen_sigs));
  } else {
    signatures_path = llvm::Optional<fs::path>();
  }

  auto [dir, llvm_val_map] =
      factgen_module(mod, output_dir, signatures_path, chosen_cs);
  const auto pa = get_interface(chosen_analysis);
  ...
  if (chosen_check) {
    pa->checkAssertions(chosen_analysis == Analysis::DEBUG);
  }
  ...
  const auto callgraph_vec =
      pa->relationToVector<int, const llvm::Value *, int, const llvm::Value *>(
          callgraph_edge(chosen_analysis), llvm_val_map);
  ...
  auto var_points_to_rel = pa->relationToVector<
      ...
      int,
      const llvm::Value *>(var_points_to(chosen_analysis), llvm_val_map);
  ...
  result_ = std::make_unique<PointerAnalysisAAResult>(
      ...
      pa->relationToVector<
          ...
          boost::flyweight<std::string>,
          boost::flyweight<std::string>>(
          alloc_may_alias(chosen_analysis), llvm_val_map),
      pa->relationToVector<
          int,
          boost::flyweight<std::string>,
          boost::flyweight<std::string>>(
          alloc_must_alias(chosen_analysis), llvm_val_map),
      pa->relationToVector<
          int,
          boost::flyweight<std::string>,
          boost::flyweight<std::string>>(
          alloc_subregion(chosen_analysis), llvm_val_map),
      pa->relationToVector<
          int,
          boost::flyweight<std::string>,
          boost::flyweight<std::string>>(
          alloc_contains(chosen_analysis), llvm_val_map),
      pa->relationToVector<
          int,
          boost::flyweight<std::string>,
          int,
          boost::flyweight<std::string>>(
          ptr_points_to(chosen_analysis), llvm_val_map),
      pa->relationToVector<
          int,
          boost::flyweight<std::string>,
          int,
          const llvm::Value *>(
          operand_points_to(chosen_analysis), llvm_val_map),
      ...
      pa->relationToVector<int, boost::flyweight<std::string>, int>(
          allocation_size(chosen_analysis), llvm_val_map),
      pa->relationToVector<
          int,
          const llvm::Value *,
          int,
          boost::flyweight<std::string>>(
          allocation_by_instr(chosen_analysis), llvm_val_map),
