...
public:
  debug_info() : m_file(""), m_line(-1), m_col(-1), m_id(-1) {}
  ...
  bool operator==(const debug_info &other) const {
    return (m_id == other.m_id && m_file == other.m_file &&
            m_line == other.m_line && m_col == other.m_col);
  }

  bool has_debug() const {
    return ((m_id >= 0) || ((m_file != "") && (m_line >= 0) && (m_col >= 0)));
  }
...
public:
  ...
  virtual void write(crab_os &o) const override { o << "unreachable"; }
};
...
public:
  ...
  region_cast_stmt(variable_t src_region, variable_t dst_region,
                   basic_block_t *parent)
      : statement_t(REGION_CAST, parent), m_src_region(src_region),
  ...
  virtual void write(crab_os &o) const override {
    o << "region_cast " << src() << ":" << src_type() << " to " << dst() << ":"
      << dst_type();
  }
...
public:
  ...
  virtual statement_t *clone(basic_block_t *parent) const override {
    return new this_type(m_lhs, m_region, m_size, m_alloc_site, parent,
                         this->m_dbg_info);
  }

  virtual void write(crab_os &o) const override {
    o << m_lhs << " := "
      << "make_ref(" << m_region << ":" << m_region.get_type() << "," << m_size
      << "," << m_alloc_site << ")";
  }
...
public:
  ...
  remove_ref_stmt(variable_t region, variable_t ref, basic_block_t *parent,
                  debug_info dbg_info = debug_info())
      : statement_t(REF_REMOVE, parent, dbg_info), m_region(region),
        m_ref(ref) {
    this->m_live.add_use(m_region);
  ...
  virtual void write(crab_os &o) const override {
    o << "remove_ref(" << m_region << ":" << m_region.get_type() << "," << m_ref
      << ":" << m_ref.get_type() << ")";
  }
...
public:
  ...
  intrinsic_stmt(std::string intrinsic_name,
      ...
      : statement_t(CRAB_INTRINSIC, parent, dbg_info),
        m_intrinsic_name(intrinsic_name) {
...
public:
  ...
  const basic_block_label_t &entry() const { return get().entry(); }
  ...
  bool has_func_decl() const { return get().has_func_decl(); }

  const fdecl_t &get_func_decl() const { return get().get_func_decl(); }

  bool has_exit() const { return get().has_exit(); }

  const basic_block_label_t &exit() const { return get().exit(); }
...
// big bottleneck.
template <typename CFG> class callsite_or_fdecl {
public:
...
private:
  ...
  static bool match_callee_and_function(const callsite_t &cs,
                                        const fdecl_t &fdecl) {
...
template <class CFG> struct hasher {
  size_t operator()(const callsite_or_fdecl<CFG> &o) const { return o.hash(); }
};
template <class CFG> struct compare {
