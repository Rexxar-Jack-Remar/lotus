...
// patricia tries.
template <typename CFG> class assert_wrapper : public indexable {
public:
  using statement_t =
      statement<typename CFG::basic_block_label_t, typename CFG::number_t,
...
public:
  assert_wrapper(ikos::index_t id, statement_t *assert)
    ...
    if (!(m_assert->is_assert() || m_assert->is_ref_assert() ||
          m_assert->is_bool_assert())) {
      CRAB_ERROR("assert_wrapper must be assert, ref_assert, or bool_assert");
    }
  }

  statement_t &get() { return *m_assert; }
  const statement_t &get() const { return *m_assert; }
  virtual ikos::index_t index() const override { return m_id; }
  bool operator==(this_type o) const { return m_id == o.m_id; }
  bool operator<(this_type o) const { return m_id < o.m_id; }
...
template <typename CFG> class assertion_crawler_domain {
public:
...
public:
  ...
  first_domain_t &get_first() { return m_first; }

  const first_domain_t &get_first() const { return m_first; }

  second_domain_t &get_second() { return m_second; }

  const second_domain_t &get_second() const { return m_second; }

  bool operator<=(const this_type &other) const {
    return (get_first() <= other.get_first() &&
  ...
  friend crab::crab_os &operator<<(crab::crab_os &o,
                                   const assertion_crawler_domain<CFG> &dom) {
    dom.write(o);
    return o;
  }
};
...
template <typename Key, typename Value> struct transform_discrete_pair_domain {
  ...
  template <typename Op>
  discrete_pair_domain_t apply_on_key_and_value(discrete_pair_domain_t sol,
                                                Op &op) {
    // All of this is needed because discrete_pair_domain_t cannot
    // be modified in-place
    if (sol.is_bottom() || sol.is_top()) {
...
// assertion_crawler_domain.
template <class CFG> class summary {
public:
  using variable_t = typename CFG::variable_t;
...
public:
  summary(const std::vector<variable_t> &inputs,
          const std::vector<variable_t> &outputs, abs_dom_t sum)
      : m_sum(sum) {
  ...
  const abs_dom_t &get_summary() const { return m_sum; }
  abs_dom_t &get_summary() { return m_sum; }
...
public:
  using assertion_crawler_domain_t =
      assertion_crawler_impl::assertion_crawler_domain<CFG>;
  ...
  using assert_map_t =
      typename std::unordered_map<statement_t *, assert_wrapper_t>;
  ...
  using summary_map_t =
      callsite_or_fdecl_map<CFG, assertion_crawler_impl::summary<CFG>>;

private:
  using killgen_operations_api_t =
      killgen_operations_api<CFG, assertion_crawler_domain_t>;
  using assert_map_domain_t =
      typename assertion_crawler_domain_t::first_domain_t;
  using summary_dependencies_domain_t =
      typename assertion_crawler_domain_t::second_domain_t;
...
public:
  ...
  class transfer_function
      : public statement_visitor<basic_block_label_t, number_t, varname_t> {
    using visitor_t =
    ...
    public:
      add_data_deps(const live_t &l)
          : m_uses(var_dom_t::bottom()), m_defs(var_dom_t::bottom()) {
        for (auto v :
             boost::make_iterator_range(l.uses_begin(), l.uses_end())) {
          m_uses += v;
        }
        for (auto v :
             boost::make_iterator_range(l.defs_begin(), l.defs_end())) {
          m_defs += v;
        }
      }
      ...
      std::pair<var_dom_t, bool> operator()(var_dom_t d) {
        bool change = false;
        if (m_defs.is_bottom() && !m_uses.is_bottom() &&
            !(m_uses & d).is_bottom()) {
          d += m_uses;
          change = true;
        }
    ...
    using transform_sol_t =
        assertion_crawler_impl::transform_discrete_pair_domain<assert_wrapper_t,
                                                               var_dom_t>;
    using transform_sum_deps_t =
        assertion_crawler_impl::transform_discrete_pair_domain<variable_t,
                                                               var_dom_t>;
    ...
    inline void propagate_data(statement_t &s) {
      ...
      if (m_opt_ignore_region_offset) {
        ...
        if (op) {
          transform_sol_t f1;
          m_sol.get_first() =
              std::move(f1.apply_on_value(m_sol.get_first(), *op));
          transform_sum_deps_t f2;
          m_sol.get_second() =
              std::move(f2.apply_on_value(m_sol.get_second(), *op));
        }
      } else {
    ...
    inline void propagate_data_and_control(statement_t &s) {
      ...
      m_sol.get_second() =
          std::move(df2.apply_on_value(m_sol.get_second(), op));
      ...
      if (!m_cdg.empty()) {
        // -- add control dependencies
        for (auto const &pred :
             boost::make_iterator_range(s.get_parent()->prev_blocks())) {
          ...
          if (it != m_cdg.end()) {
            ...
            transform_sol_t cf;
            m_sol.get_first() =
                std::move(cf.apply_on_key_and_value(m_sol.get_first(), op));
  ...
  public:
    transfer_function(assertion_crawler_domain_t init, const cdg_t &g,
                      assert_map_t &assert_map, summary_map_t &summaries,
    ...
    assertion_crawler_domain_t get_solution() const { return m_sol; }

    virtual void visit(bin_op_t &s) override { propagate_data(s); }

    virtual void visit(assign_t &s) override { propagate_data(s); }

    virtual void visit(assume_t &s) override { propagate_data_and_control(s); }

    virtual void visit(select_t &s) override { propagate_data(s); }

    virtual void visit(assert_t &s) override { process_assertion(s); }

    virtual void visit(int_cast_t &s) override { propagate_data(s); }

    virtual void visit(unreach_t &) override { m_sol.set_to_bottom(); }
    ...
    static void rename(discrete_pair_domain<Key, var_dom_t> &dpd,
                       const std::vector<variable_t> &from,
      ...
      if (from.size() != to.size()) {
        CRAB_ERROR("discrete_pair_domain::rename with input vectors of "
                   "different sizes");
      }
    ...
    // This function replaces the occurrences of each sdd's key in dpd with its
    // value. That is, [key1 -> {i1,i2,x1,...}, key2 -> {i3,i4,x2,...}, ...]
    template <typename Key>
    ...
    virtual void visit(callsite_t &s) override {
      ...
      if (it != m_summaries.end()) {
        ...
        const std::vector<variable_t> &callee_inputs =
            callee_summary_info.get_inputs();
        const std::vector<variable_t> &callee_outputs =
            callee_summary_info.get_outputs();
        auto &callee_summary = callee_summary_info.get_summary();

        CRAB_LOG(
            "assertion-crawler-step-cs",
        ...
        CRAB_LOG(
            "assertion-crawler-step-cs",
            CRAB_WARN("assertion-crawler did not find summary for callee at ",
                      s));
      ...
      CRAB_LOG("assertion-crawler-step-cs",
               crab::outs() << "\tAFTER " << m_sol << "\n";);
    }

    virtual void visit(intrinsic_t &s) override { propagate_data(s); }

    virtual void visit(make_ref_t &s) override { propagate_data(s); }

    virtual void visit(remove_ref_t &s) override { propagate_data(s); }

    virtual void visit(region_init_t &s) override { propagate_data(s); }

    virtual void visit(region_copy_t &s) override { propagate_data(s); }

    virtual void visit(region_cast_t &s) override { propagate_data(s); }

    virtual void visit(load_from_ref_t &s) override { propagate_data(s); }

    virtual void visit(store_to_ref_t &s) override { propagate_data(s); }

    virtual void visit(gep_ref_t &s) override { propagate_data(s); }

    virtual void visit(assume_ref_t &s) override {
      propagate_data_and_control(s);
    }

    virtual void visit(assert_ref_t &s) override { process_assertion(s); }

    virtual void visit(select_ref_t &s) override { propagate_data(s); }

    virtual void visit(bool_bin_op_t &s) override { propagate_data(s); }

    virtual void visit(bool_assign_var_t &s) override { propagate_data(s); }

    virtual void visit(bool_assign_cst_t &s) override { propagate_data(s); }

    virtual void visit(bool_select_t &s) override { propagate_data(s); }

    virtual void visit(bool_assume_t &s) override {
      propagate_data_and_control(s);
    }

    virtual void visit(bool_assert_t &s) override { process_assertion(s); }
  };
...
public:
  assertion_crawler_operations(CFG cfg, assert_map_t &assert_map,
                               summary_map_t &summaries, bool only_data,
                               bool ignore_region_offset)
      : killgen_operations_api_t(cfg), m_assert_map(assert_map),
        m_summaries(summaries), m_opt_only_data(only_data),
        m_opt_ignore_region_offset(ignore_region_offset) {}
  ...
  virtual assertion_crawler_domain_t entry() override {
    if (this->m_cfg.has_func_decl()) {
      return assertion_crawler_domain_t(
          this->m_cfg.get_func_decl().get_outputs());
    } else {
      return assertion_crawler_domain_t();
    }
  }
  ...
  virtual assertion_crawler_domain_t
  analyze(const basic_block_label_t &bb_id,
  ...
  const cdg_t &get_cdg() const { return m_cdg; }
  assert_map_t &get_assert_map() { return m_assert_map; }
  const assert_map_t &get_assert_map() const { return m_assert_map; }
  summary_map_t &get_summary_map() { return m_summaries; }
  const summary_map_t &get_summary_map() const { return m_summaries; }
};

template <typename CallGraph> class inter_assertion_crawler;
...
class assertion_crawler
    : public killgen_fixpoint_iterator<CFG, assertion_crawler_operations<CFG>> {
...
public:
  using assertion_crawler_domain_t =
      typename assertion_crawler_op_t::assertion_crawler_domain_t;
  using assert_map_domain_t =
      typename assertion_crawler_domain_t::first_domain_t;
  ...
  using results_map_t =
      std::unordered_map<basic_block_label_t, assertion_crawler_domain_t>;
...
public:
  assertion_crawler(CFG cfg, assert_map_t &assert_map, summary_map_t &summaries,
                    bool only_data = false, bool ignore_region_offset = false)
      : fixpo_t(cfg, m_assert_crawler_op),
        m_assert_crawler_op(cfg, assert_map, summaries, only_data,
                            ignore_region_offset) {}
    ...
    if (it != m_results.end()) {
      if (!it->second.get_first().is_bottom()) {
        ...
        typename assertion_crawler_op_t::transfer_function vis(
            it->second /* OUT dataflow facts */, m_assert_crawler_op.m_cdg,
            m_assert_crawler_op.m_assert_map, m_assert_crawler_op.m_summaries,
...
public:
  ...
  using assertion_crawler_domain_t =
      typename assertion_crawler_t::assertion_crawler_domain_t;
...
private:
  ...
  using inter_results_map_t =
      std::unordered_map<cfg_t, std::unique_ptr<assertion_crawler_t>>;
...
public:
  inter_assertion_crawler(cg_t &cg, bool only_data = false,
                          bool ignore_region_offset = false)
      : m_cg(cg), m_opt_only_data(only_data),
        m_opt_ignore_region_offset(ignore_region_offset) {}
  ...
  void run(bool do_type_checking = true) {
    ...
    CRAB_LOG("assertion-crawler", m_cg.write(crab::outs());
             crab::outs() << "\n");
    ...
    auto analyze = [this](cfg_t cfg) {
      ...
      assertion_crawler_domain_t &results =
          intra_analysis->lookup_results(cfg.entry());
    ...
    auto hasStabilized = [](assertion_crawler_t &old,
                            assertion_crawler_t &current, cfg_t cfg) {
      for (auto it = cfg.label_begin(), et = cfg.label_end(); it != et; ++it) {
        ...
        const assertion_crawler_domain_t &current_d =
            current.lookup_results(bb);
        if (!(current_d <= old_d)) {
          return false;
        }
      }
    ...
    for (auto const &n : rev_order) {
      ...
      if (isRecursive) {
        ...
        if (iter > max_fixpo_iters) {
          CRAB_WARN("Inter-procedural Assertion Crawler analysis was abruptly "
                    "stopped on a ",
        ...
        for (auto m : scc_mems) {
          ...
          m_inter_results.insert(
              std::make_pair(cfg, std::move(intra_analysis)));
        }
      }
    }
  ...
  // Return the dataflow facts that hold at the entry of the basic block b in
  // cfg.
  assert_map_domain_t get_results(const cfg_t &cfg,
                                  const basic_block_label_t &bb) const {
    auto it = m_inter_results.find(cfg);
