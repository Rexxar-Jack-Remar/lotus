...
class swrapped_interval_domain final
    : public abstract_domain_api<swrapped_interval_domain<
...
private:
  ...
  swrapped_interval_t eval_expr(const linear_expression_t &expr,
    ...
    for (auto kv : expr) {
      ...
      swrapped_interval_t c =
          swrapped_interval_t::mk_swinterval(kv.first, width);
...
public:
  ...
  bool operator<=(const swrapped_interval_domain_t &e) const override {
    ...
    CRAB_LOG("swrapped-int", crab::outs() << *this << " <= " << e << "=";);
    bool res = (this->_env <= e._env);
    CRAB_LOG("swrapped-int", crab::outs() << (res ? "yes" : "not") << "\n";);
    return res;
  }
  ...
  void set(const variable_t &v, interval_t i) {
    ...
    if (i.lb().is_finite() && i.ub().is_finite()) {
      swrapped_interval_t rhs = swrapped_interval_t::mk_swinterval(
  ...
  void set(const variable_t &v, number_t n) {
    crab::CrabStats::count(domain_name() + ".count.assign");
    crab::ScopedCrabStats __st__(domain_name() + ".assign");
    this->_env.set(v, swrapped_interval_t::mk_swinterval(
                          n, v.get_type().is_integer()
                                 ? v.get_type().get_integer_bitwidth()
                                 : 0));
    // crab::outs() << "set" << "\n";
    CRAB_LOG("swrapped-int",
             crab::outs() << v << ":=" << n << "=" << _env.at(v) << "\n");
  }
  ...
  void weak_assign(const variable_t &x, const linear_expression_t &e) override {
    ...
    CRAB_LOG("swrapped-int", crab::outs() << "weak_assign(" << x << "," << e
                                          << ")=" << _env.at(x) << "\n");
  }
  ...
  linear_constraint_system_t to_linear_constraint_system() const override {
    ...
    for (iterator it = this->_env.begin(); it != this->_env.end(); ++it) {
      ...
      if (!i.is_top() && (i.end_0() != wrapint::get_signed_max(width)) &&
        ...
        csts +=
            linear_constraint_t(v >= i.getSignedMinValue().get_signed_bignum());
        csts +=
            linear_constraint_t(v <= i.getSignedMaxValue().get_signed_bignum());
      }
    }
    return csts;
  }
