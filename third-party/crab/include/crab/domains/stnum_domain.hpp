...
private:
  using separate_domain_t = ikos::separate_domain<variable_t, stnum_t>;
  using solver_t =
...
private:
  ...
  stnum_t eval_expr(const linear_expression_t &expr, bitwidth_t width) {
    if (width == 0) {
      return stnum_t::top();
    }

    stnum_t r = stnum_t::mk_stnum(expr.constant(), width);
    for (auto kv : expr) {
...
public:
  ...
  stnum_domain_t operator|(const stnum_domain_t &e) const override {
    crab::CrabStats::count(domain_name() + ".count.join");
    crab::ScopedCrabStats __st__(domain_name() + ".join");
    CRAB_LOG("stnum", crab::outs() << *this << " U " << e << " = ");
  ...
  void set(const variable_t &v, interval_t i) {
    ...
    if (i.lb().is_finite() && i.ub().is_finite()) {
      stnum_t rhs = stnum_t::mk_stnum(
          *(i.lb().number()), *(i.ub().number()),
          v.get_type().is_integer() ? v.get_type().get_integer_bitwidth() : 0);
      this->_env.set(v, rhs);
      CRAB_LOG("stnum", crab::outs()
                            << v << ":=" << i << "=" << _env.at(v) << "\n");
    } else {
      CRAB_WARN("ignored assignment of an open interval in stnum domain");
      *this -= v;
    }
  }
  ...
  void set(const variable_t &v, number_t n) {
    ...
    crab::ScopedCrabStats __st__(domain_name() + ".assign");
    this->_env.set(
        v, stnum_t::mk_stnum(n, v.get_type().is_integer()
  ...
  stnum_t get_stnum(const variable_t &v) const { return this->_env.at(v); }
  ...
  void assign(const variable_t &x, const linear_expression_t &e) override {
    ...
    if (std::optional<variable_t> v = e.get_variable()) {
      ...
      stnum_t r = eval_expr(e, x.get_type().is_integer()
  ...
  void weak_assign(const variable_t &x, const linear_expression_t &e) override {
    ...
    CRAB_LOG("stnum", crab::outs() << "weak_assign(" << x << "," << e
                                   << ")=" << _env.at(x) << "\n");
  }
  ...
  void apply(arith_operation_t op, const variable_t &x, const variable_t &y,
    ...
    this->_env.set(x, xi);
    CRAB_LOG("stnum", crab::outs() << x << ":=" << y << " " << op << " " << z
                                   << "=" << _env.at(x) << "\n");
  }
  ...
  void apply(arith_operation_t op, const variable_t &x, const variable_t &y,
    ...
    this->_env.set(x, xi);
    CRAB_LOG("stnum", crab::outs() << x << ":=" << y << " " << op << " " << k
                                   << "=" << _env.at(x) << "\n");
  }
  ...
  linear_constraint_system_t to_linear_constraint_system() const override {
    ...
    for (iterator it = this->_env.begin(); it != this->_env.end(); ++it) {
      ...
      if (!flag0 && flag1) {
        ...
        csts +=
            linear_constraint_t(v <= (w_value0 + w_mask0).get_signed_bignum());
      } else if (flag0 && !flag1) {
        ...
        csts +=
            linear_constraint_t(v <= (w_value1 + w_mask1).get_signed_bignum());
      } else {
        wrapint w_mask0 = i.get_tnum_0().mask();
        wrapint w_value0 = i.get_tnum_0().value();
        wrapint w_value1 = i.get_tnum_1().value();
  ...
  to_disjunctive_linear_constraint_system() const override {
    CRAB_ERROR("TODO: to_disjunctive_linear_constraint_system in stnum");
  }
...
template <typename Number, typename VariableName>
class constraint_simp_domain_traits<stnum_domain<Number, VariableName>> {
public:
