...
class tnum_domain final
  ...
  using tnum_domain_t = tnum_domain<Number, VariableName, max_reduction_cycles>;
...
private:
  using separate_domain_t = ikos::separate_domain<variable_t, tnum_t>;
  using solver_t =
...
private:
  ...
  tnum_t eval_expr(const linear_expression_t &expr, bitwidth_t width) {
    if (width == 0) {
      return tnum_t::top();
    }

    tnum_t r = tnum_t::mk_tnum(expr.constant(), width);
    for (auto kv : expr) {
...
public:
  ...
  tnum_domain_t operator|(const tnum_domain_t &e) const override {
    crab::CrabStats::count(domain_name() + ".count.join");
    crab::ScopedCrabStats __st__(domain_name() + ".join");
    CRAB_LOG("tnum", crab::outs() << *this << " U " << e << " = ");
  ...
  void set(const variable_t &v, interval_t i) {
    ...
    if (i.lb().is_finite() && i.ub().is_finite()) {
      ...
      CRAB_WARN("ignored assignment of an open interval in tnum domain");
      *this -= v;
    }
  }
  ...
  void set(const variable_t &v, number_t n) {
    ...
    crab::ScopedCrabStats __st__(domain_name() + ".assign");
    this->_env.set(v,
                   tnum_t::mk_tnum(n, v.get_type().is_integer()
  ...
  tnum_t get_tnum(const variable_t &v) const { return this->_env.at(v); }
  ...
  void assign(const variable_t &x, const linear_expression_t &e) override {
    ...
    if (std::optional<variable_t> v = e.get_variable()) {
      ...
      tnum_t r = eval_expr(e, x.get_type().is_integer()
  ...
  void weak_assign(const variable_t &x, const linear_expression_t &e) override {
    ...
    CRAB_LOG("tnum", crab::outs() << "weak_assign(" << x << "," << e
                                  << ")=" << _env.at(x) << "\n");
  }
  ...
  void apply(arith_operation_t op, const variable_t &x, const variable_t &y,
    ...
    this->_env.set(x, xi);
    CRAB_LOG("tnum", crab::outs() << x << ":=" << y << " " << op << " " << z
                                  << "=" << _env.at(x) << "\n");
  }
  ...
  void apply(arith_operation_t op, const variable_t &x, const variable_t &y,
    ...
    this->_env.set(x, xi);
    CRAB_LOG("tnum", crab::outs() << x << ":=" << y << " " << op << " " << k
                                  << "=" << _env.at(x) << "\n");
  }
  ...
  linear_constraint_system_t to_linear_constraint_system() const override {
    ...
    for (iterator it = this->_env.begin(); it != this->_env.end(); ++it) {
      ...
      if (w_mask.msb()) {
        ...
        csts +=
            linear_constraint_t(v <= (w_value + w_mask).get_signed_bignum());
      }
    }
    return csts;
  }
...
template <typename Number, typename VariableName>
class constraint_simp_domain_traits<tnum_domain<Number, VariableName>> {
public:
