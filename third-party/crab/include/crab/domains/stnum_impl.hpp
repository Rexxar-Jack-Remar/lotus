...
stnum<Number>
stnum<Number>::default_implementation(const stnum<Number> &x) const {
...
template <typename Number>
stnum<Number>::stnum(tnum_t tnum0, tnum_t tnum1, bool is_bottom0,
                     bool is_bottom1)
    : tnum_0(tnum0), tnum_1(tnum1), m_is_bottom_0(is_bottom0),
      m_is_bottom_1(is_bottom1) {
...
template <typename Number>
stnum<Number>::stnum(wrapint value_0, wrapint mask_0, wrapint value_1,
                     wrapint mask_1, bool is_bottom_0, bool is_bottom_1) {
...
template <typename Number> stnum<Number> stnum<Number>::Shl(uint64_t k) const {
  tnum_t t0 = tnum_0.Shl(k);
  tnum_t t1 = tnum_1.Shl(k);
  return stnum<Number>::normalize(t0, t1);
}

template <typename Number> stnum<Number> stnum<Number>::LShr(uint64_t k) const {
  tnum_t t0 = tnum_0.LShr(k);
  tnum_t t1 = tnum_1.LShr(k);
  return stnum<Number>::normalize(t0, t1);
}

template <typename Number> stnum<Number> stnum<Number>::AShr(uint64_t k) const {
...
template <typename Number>
stnum<Number>::stnum()
    : tnum_0(tnum<Number>(wrapint(0, 3), wrapint(3, 3), false)),
      tnum_1(tnum<Number>(wrapint(4, 3), wrapint(3, 3), false)),
      m_is_bottom_0(false), m_is_bottom_1(false) {}

// return top if n does not fit into a wrapint. No runtime errors.
template <typename Number>
stnum<Number> stnum<Number>::mk_stnum(Number n, wrapint::bitwidth_t width) {
...
template <typename Number>
stnum<Number> stnum<Number>::mk_stnum(Number lb, Number ub,
  ...
  if (!wrapint::fits_wrapint(lb, width)) {
    ...
    if (!(lbflag ^ ubflag)) {
      ...
      tnum_t t0(wrapint::get_signed_max(width), wrapint::get_signed_max(width),
                true);
      ...
      tnum_t pos = stnum<Number>::tnum_from_range_s(
          wrapint::get_unsigned_min(width), ubwrap);
      tnum_t neg = stnum<Number>::tnum_from_range_s(
          lbwrap, wrapint::get_unsigned_max(width));
...
tnum<Number> stnum<Number>::tnum_from_range_s(wrapint min, wrapint max) {
  ...
  CRAB_LOG("stnum-imply-tnum_from_range_s",
           crab::outs() << "tnum_from_range_s of " << min << " and " << max
                        << " is: " << unsign_tnum << "\n";);
  return unsign_tnum;
}
...
template <typename Number> stnum<Number> stnum<Number>::top() {
  tnum_t t0(wrapint(0, 3), wrapint(3, 3), false);
  tnum_t t1(wrapint(4, 3), wrapint(3, 3), false);
  return stnum<Number>(t0, t1, false, false);
}

template <typename Number> tnum<Number> stnum<Number>::top_0() {
  tnum_t t0(wrapint(0, 3), wrapint(3, 3), false);
  return t0;
}

template <typename Number> tnum<Number> stnum<Number>::top_1() {
  tnum_t t1(wrapint(4, 3), wrapint(3, 3), false);
  return t1;
}

template <typename Number> stnum<Number> stnum<Number>::top(bitwidth_t width) {
  tnum_t t0(wrapint::get_unsigned_min(width), wrapint::get_signed_max(width),
            false);
  tnum_t t1(wrapint::get_signed_min(width), wrapint::get_signed_max(width),
            false);
  return stnum<Number>(t0, t1, false, false);
}

template <typename Number> stnum<Number> stnum<Number>::bottom() {
  // the wrapint is irrelevant.
  return stnum<Number>(bottom_0(), bottom_1(), true, true);
}

template <typename Number> tnum<Number> stnum<Number>::bottom_0() {
  // the wrapint is irrelevant.
  wrapint i(3, 3);
...
template <typename Number> tnum<Number> stnum<Number>::bottom_1() {
  // the wrapint is irrelevant.
  wrapint i(7, 3);
  tnum_t t = tnum<Number>(i, i, true);
  return t;
...
stnum<Number> stnum<Number>::bottom(bitwidth_t width) {
  ...
  tnum_t t0(wrapint::get_signed_max(width), wrapint::get_signed_max(width),
            true);
  tnum_t t1(wrapint::get_unsigned_max(width), wrapint::get_unsigned_max(width),
            true);
...
template <typename Number> wrapint stnum<Number>::getSignedMaxValue() const {
  if (!is_bottom_0()) {
    CRAB_LOG("stnum-imply", crab::outs()
                                << "getSignedMaxValue of 1: "
                                << tnum_0.value() + tnum_0.mask() << "\n";);
    return tnum_0.value() + tnum_0.mask();
  } else if (!is_bottom_1()) {
    CRAB_LOG("stnum-imply", crab::outs()
                                << "getSignedMaxValue of 2: "
...
template <typename Number> wrapint stnum<Number>::getUnsignedMaxValue() const {
  if (!is_bottom_1()) {
    CRAB_LOG("stnum-imply", crab::outs()
                                << "getUnsignedMaxValue of 1: "
                                << tnum_1.value() + tnum_1.mask() << "\n";);
    return tnum_1.value() + tnum_1.mask();
  } else if (!is_bottom_0()) {
    CRAB_LOG("stnum-imply", crab::outs()
                                << "getUnsignedMaxValue of 2: "
                                << tnum_0.value() + tnum_0.mask() << "\n";);
    return tnum_0.value() + tnum_0.mask();
  } else {
...
template <typename Number>
stnum<Number> stnum<Number>::lower_half_line(bool is_signed) const {
  if (is_top() || is_bottom())
    return *this;
...
// make use of x's lb
template <typename Number>
stnum<Number> stnum<Number>::lower_half_line(const stnum<Number> &x,
                                             bool is_signed) const {
  CRAB_LOG("stnum-imply-lower", crab::outs() << "lower_half_line of " << *this
                                             << " and " << x << " : \n";);
  if (is_top() || is_bottom())
    return *this;
  ...
  CRAB_LOG("stnum-imply-lower", crab::outs() << "lower_half_line 1, xmin_s = "
                                             << xmin_sign << " : \n";);
  ...
  if (is_signed) {
    if ((max_sign - wrapint::get_signed_min(w)) <
        (xmin_sign - wrapint::get_signed_min(w))) {
      CRAB_LOG("stnum-imply-lower",
               crab::outs() << "lower_half_line, res = bottom" << "\n";);
      return stnum<Number>::bottom();
    }
    ...
    CRAB_LOG("stnum-imply-lower",
             crab::outs() << "lower_half_line 2, lbwrap = " << lbwrap
                          << ", ubwrap = " << ubwrap << " : \n";);
    // need to fix
    if (!(lbflag ^ ubflag)) {
      tnum_t t = stnum<Number>::tnum_from_range_s(lbwrap, ubwrap);
      CRAB_LOG("stnum-imply-lower", crab::outs()
                                        << "lower_half_line 3 = " << " : \n";);
      if (lbflag) {
      ...
      CRAB_LOG("stnum-imply-lower",
               crab::outs() << "lower_half_line 4 = " << "umin = "
                            << wrapint::get_unsigned_min(w) << " : \n";);
      tnum<Number> pos = stnum<Number>::tnum_from_range_s(
          wrapint::get_unsigned_min(w), ubwrap);
      tnum<Number> neg = stnum<Number>::tnum_from_range_s(
          lbwrap, wrapint::get_unsigned_max(w));
      CRAB_LOG("stnum-imply-lower", crab::outs()
                                        << "lower_half_line 4, pos = " << pos
                                        << ", neg = " << neg << " : \n";);
      res = stnum<Number>(pos, neg, false, false);
    }
  } else {
    if (max_unsign < xmin_unsign)
      return stnum<Number>::bottom();
    tnum_t t = tnum<Number>::tnum_from_range(xmin_unsign, max_unsign);
    res = construct_from_tnum(t);
  }
  CRAB_LOG("stnum-imply-lower",
           crab::outs() << "lower_half_line, res = " << res << "\n";);
  return res;
}
...
// very imprecise, we donot use this in the solver
template <typename Number>
stnum<Number> stnum<Number>::upper_half_line(bool is_signed) const {
  if (is_top() || is_bottom())
    return *this;
...
// make use of x's ub
template <typename Number>
stnum<Number> stnum<Number>::upper_half_line(const stnum<Number> &x,
                                             bool is_signed) const {
  CRAB_LOG("stnum-imply", crab::outs() << "upper_half_line of " << *this
                                       << " and " << x << " : \n";);
  if (is_top() || is_bottom())
    return *this;
  wrapint::bitwidth_t w = stnum<Number>::get_bitwidth(__LINE__);
  ...
  if (is_signed) {
    ...
    if (!(lbflag ^ ubflag)) {
      ...
      CRAB_LOG("stnum-imply", crab::outs()
                                  << "upper_half_line of 2, ubwrap = " << ubwrap
                                  << ", lbwrap = " << lbwrap << " : \n";);
      tnum<Number> pos = stnum<Number>::tnum_from_range_s(
          wrapint::get_unsigned_min(w), ubwrap);
      CRAB_LOG("stnum-imply",
               crab::outs() << "upper_half_line of 2 pos =" << pos << " : \n";);
      tnum<Number> neg = stnum<Number>::tnum_from_range_s(
          lbwrap, wrapint::get_unsigned_max(w));
      CRAB_LOG("stnum-imply", crab::outs() << "upper_half_line of 2 neg = "
                                           << neg << " : \n";);
      return stnum<Number>(pos, neg, false, false);
    }
  } else {
    if (min_unsign > xmax_unsign)
      return stnum<Number>::bottom();
...
template <typename Number> bool stnum<Number>::at(wrapint x) const {
  CRAB_LOG("stnum-imply", crab::outs() << x << " at " << *this << " ?: \n";);
  if (is_bottom()) {
    return false;
  } else if (is_top()) {
    return true;
...
template <typename Number>
bool stnum<Number>::operator<=(const stnum<Number> &x) const {

  CRAB_LOG("stnum-imply", crab::outs() << *this << " <= " << x << " ?: \n";);
...
template <typename Number>
stnum<Number> stnum<Number>::operator|(const stnum<Number> &x) const {
  CRAB_LOG("stnum-imply-join", crab::outs() << *this << " U " << x << " : \n";);
  if (is_bottom()) {
    return x;
  }
  if (x.is_bottom()) {
 ...
 //   crab::outs() << "res.tnum_0 = " << res.tnum_0 << "\n";);
  ...
  CRAB_LOG("stnum-imply-join", crab::outs() << "res = " << res << "\n";);
  return res;
}

template <typename Number>
stnum<Number> stnum<Number>::operator&(const stnum<Number> &x) const {
  CRAB_LOG("stnum-imply-meet", crab::outs() << *this << " M " << x << " : \n";);
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
    ;
  }
  ...
  CRAB_LOG("stnum-imply-meet", crab::outs() << "res = " << res << " \n";);
  return res;
}
  ...
  if (is_bottom()) {
    ...
    if (is_top_0()) {
      ...
      if (tr_zero == xtr_zero && (tr_zero == (xld_zero + 1)) && tr_zero != 0) {
        ...
        // \n"; crab::outs() << "stnum-imply-widen 0, tr_zero > xtr_zero, use
        // join \n";
        res0 = tnum_0 | x.tnum_0;
      }
    }
    ...
    if (is_top_1()) {
      ...
      if (tr_zero == xtr_zero && (tr_zero == (xld_zero + 1)) && tr_zero != 0) {
        ...
        // \n"; crab::outs() << "stnum-imply-widen 1, tr_zero > xtr_zero, use
        // join \n";
        res1 = tnum_1 | x.tnum_1;
      }
      ...
      //crab::outs() << "stnum-imply-widen , resValue = "<< resValue <<
      ",unsignMax = " << unsignMax << "  \n"; tnum<Number> res1  =
      tnum<Number>(resValue, unsignMax);
      //crab::outs() << "stnum-imply-widen res1 = "<< res1 << "  \n";
    ...
    // crab::outs() << "stnum-imply-widen, res0 = " << res0 << ", res1 = "<<
    // res1 <<", res = " << res << " \n";
    return res;
  }
}
...
template <typename Number>
stnum<Number>
stnum<Number>::widening_thresholds(const stnum<Number> &x,
...
stnum<Number> stnum<Number>::operator+(const stnum<Number> &x) const {
  ...
  CRAB_LOG("stnum-imply-add", crab::outs()
                                  << *this << " + " << x << " = "
                                  << (same_circle | diff_circle) << "\n";);
  return same_circle | diff_circle;
}

template <typename Number>
stnum<Number> &stnum<Number>::operator+=(const stnum<Number> &x) {
  return this->operator=(this->operator+(x));
}

template <typename Number> stnum<Number> stnum<Number>::operator-() const {
  ...
  CRAB_LOG("stnum-imply", crab::outs() << "-" << *this << " : \n";);
  if (is_bottom()) {
    return stnum<Number>::bottom();
  }
  if (is_top()) {
...
stnum<Number> stnum<Number>::operator-(const stnum<Number> &x) const {
  ...
  CRAB_LOG("stnum-imply-sub", crab::outs()
                                  << *this << " - " << x << " = "
                                  << (same_circle | diff_circle) << "\n";);
  return same_circle | diff_circle;
}
...
template <typename Number>
stnum<Number> stnum<Number>::operator*(const stnum<Number> &x) const {
  CRAB_LOG("stnum-imply", crab::outs() << *this << " mul " << x << " : \n";);
  // wrapint::bitwidth_t w =  get_bitwidth(__LINE__);
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  }
...
template <typename Number>
stnum<Number> stnum<Number>::SDiv(const stnum<Number> &x) const {
  CRAB_LOG("stnum-imply", crab::outs() << *this << " SDiv " << x << " : \n";);
  // wrapint::bitwidth_t w =  get_bitwidth(__LINE__);
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  }
...
template <typename Number>
stnum<Number> stnum<Number>::UDiv(const stnum<Number> &x) const {
  CRAB_LOG("stnum-imply", crab::outs() << *this << " UDiv " << x << " : \n";);
  // wrapint::bitwidth_t w = get_bitwidth(__LINE__);
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  }
...
template <typename Number>
stnum<Number> stnum<Number>::SRem(const stnum<Number> &x) const {
  CRAB_LOG("stnum-imply", crab::outs() << *this << " SRem " << x << " : \n";);
  // wrapint::bitwidth_t w = get_bitwidth(__LINE__);
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  }
...
template <typename Number>
stnum<Number> stnum<Number>::URem(const stnum<Number> &x) const {
  CRAB_LOG("stnum-imply", crab::outs() << *this << " URem " << x << " : \n";);
  // wrapint::bitwidth_t w = get_bitwidth(__LINE__);
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
...
template <typename Number>
stnum<Number> stnum<Number>::ZExt(unsigned bits_to_add) const {
  if (is_bottom()) {
    return stnum<Number>::bottom();
  }
  if (is_top()) {
  ...
  tnum_t t1(wrapint::get_unsigned_max(w), wrapint::get_unsigned_max(w), true);
  return stnum<Number>(t0.ZExt(bits_to_add), t1, m_is_bottom_0, true);
}

template <typename Number>
stnum<Number> stnum<Number>::SExt(unsigned bits_to_add) const {
  if (is_bottom()) {
    return stnum<Number>::bottom();
  }
  if (is_top()) {
...
template <typename Number>
stnum<Number> stnum<Number>::Trunc(unsigned bits_to_keep) const {
  if (is_bottom()) {
    return stnum<Number>::bottom();
  }
  if (is_top()) {
...
template <typename Number>
stnum<Number> stnum<Number>::Shl(const stnum<Number> &x) const {
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  } else if (is_top() || x.is_top()) {
    return stnum<Number>::top();
...
template <typename Number>
stnum<Number> stnum<Number>::LShr(const stnum<Number> &x) const {
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  } else if (is_top() || x.is_top()) {
    return stnum<Number>::top();
  ...
  tnum_t t1(wrapint::get_unsigned_max(width), wrapint::get_unsigned_max(width),
            true);
  return stnum<Number>((tnum_0.LShr(x.tnum_0) | tnum_1.LShr(x.tnum_0)), t1);
}

template <typename Number>
stnum<Number> stnum<Number>::AShr(const stnum<Number> &x) const {
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  } else if (is_top() || x.is_top()) {
    return stnum<Number>::top();
...
template <typename Number>
stnum<Number> stnum<Number>::And(const stnum<Number> &x) const {
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  }
  if (is_top() || x.is_top()) {
  ...
  return stnum<Number>(tnum_0.And(x.tnum_0) | tnum_0.And(x.tnum_1) |
                           tnum_1.And(x.tnum_0),
                       tnum_1.And(x.tnum_1));
}

template <typename Number>
stnum<Number> stnum<Number>::Or(const stnum<Number> &x) const {
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  }
  if (is_top() || x.is_top()) {
  ...
  return stnum<Number>(tnum_0.Or(x.tnum_0), tnum_1.Or(x.tnum_1) |
                                                tnum_1.Or(x.tnum_0) |
                                                tnum_0.Or(x.tnum_1));
}

template <typename Number>
stnum<Number> stnum<Number>::Xor(const stnum<Number> &x) const {
  if (is_bottom() || x.is_bottom()) {
    return stnum<Number>::bottom();
  }
  if (is_top() || x.is_top()) {
...
template <typename Number> void stnum<Number>::write(crab::crab_os &o) const {
  if (is_bottom()) {
    o << "_|_";
  } else if (is_top()) {
    o << "top";
