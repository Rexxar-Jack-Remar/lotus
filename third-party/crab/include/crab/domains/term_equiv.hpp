...
private:
  ...
  term_id_t term_of_itv(bound_t lb, bound_t ub) {
    std::optional<number_t> n_lb = lb.number();
    std::optional<number_t> n_ub = ub.number();
  ...
  void eval_ftor_down(dom_t &dom, ttbl_t &tbl, term_id_t t) {
    ...
    if (t_ptr->kind() == term::TERM_APP) {
      ...
      if (std::optional<arith_operation_t> arith_op = term::conv2arith(op)) {
  ...
  term_id_t term_of_const(const number_t &n) {
    dom_number dom_n(n);
    std::optional<term_id_t> opt_n(_ttbl.find_const(dom_n));
    if (opt_n) {
      return *opt_n;
    } else {
  ...
  term_id_t build_term(OpTy op, term_id_t ty, term_id_t tz) {
    ...
    std::optional<term_id_t> eopt(_ttbl.find_ftor(binop, ty, tz));
    if (eopt) {
      return *eopt;
    } else {
      // Create the term
  ...
  term_id_t build_function(term_id_t ty, term_id_t tz) {
    ...
    // Check if the term already exists
    std::optional<term_id_t> eopt(_ttbl.find_ftor(op, ty, tz));
    if (eopt) {
      return *eopt;
    } else {
      // Create the term
  ...
  std::optional<dom_var_t> domvar_of_term(term_id_t id) const {
    auto it = _term_map.find(id);
    if (it != _term_map.end()) {
      return (*it).second;
    } else {
  ...
  rename_linear_cst_rev(dom_lincst_t cst, rev_map_t rev_map) const {
    std::optional<linear_expression_t> e =
...
public:
  ...
  term_domain_t operator&(const term_domain_t &o) const override {
    ...
    if (is_bottom() || o.is_top()) {
      ...
      for (auto p : out_vmap) {
        ...
        if (xit != _var_map.end()) {
          if (std::optional<dom_var_t> vx = domvar_of_term(xit->second)) {
            x_impl.assign(vt, *vx);
          }
        }
        ...
        if (yit != o._var_map.end()) {
          if (std::optional<dom_var_t> vy = o.domvar_of_term(yit->second)) {
            y_impl.assign(vt, *vy);
          }
        }
        out_varnames.push_back(vt);
  ...
  */
  ...
  interval_t at(const variable_t &x) const override {
	...
	if (std::optional<dom_var_t> dom_x = domvar_of_term(it->second)) {
  ...
  linear_constraint_system_t to_linear_constraint_system() const override {
    ...
    for (auto p : _var_map) {
      std::optional<dom_var_t> dv_opt = domvar_of_term(p.second);
      if (!dv_opt)
        continue;
