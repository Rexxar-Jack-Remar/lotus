...
tnum<Number> tnum<Number>::default_implementation(const tnum<Number> &x) const {
...
template <typename Number>
tnum<Number>::tnum(wrapint value, wrapint mask, bool is_bottom)
...
template <typename Number> uint64_t tnum<Number>::countMinLeadingZeros() const {
  wrapint max = m_value + m_mask;
  return max.countl_zero();
}

// Returns the maximum number of leading zero bits possible.
template <typename Number> uint64_t tnum<Number>::countMaxLeadingZeros() const {
  return m_value.countl_zero();
}

template <typename Number> tnum<Number> tnum<Number>::Shl(uint64_t k) const {
...
template <typename Number> tnum<Number> tnum<Number>::LShr(uint64_t k) const {
...
template <typename Number> tnum<Number> tnum<Number>::AShr(uint64_t k) const {
...
tnum<Number>::tnum(wrapint min, wrapint max, bitwidth_t width)
    : m_value(wrapint(0, width)), m_mask(wrapint::get_unsigned_max(width)),
      m_is_bottom(false) {
  wrapint chi = min ^ max;
...
template <typename Number>
tnum<Number> tnum<Number>::mk_tnum(Number n, wrapint::bitwidth_t width) {
...
template <typename Number>
tnum<Number> tnum<Number>::mk_tnum(Number lb, Number ub,
                                   wrapint::bitwidth_t width) {
  CRAB_LOG("tnum-imply", crab::outs() << "mk_tnum of " << lb << ub << ":\n";);
  if (!wrapint::fits_wrapint(lb, width)) {
    CRAB_WARN(lb, " does not fit into a wrapint. Returned top tnum");
    return tnum<Number>::top();
  } else if (!wrapint::fits_wrapint(ub, width)) {
    CRAB_WARN(ub, " does not fit into a wrapint. Returned top tnum");
    return tnum<Number>::top();
  } else {
    ...
    if (!(lbflag ^ ubflag)) {
      ...
      tnum<Number> pos =
          tnum_from_range(wrapint::get_unsigned_min(width), ubwrap);
      tnum<Number> neg =
          tnum_from_range(lbwrap, wrapint::get_unsigned_max(width));
      return pos | neg;
    }
  }
}

template <typename Number> tnum<Number> tnum<Number>::top() {
  return tnum<Number>(wrapint(0, 3), wrapint(7, 3), false);
}

template <typename Number> tnum<Number> tnum<Number>::top(bitwidth_t width) {
  return tnum<Number>(wrapint::get_unsigned_min(width),
                      wrapint::get_unsigned_max(width), false);
}

template <typename Number> tnum<Number> tnum<Number>::bottom() {
  // the wrapint is irrelevant.
  wrapint i(7, 3);
  return tnum<Number>(i, i, true);
}

template <typename Number> tnum<Number> tnum<Number>::bottom(bitwidth_t width) {
  // the wrapint is irrelevant.
...
template <typename Number> wrapint tnum<Number>::getSignedMinValue() const {
  ...
  /*if(m_value.is_signed_min()){
    //CRAB_LOG("tnum-imply", crab::outs() << "min getSignedMinValue of " <<
  *this << " = " << m_value<< ":\n";); return m_value;
  }*/
  wrapint min = m_value;
  if (m_mask.msb()) {
...
template <typename Number> tnum<Number> tnum<Number>::getZeroCircle() const {
  CRAB_LOG("tnum-imply", crab::outs()
                             << "getZeroCircle of " << *this << ":\n";);
...
template <typename Number> tnum<Number> tnum<Number>::getOneCircle() const {
...
template <typename Number> bool tnum<Number>::is_bottom() const {
  // CRAB_LOG("tnum-imply", crab::outs()<< "(" << m_value << ", " << m_mask <<
  // ")" <<  "is bottom?" << ":";);
...
template <typename Number> bool tnum<Number>::is_top() const {
  // CRAB_LOG("tnum-imply", crab::outs() << "(" << m_value << ", " << m_mask <<
  // ")" << "is top?" << ": ";); wrapint maxspan =
  // wrapint::get_unsigned_max(m_value.get_bitwidth());
  bool flag =
      (!m_is_bottom && (m_value.is_zero()) && (m_mask.is_unsigned_max()));
...
ikos::interval<Number> tnum<Number>::to_interval() const {
  ...
  if (is_bottom()) {
    ...
    return interval_t(m_value.get_signed_bignum(),
                      (m_value + m_mask).get_signed_bignum());
  }
}
...
template <typename Number>
tnum<Number> tnum<Number>::lower_half_line(bool is_signed) const {
  if (is_top() || is_bottom())
    return *this;
...
// make use of x's lb
template <typename Number>
tnum<Number> tnum<Number>::lower_half_line(const tnum<Number> &x,
                                           bool is_signed) const {
  CRAB_LOG("tnum-imply-low", crab::outs() << "lower_half_line of " << *this
                                          << " and " << x << " : \n";);
  if (is_top() || is_bottom()) {
  ...
  if (is_signed) {
    ...
    wrapint lbwrap = xmin;
    wrapint ubwrap = max;
    // CRAB_LOG("tnum-imply", crab::outs() << "lower_half_line of 3"<< lbwrap <<
    // ubwrap << "\n";);
    bool lbflag = lbwrap.msb();
    bool ubflag = ubwrap.msb();
    if (!(lbflag ^ ubflag)) {
      tnum<Number> res = tnum_from_range(lbwrap, ubwrap);
      CRAB_LOG("tnum-imply-low",
...
template <typename Number>
tnum<Number> tnum<Number>::tnum_from_range(wrapint min, wrapint max) {
  wrapint::bitwidth_t w = min.get_bitwidth();
  CRAB_LOG("tnum-imply-tnum_from_range",
           crab::outs() << "min = " << min << ", max = " << max << "\n";);
  wrapint chi = min ^ max;
  uint64_t bits = chi.fls();
  CRAB_LOG("tnum-imply-tnum_from_range",
           crab::outs() << "chi = " << chi << ", bits = " << bits << "\n";);
  if (bits > w - 1) {
    return tnum<Number>::top();
  }
  ...
  CRAB_LOG("tnum-imply-tnum_from_range",
           crab::outs() << "tnum_from_range of " << min << " and " << max
                        << " is: " << unsign_tnum << "\n";);
  return unsign_tnum;
}
...
// very imprecise, we donot use this in the solver
template <typename Number>
tnum<Number> tnum<Number>::upper_half_line(bool is_signed) const {
  if (is_top() || is_bottom())
    return *this;
...
// make use of x's ub
template <typename Number>
tnum<Number> tnum<Number>::upper_half_line(const tnum<Number> &x,
                                           bool is_signed) const {
  CRAB_LOG("tnum-imply-upper", crab::outs() << "upper_half_line of " << *this
                                            << " and " << x << " : \n";);
  if (is_top() || is_bottom())
    return *this;
  wrapint::bitwidth_t w = get_bitwidth(__LINE__);
  ...
  if (is_signed) {
    ...
    wrapint lbwrap = min;
    wrapint ubwrap = xmax;
    CRAB_LOG("tnum-imply-upper", crab::outs()
                                     << "upper_half_line lbwrap = " << lbwrap
                                     << ", ubwrap = " << ubwrap << "\n";);
    bool lbflag = lbwrap.msb();
    bool ubflag = ubwrap.msb();
    if (!(lbflag ^ ubflag)) {
      tnum<Number> res = tnum_from_range(lbwrap, ubwrap);
      CRAB_LOG("tnum-imply-upper",
               crab::outs() << "upper_half_line res = " << res << "\n";);
      return res;
    } else {
      tnum<Number> pos =
          tnum_from_range(wrapint::get_unsigned_min(width), ubwrap);
...
template <typename Number>
bool tnum<Number>::operator<=(const tnum<Number> &x) const {
  ...
  if (x.is_top() || is_bottom()) {
    ...
    return true;
  } else if (!(m_mask & (~x.m_mask)).is_zero()) { // this[i] unknow but x[i]
                                                  // know
    return false;
  } else {
...
template <typename Number>
tnum<Number> tnum<Number>::operator|(const tnum<Number> &x) const {
...
tnum<Number> tnum<Number>::operator||(const tnum<Number> &x) const {
  ...
  if (is_bottom()) {
    ...
    //crab::outs() << "tnum-imply-widen , resValue = "<< resValue << ",unsignMax
    = " << unsignMax << "  \n"; tnum<Number> res  = tnum<Number>(resValue,
    unsignMax);
     //crab::outs() << "tnum-imply-widen res = "<< res << "  \n";
...
template <typename Number>
tnum<Number>
tnum<Number>::widening_thresholds(const tnum<Number> &x,
...
template <typename Number>
tnum<Number> &tnum<Number>::operator+=(const tnum<Number> &x) {
  // crab::outs() << "operator+=" << "\n";
  return this->operator=(this->operator+(x));
}

template <typename Number> tnum<Number> tnum<Number>::operator-() const {
...
template <typename Number>
tnum<Number> tnum<Number>::divComputeLowBit(tnum<Number> Known,
                                            const tnum<Number> &LHS,
                                            const tnum<Number> &RHS) {
  // crab::outs() << "divComputeLowBit" << "\n";
  //  If LHS is Odd, the result is Odd no matter what.
  //  Odd / Odd -> Odd
  CRAB_LOG("tnum-imply-divComputeLowBit",
           crab::outs() << "divComputeLowBit begin with Known = " << Known
                        << "\n";);
  ...
  CRAB_LOG("tnum-imply-divComputeLowBit",
           crab::outs() << "divComputeLowBit compute with MinTZ = " << MinTZ
                        << ", MaxTZ = " << MaxTZ << "\n";);
  ...
  if (MinTZ >= 0) {
    ...
    CRAB_LOG("tnum-imply-divComputeLowBit",
             crab::outs() << "divComputeLowBit after trailing zero, Known = "
                          << Known << "\n";);
  ...
  CRAB_LOG("tnum-imply-divComputeLowBit",
           crab::outs() << "divComputeLowBit of " << LHS << " and " << RHS
                        << " = " << Known << "\n";);
  return Known;
}

template <typename Number>
tnum<Number> tnum<Number>::remGetLowBits(const tnum<Number> &LHS,
                                         const tnum<Number> &RHS) const {
  CRAB_LOG("tnum-imply-remGetLowBits", crab::outs()
                                           << "remGetLowBits of " << LHS
                                           << " and " << RHS << "\n";);
  wrapint::bitwidth_t w = LHS.get_bitwidth(__LINE__);
...
template <typename Number>
tnum<Number> tnum<Number>::signed_div(const tnum<Number> &x) const {
  ...
  if (is_negative() && x.is_negative()) {
    ...
    if (m_value.is_signed_min() && x.m_value.is_unsigned_max() &&
        x.m_mask.is_zero()) {
    ...
    CRAB_LOG("tnum-imply-s-div",
             crab::outs() << "Denom = " << Denom << ", Num = " << Num << "\n";);
    ...
    if (getSignedMinValue() >= (-x.getSignedMinValue())) {
      ...
      tmp = Num.sdiv(Denom);
      // CRAB_LOG("tnum-imply-s-div", crab::outs() << "Denom = " << Denom << ",
      // NUm = " << Num << "\n";);
    }
  }
  ...
  CRAB_LOG("tnum-imply-s-div", crab::outs()
                                   << "after high Res = " << Res << "\n";);
...
template <typename Number>
tnum<Number> tnum<Number>::SDiv(const tnum<Number> &x) const {
...
template <typename Number>
tnum<Number> tnum<Number>::UDiv(const tnum<Number> &x) const {
  ...
  if (flag) {
    ...
    tnum<Number> Res = tnum<Number>::top(w);
    wrapint MaxRes =
        flag ? (m_value + m_mask) : (m_value + m_mask).udiv(x.m_value);
    // determine leading bits
    uint64_t LeadZ = MaxRes.countl_zero();
    CRAB_LOG("tnum-udiv", crab::outs() << "MaxRes = " << MaxRes
                                       << ", LeadZ = " << LeadZ << "\n";);
    Res.m_value.clearHighBits(LeadZ);
    Res.m_mask.clearHighBits(LeadZ);
    CRAB_LOG("tnum-udiv",
             crab::outs() << "determine leading bits, Res = " << Res << "\n";);
    if (LeadZ == w) {
    ...
    Res = divComputeLowBit(Res, *this, x);
    CRAB_LOG("tnum-udiv", crab::outs()
                              << "determine low bits, Res = " << Res << "\n";);
    return Res;
  }
}

template <typename Number>
tnum<Number> tnum<Number>::SRem(const tnum<Number> &x) const {
  ...
  if (x.m_value.is_zero()) {
    ...
    if (x.m_mask.is_zero() && !x.m_value.msb() &&
        ((x.m_value.countr_zero() + x.m_value.countl_zero() + 1) ==
         x.get_bitwidth(__LINE__))) {
      wrapint LowBits = x.m_value - wrapint(1, x.m_value.get_bitwidth());
      // If the first operand is non-negative or has all low bits zero, then
      // the upper bits are all zero.
      if (is_nonnegative() ||
          (x.m_value.countr_zero() <= countMinTrailingZeros())) {
...
template <typename Number>
tnum<Number> tnum<Number>::URem(const tnum<Number> &x) const {
  ...
  if (is_bottom() || x.is_bottom()) {
    ...
    if (x.m_mask.is_zero() && !x.m_value.msb() &&
        ((x.m_value.countr_zero() + x.m_value.countl_zero() + 1) ==
         x.get_bitwidth(__LINE__))) {
      res.m_value =
          (x.m_value - wrapint(1, x.m_value.get_bitwidth())) & res.m_value;
      res.m_mask =
          (x.m_value - wrapint(1, x.m_value.get_bitwidth())) & res.m_mask;
      return res;
    }
    ...
    uint64_t leadingz =
        std::max(countMinLeadingZeros(), x.countMinLeadingZeros());
    res.m_value.clearHighBits(leadingz);
    res.m_mask.clearHighBits(leadingz);
    return res;
  }
...
template <typename Number>
tnum<Number> tnum<Number>::ZExt(unsigned bits_to_add) const {
  CRAB_LOG("tnum-imply", crab::outs()
                             << *this << "ZExt" << bits_to_add << ":\n";);
  if (is_bottom()) {
    return tnum<Number>::bottom();
  }
...
template <typename Number>
tnum<Number> tnum<Number>::SExt(unsigned bits_to_add) const {
  CRAB_LOG("tnum-imply", crab::outs()
                             << *this << "SExt" << bits_to_add << ":\n";);
  if (is_bottom()) {
    return tnum<Number>::bottom();
  }
...
template <typename Number>
tnum<Number> tnum<Number>::Trunc(unsigned bits_to_keep) const {
  CRAB_LOG("tnum-imply", crab::outs()
                             << *this << "Trunc" << bits_to_keep << ":\n";);
  if (is_bottom()) {
    return tnum<Number>::bottom();
  }
...
template <typename Number>
tnum<Number> tnum<Number>::Shl(const tnum_t &x) const { //
...
template <typename Number>
tnum<Number> tnum<Number>::LShr(const tnum_t &x) const { //
  ...
  if (x.is_singleton()) {
    ...
    tnum<Number> maxRes = tnum<Number>::top(w);
    // crab::outs() << "LSHR len =" << len << ", other =" <<(len +
    // x.m_value.get_uint64_t()) << "\n";
...
template <typename Number>
tnum<Number> tnum<Number>::AShr(const tnum_t &x) const { //
...
template <typename Number>
tnum<Number> tnum<Number>::And(const tnum<Number> &x) const {
...
template <typename Number>
tnum<Number> tnum<Number>::Or(const tnum<Number> &x) const {
...
template <typename Number>
tnum<Number> tnum<Number>::Xor(const tnum<Number> &x) const {
...
template <typename Number> void tnum<Number>::write(crab::crab_os &o) const {
  if (is_bottom()) {
    o << "_|_";
  } else if (is_top()) {
    o << "top";
