...
public:
  using const_iterator =
      boost::transform_iterator<get_term_transform,
                                typename var_map_t::const_iterator>;
...
private:
  ...
  boost::optional<term_id_t> choose_non_var(ttbl_t &ttbl,
    ...
    auto it =
        std::copy_if(terms.begin(), terms.end(), non_var_terms.begin(),
                     [&ttbl](term_id_t t) {
...
public:
  ...
  uf_domain_t
  widening_thresholds(const uf_domain_t &other,
    ...
    if (is_bottom() || o.is_top()) {
      ...
      for (auto p : m_var_map) {
        ...
        if (o.m_var_map.find(v) == o.m_var_map.end()) {
          ...
          // both operands
          t_new = build_dag_term(out_ttbl, solver.get_class(t_old), solver,
                                 out_ttbl, stack, cache);
        }
        out_vmap[v] = t_new;
  ...
  void set(const variable_t &x, term::term_operator_t functor,
           const variable_vector_t &args) {
    if (!is_bottom()) {
      std::vector<term_id_t> targs;
  ...
  interval_t operator[](const variable_t &x) override { return at(x); }
  ...
  void rename(const variable_vector_t &from,
    ...
    CRAB_LOG(
        "uf", crab::outs() << "Renaming {"; for (auto v : from) {
          crab::outs() << v << ";";
        } crab::outs() << "} with ";
        for (auto v : to) { crab::outs() << v << ";"; } crab::outs() << "}:\n";
        crab::outs() << *this << "\n";);
