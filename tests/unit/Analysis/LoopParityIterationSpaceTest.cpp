...
static std::string llPath(const std::string &name) {
  return std::string(LOTUS_NOELLE_LOOP_PARITY_LL_DIR) + "/loop_domain_space_" + name + ".ll";
}

static std::string goldenPath(const std::string &name) {
  return std::string(LOTUS_NOELLE_LOOP_PARITY_FIXTURE_DIR) + "/loop_domain_space/" + name + "/test.txt";
}
...
static Values collectDisjointAccesses(FunctionLoopAnalyses &analyses, llvm::LoopInfo &LI) {
  Values values;
  auto *loop = *LI.begin();
  ...
  for (auto *a : accesses) {
    for (auto *b : accesses) {
      ...
      if (iteration->areInstructionsAccessingDisjointMemoryLocationsBetweenIterations(a, b)) {
        values.insert(combineUnorderedValues({valueToString(a), valueToString(b)}));
      }
    }
  }
  return values;
...
static void runCase(const std::string &name) {
  ...
  auto module = loadModule(llPath(name), context, "LoopParityIterationSpaceTest");
  ...
  EXPECT_TRUE(sectionMatches(golden, "verifyDisjointAccessBetweenIterations", actual, &diff))
      << "Known parity deviation for loop_domain_space/" << name << ": " << diff;
}
