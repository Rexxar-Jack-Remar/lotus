...
TEST_F(EscapeAnalysisTest, PthreadCreateArgEscaped) {
  ...
  const Function *main_func = getFunctionChecked(*module, "main");
  const Instruction *alloca = getFirstInstruction(*main_func);
...
TEST_F(EscapeAnalysisTest, BitcastedThreadPayloadEscapesUnderlyingRoot) {
  ...
  const Function *main_func = getFunctionChecked(*module, "main");
  const auto *slot = getFirstInstructionAs<AllocaInst>(*main_func);
...
TEST_F(EscapeAnalysisTest, AsyncWithUnknownPolicyEscapesPointerPayload) {
  ...
  const Function *main_func = getFunctionChecked(*module, "main");
  const auto *payload = getFirstInstructionAs<AllocaInst>(*main_func);
  ASSERT_NE(payload, nullptr);
  EXPECT_TRUE(analysis.isEscaped(payload));
}
