...
struct IFDSTestFact {
  ...
  static IFDSTestFact make_value(const llvm::Value* v, const std::string& label = "") {
...
public:
  ...
  FactSet normal_flow(const llvm::Instruction* stmt, const IFDSTestFact& fact) override {
    (void)stmt;
    if (m_mode == KILL_AT_EXIT) return {};
    if (fact.is_zero()) return {fact};
    return {fact};
  }
  ...
  FactSet return_flow(const llvm::CallBase* call, const llvm::Function* callee,
                     const IFDSTestFact& exit_fact, const IFDSTestFact& call_fact) override {
    (void)call;
    (void)callee;
    (void)call_fact;
    if (exit_fact.is_zero()) return {exit_fact};
    return {exit_fact};
  }

  FactSet call_to_return_flow(const llvm::CallBase* call, const IFDSTestFact& fact) override {
    (void)call;
    if (fact.is_zero()) return {fact};
    return {fact};
  }
  ...
  TestResult validate_results(
      const std::unordered_map<const llvm::Instruction*, std::set<IFDSTestFact>>& results,
      const std::unordered_map<const llvm::Instruction*, std::set<IFDSTestFact>>& expected) const {
    TestResult r;
    r.passed = true;
    r.message = "OK";
    r.facts_at_exit = 0;
    for (const auto& p : results) r.facts_at_exit += p.second.size();
    if (results != expected) {
      r.passed = false;
