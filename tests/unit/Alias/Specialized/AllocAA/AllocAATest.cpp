...
protected:
  Function *getFunction(Module &M, StringRef name) {
    return getFunctionChecked(M, name);
  }
...
TEST_F(AllocAATest, BasicModuleParsing) {
  ...
  Function *mainFn = getFunctionChecked(*module, "main");
...
TEST_F(AllocAATest, GEPConstantIndices) {
  ...
  Function *F = getFunctionChecked(*module, "test_gep_indices");
...
TEST_F(AllocAATest, HeapAllocationDetection) {
  ...
  Function *F = getFunctionChecked(*module, "test_heap_alloc");
