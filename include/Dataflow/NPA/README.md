...
- **Differential** (Esparza et al. Defn. 3.1, 3.5): Term→0, Seq→c·d(t), Call→\( \nu(f)\cdot d(arg) + f(\nu(arg)) \), Cond/Ndet by linearity, Hole→X, Bound→0, **Concat**→\( D(t_1)\cdot \nu_X\cdot t_2 + t_1\cdot X\cdot t_2 + t_1\cdot \nu_X\cdot D(t_2) \). `InfClos` is an implementation-only evaluator extension and is not handled by the Newton differential; when present, `NewtonSolver` falls back to Kleene-style rounds starting from \( f(\bot) \).
...
- **Tensor product** (Reps et al. Alg. 3.4): `TensorSemiringTraits<D>` defines the tensor semiring, coefficient coupling, and tensor-side readout for a base domain. The default uses the exact correlated paired tensor domain from `TensorProductDomain.h`, and domains can specialize the trait to model more TOPLAS-style admissible semirings directly.
...
- **Tensor extension point**: domains can specialize `TensorSemiringTraits<D>` to provide a custom tensor semiring and tensor-side readout instead of using the default exact-correlated tensor domain. This is the hook to model more TOPLAS-style admissible semirings.
...
- `static constexpr int max_fixpoint_iters`: If declared (≥0), caps generic fixpoint iteration (used by `InfClos` and the naive linear solver). Leaving it undefined keeps the solver exact.
- `static constexpr long max_linear_steps`: If declared (≥0), caps worklist/SCC steps when solving the linearized system (useful for numeric semirings). Leaving it undefined keeps the solver exact.
