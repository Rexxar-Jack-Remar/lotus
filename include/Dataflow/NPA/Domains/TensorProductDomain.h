 ...
 *
 * value_type is a finite set of pairs (stored as a vector with dedup). combine()
 * is set-union, extend() is pairwise ⊗_p, and project() is ⊕ over readouts.
 *
 * This avoids the classic correlation loss where (a1,b1) ⊕_p (a2,b2) would
 * project to (a1⊕a2)⊗(b1⊕b2), introducing cross terms.
 */
template <class D>
struct TensorProductExactDomain {
  using V = typename D::value_type;
  ...
  static value_type combine(value_type a, const value_type &b) {
    for (const auto &p : b) add_unique(a, p);
    return a;
  }
  ...
  static value_type extend(const value_type &a, const value_type &b) {
    ...
    for (const auto &p1 : a) {
      for (const auto &p2 : b) {
        pair_type p{D::extend(p2.first, p1.first), D::extend(p1.second, p2.second)};
        add_unique(out, p);
      }
    }
    return out;
  ...
  static V project(const value_type &ps) {
    V out = D::zero();
    for (const auto &p : ps) out = D::combine(out, D::extend(p.first, p.second));
    return out;
  }
...
private:
  static void add_unique(value_type &vec, const pair_type &p) {
    ...
    for (size_t i = 0; i < vec.size();) {
      const auto &q = vec[i];
      const bool eq =
          domain_equal<D>(p.first, q.first) && domain_equal<D>(p.second, q.second);
      if (eq) return;
      const bool p_leq_q = domain_leq_idempotent<D>(p.first, q.first) &&
                           domain_leq_idempotent<D>(p.second, q.second);
      if (p_leq_q) return; // new pair is dominated
