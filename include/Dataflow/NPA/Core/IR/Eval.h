...
#include "Dataflow/NPA/Core/Solve/Fixpoint.h"
...
private:
  ...
  static V rec(const Lookup &lookup, const Env &env, const E1<D> &e) {
    ...
    case K::Term:
      ...
      break;
    case K::Seq:
      v = D::extend_lin(e->c, rec(lookup, env, e->t));
      break;
    case K::SeqR:
      v = D::extend_lin(rec(lookup, env, e->t), e->c);
      break;
    case K::Call: {
      auto it = env.find(e->sym);
    ...
    case K::Cond:
      v = D::condCombine(e->phi, rec(lookup, env, e->t1),
                         rec(lookup, env, e->t2));
      break;
    case K::Add:
      v = D::combine(rec(lookup, env, e->t1), rec(lookup, env, e->t2));
      break;
    case K::Sub:
      v = D::subtract(rec(lookup, env, e->t1), rec(lookup, env, e->t2));
      break;
    case K::Ndet:
      v = D::ndetCombine(rec(lookup, env, e->t1), rec(lookup, env, e->t2));
      break;
    case K::Project:
      v = domain_project<D>(rec(lookup, env, e->t));
      break;
    case K::Hole:
      v = lookup(e->sym);
      break;
    ...
    case K::Concat: {
      ...
      v = D::extend_lin(rec(lookup, env, e->t1),
                        D::extend_lin(mid, rec(lookup, env, e->t2)));
    } break;
    case K::Star:
    case K::Mu: {
      V init = D::zero();
