...
void map_facts_to_callee_with_policies(
    ...
    const Fact &Source, FactSet &Out, ShouldPropagate &&ShouldPropagateArg,
    MakeFact &&MakeMappedFact, IsZero &&IsZeroFact, IsGlobal &&IsGlobalFact,
    bool PropagateGlobals = true, bool PropagateZero = true,
    HandleVarArg &&HandleVarArgs = {}) {
  if (!Call || !Callee || Callee->isDeclaration()) {
    return;
  }
