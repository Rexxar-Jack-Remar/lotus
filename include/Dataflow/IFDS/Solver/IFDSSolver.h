...
private:
  ...
  // Summary edges: keyed by (callee, entry_fact) -> set of (exit inst, exit
  // fact).
  ...
  std::unordered_map<SummaryKey,
                     std::unordered_set<ExitSummary, ExitSummaryHash>,
                     SummaryKeyHash>
      m_summaries;
...
#include "Dataflow/IFDS/Solver/IFDSSolver.tpp"
