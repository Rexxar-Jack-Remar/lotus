...
struct IFDSIDESolverStatistics {
  ...
  double path_edges_per_second() const {
    return total_time_seconds > 0 
        ? path_edges_processed / total_time_seconds 
        : 0.0;
  }
  ...
  void print_summary(llvm::raw_ostream& os) const {
    ...
    os << "║   Initialization:          " << format_time(initialization_time_seconds) << "\n";
    os << "║   Tabulation:              " << format_time(tabulation_time_seconds) << "\n";
    if (value_computation_time_seconds > 0) {
      os << "║   Value computation (IDE): " << format_time(value_computation_time_seconds) << "\n";
    }
    os << "╠════════════════════════════════════════════════════════════╣\n";
    ...
    os << "║   Path edges propagated:   " << format_number(path_edges_propagated) << "\n";
    os << "║   Total unique path edges: " << format_number(path_edges_total) << "\n";
    os << "║   Max worklist size:       " << format_number(max_worklist_size) << "\n";
    os << "║   Processing rate:         " << format_rate(path_edges_per_second()) << " edges/sec\n";
    os << "╠════════════════════════════════════════════════════════════╣\n";
    
    // Flow functions
    os << "║ FLOW FUNCTIONS                                             ║\n";
    os << "║   Normal flow:             " << format_number(normal_flow_applications) << "\n";
    os << "║   Call flow:               " << format_number(call_flow_applications) << "\n";
    os << "║   Return flow:             " << format_number(return_flow_applications) << "\n";
    os << "║   Call-to-return flow:     " << format_number(call_to_return_flow_applications) << "\n";
    if (flow_function_cache_hits + flow_function_cache_misses > 0) {
      os << "║   Cache hit rate:          " << format_percentage(flow_function_cache_hit_rate()) << "\n";
    }
    os << "╠════════════════════════════════════════════════════════════╣\n";
    ...
    if (edge_function_applications > 0) {
      ...
      os << "║   Applications:            " << format_number(edge_function_applications) << "\n";
      os << "║   Compositions:            " << format_number(edge_function_compositions) << "\n";
      os << "║   Joins:                   " << format_number(edge_function_joins) << "\n";
      os << "║   Identity functions:      " << format_number(identity_edge_functions) << "\n";
      if (edge_function_cache_hits + edge_function_cache_misses > 0) {
        os << "║   Cache hit rate:          " << format_percentage(edge_function_cache_hit_rate()) << "\n";
      }
      if (compose_cache_hits + compose_cache_misses > 0) {
        os << "║   Compose cache hit rate:  " << format_percentage(compose_cache_hit_rate()) << "\n";
      }
      if (join_cache_hits + join_cache_misses > 0) {
        os << "║   Join cache hit rate:     " << format_percentage(join_cache_hit_rate()) << "\n";
      }
      os << "╠════════════════════════════════════════════════════════════╣\n";
    }
    ...
    if (summary_edges_total > 0) {
      os << "║ SUMMARY EDGES                                              ║\n";
      os << "║   Created:                 " << format_number(summary_edges_created) << "\n";
      os << "║   Reused:                  " << format_number(summary_edges_reused) << "\n";
      os << "║   Total:                   " << format_number(summary_edges_total) << "\n";
      os << "║   Reuse rate:              " << format_percentage(summary_reuse_rate()) << "\n";
      os << "╠════════════════════════════════════════════════════════════╣\n";
    }
    ...
    os << "║   Instructions analyzed:   " << format_number(instructions_analyzed) << "\n";
    os << "║   Call sites:              " << format_number(call_sites_analyzed) << "\n";
    if (indirect_call_sites > 0) {
      os << "║   Indirect calls:          " << format_number(indirect_call_sites) << "\n";
    }
    os << "╠════════════════════════════════════════════════════════════╣\n";
    ...
    os << "║   Avg facts per inst:      " << format_decimal(avg_facts_per_instruction()) << "\n";
    os << "║   Max facts at inst:       " << format_number(max_facts_at_instruction) << "\n";
    ...
    if (!facts_per_function.empty()) {
      ...
      for (const auto& pair : facts_per_function) {
        if (pair.first && pair.first->hasName()) {
          os << "  " << pair.first->getName() << ": " << pair.second << " facts";
  ...
  void export_json(llvm::raw_ostream& os) const {
    ...
    os << "    \"value_computation_seconds\": " << value_computation_time_seconds << "\n";
    os << "  },\n";
    os << "  \"worklist\": {\n";
