...
private:
  ...
  void buildMatrix() {
    ...
    for (const auto &Src : Nodes) {
      ...
      for (const auto &Dst : Problem.succs(Src)) {
        ...
        Matrix[SrcIdx][DstIdx] = Exprs.unite(
            Matrix[SrcIdx][DstIdx], Exprs.atom(Problem.edgeTransfer(Src, Dst)));
      }
    }
  }
  ...
  template <typename ReducibleViewT>
  bool
  computePathExprDelayed(const ReducibleViewT &R, ADTNode *W,
                         const std::unordered_map<n_t, ADTNode *> &LeafOf) {
    if (!W) {
      return false;
    }
    if (W->Leaf) {
      // Leaf interval: P(u,u) is handled when linking from its parent (paper
      // Fig. 6 lines 9-15). For the root leaf, handle the trivial-flowgraph
      // case.
      W->UFExpr = Exprs.one();
    ...
    for (const auto &E : W->F) {
      ...
      // Paper assumption for reducible intervals: cross forward edges target
      // the right interval entry.
      if (E.Dst != R2) {
        return false;
      }
    ...
    if (W->Right->Leaf) {
      ...
      if (hasSelfLoop(R, U)) {
        RPref =
            Exprs.concat(RPref, Exprs.star(Exprs.atom(R.edgeTransfer(U, U))));
      }
    }
