 ...
 *
 * Control locations = dataflow variables (e.g. facts); stack symbols = supergraph
 * nodes. One rule per (from_control, from_stack) -> (to_control, to_stack) with
 * a micro-function weight. Supports normal edges (one stack symbol on RHS) and
 * call edges (two stack symbols: callee_entry, return_site).
 *
 ...
 *
 * @tparam T Semiring element type (e.g. GenKillTransformer or a micro-function type).
 * @param wpds WPDS to add rules to (must already have semiring/query set).
 * @param semiring Semiring for zero() check and reference.
 * @param control_states Set of control state keys (e.g. Lambda + variable keys).
 * @param normal_edges Pairs (from_stack, to_stack) for intraprocedural/call-return edges.
 * @param call_edges Triples (from_stack, callee_entry_stack, return_site_stack) for calls.
 * @param get_weight_normal Callback (from_control, from_stack, to_control, to_stack) -> weight; return nullptr or zero to skip.
 * @param get_weight_call Callback (from_control, from_stack, to_control, callee_entry, return_site) -> weight; return nullptr or zero to skip.
 */
template <typename T>
void buildExplodedWPDS(
    ...
    const std::vector<std::tuple<wpds_key_t, wpds_key_t, wpds_key_t>>& call_edges,
    std::function<T*(wpds_key_t from_control, wpds_key_t from_stack,
                    wpds_key_t to_control, wpds_key_t to_stack)> get_weight_normal,
    std::function<T*(wpds_key_t from_control, wpds_key_t from_stack,
                    wpds_key_t to_control, wpds_key_t callee_entry,
                    wpds_key_t return_site)> get_weight_call) {
