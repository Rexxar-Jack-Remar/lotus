...
class CopyConstantAnalysis
    : public ForwardInterProceduralAnalysis<llvm::Function *,
                                            llvm::Instruction *,
...
private:
  ...
  const llvm::Constant *evaluatePhi(const llvm::PHINode *Phi,
    ...
    for (unsigned I = 0; I < Phi->getNumIncomingValues(); ++I) {
      const llvm::Constant *Incoming =
          constantOf(Phi->getIncomingValue(I), State);
      if (!Initialized) {
        Result = Incoming;
        Initialized = true;
        continue;
  ...
  const llvm::Constant *evaluateInstruction(const NodeType &Node,
    ...
    if (auto *Select = llvm::dyn_cast<llvm::SelectInst>(Node)) {
      const llvm::Constant *TrueValue =
          constantOf(Select->getTrueValue(), State);
      const llvm::Constant *FalseValue =
          constantOf(Select->getFalseValue(), State);
