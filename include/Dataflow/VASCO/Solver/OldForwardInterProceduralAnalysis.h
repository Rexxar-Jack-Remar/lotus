...
public:
  ...
  void doAnalysis() override {
    for (const auto &EntryPoint :
         this->programRepresentation().getEntryPoints()) {
      auto EntryContext = std::make_shared<vasco::Context<M, N, A>>(
          EntryPoint,
          this->programRepresentation().getControlFlowGraph(EntryPoint), false);
    ...
    while (!AnalysisStack.empty()) {
      ...
      if (!CurrentContext->getWorkList().empty()) {
        ...
        if (Item.has_value()) {
          ...
          for (const auto &Tail : Graph->tails()) {
            ExitFlow =
                this->meet(ExitFlow, CurrentContext->getValueAfter(Tail));
          }
          CurrentContext->setExitValue(ExitFlow);
          CurrentContext->markAnalysed();

          const auto *Callers =
              this->ContextTransitions.getCallers(CurrentContext);
          if (Callers) {
            std::vector<CallSiteType> SortedCallers(Callers->begin(),
                                                    Callers->end());
