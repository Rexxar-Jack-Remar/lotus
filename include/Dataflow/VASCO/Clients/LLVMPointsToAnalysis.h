...
class PointsToAnalysis
    : public OldForwardInterProceduralAnalysis<llvm::Function *, llvm::Instruction *,
                                               PointsToGraph> {
public:
...
protected:
  std::optional<DomainType> flowFunction(ContextPtr Context, const NodeType &Node,
...
private:
  ...
  bool handleMemoryIntrinsicCall(const llvm::CallBase &Call, DomainType &OutValue,
                                 const DomainType &InValue) const;
  bool handleReallocCall(const llvm::CallBase &Call, DomainType &OutValue,
                         const DomainType &InValue, std::size_t ContextId) const;
  void copyMemoryObject(const MemoryBlock &DstObject, const MemoryBlock &SrcObject,
                        DomainType &OutValue, const DomainType &InValue) const;
  ...
  ResolverReturn resolveIndirectTargets(const llvm::CallBase *Call,
                                        const DomainType &State) const;
  std::optional<DomainType> processKnownCall(ContextPtr CallerContext, NodeType Node,
                                             const MethodType &Method,
                                             const DomainType &EntryValue);
  std::optional<DomainType> processCallTarget(ContextPtr CallerContext, NodeType Call,
                                              const MethodType &Method,
