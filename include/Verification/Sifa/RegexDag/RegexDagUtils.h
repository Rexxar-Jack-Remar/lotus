...
//
// Utility functions for RegexDag overlays and marking (ported from Ultimate Sifa).
//
//===----------------------------------------------------------------------===//
...
// Loop point visitor (Ultimate's LoopPointVisitor): returns the loop point (BasicBlock*)
// of a starred regex over Transition labels.
class LoopPointVisitorTransition final
    : public lotus::pathexpressions::IRegexVisitor<Transition, const llvm::BasicBlock *, void *> {
public:
  const llvm::BasicBlock *visit(const lotus::pathexpressions::Star<Transition> &re,
                                void *arg) override {
    (void)arg;
    return re.getInner()->accept(*this, (void *)nullptr);
  }

  const llvm::BasicBlock *visit(const lotus::pathexpressions::Union<Transition> &re,
                                void *arg) override {
    (void)arg;
  ...
  const llvm::BasicBlock *visit(const lotus::pathexpressions::Concatenation<Transition> &re,
                                void *arg) override {
    (void)arg;
    const llvm::BasicBlock *a = re.getFirst()->accept(*this, (void *)nullptr);
    if (a) {
  ...
  const llvm::BasicBlock *visit(const lotus::pathexpressions::Literal<Transition> &re,
                                void *arg) override {
    (void)arg;
    return re.getLetter().source;
  }

  const llvm::BasicBlock *visit(const lotus::pathexpressions::Epsilon<Transition> &re,
                                void *arg) override {
    (void)re;
    (void)arg;
    return nullptr;
  ...
  const llvm::BasicBlock *visit(const lotus::pathexpressions::EmptySet<Transition> &re,
                                void *arg) override {
    (void)re;
    (void)arg;
...
template <typename T>
static T getOnly(const std::unordered_set<T> &s) {
  if (s.size() != 1) {
...
static std::unordered_set<const llvm::BasicBlock *>
nextLocationsTransition(
    const std::vector<RegexDagNode<Transition> *> &startPoints,
    const std::function<const llvm::BasicBlock *(const Transition &)> &getLocInStartDir,
    const std::function<std::vector<RegexDagNode<Transition> *>(RegexDagNode<Transition> *)> &nextNodes) {
  std::unordered_set<const llvm::BasicBlock *> result;
  std::queue<RegexDagNode<Transition> *> q;
  for (auto *n : startPoints) q.push(n);

  while (!q.empty()) {
    auto *node = q.front();
    q.pop();
    const auto &regex = node->getContent();
    if (dynamic_cast<const lotus::pathexpressions::Literal<Transition> *>(regex.get())) {
      const auto *lit = static_cast<const lotus::pathexpressions::Literal<Transition> *>(regex.get());
      result.insert(getLocInStartDir(lit->getLetter()));
    } else if (dynamic_cast<const lotus::pathexpressions::Star<Transition> *>(regex.get())) {
      LoopPointVisitorTransition v;
      result.insert(regex->accept(v, (void *)nullptr));
    } else if (regex->isEpsilon() || regex->isEmptySet()) {
      for (auto *n : nextNodes(node)) q.push(n);
    } else {
