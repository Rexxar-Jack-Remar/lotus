...
//
// Ultimate's StateBasedDomain<STATE> requires STATE to implement IAbstractState:
// join(other), widen(other), isBottom(). Lotus uses the same contract for
// state types that are merged (e.g. NonrelationalState, IntervalState).
//
//===----------------------------------------------------------------------===//
...
/// Concept for abstract state used in StateBasedDomain (Ultimate IAbstractState).
/// Requirements: join(other), widen(other), isBottom().
