...
// post(Edge): applies instruction-level transfer over the source block
// (sound over-approximation). Implementation in IntervalDomain.cpp.
//
//===----------------------------------------------------------------------===//
...
struct Interval {
  ...
  Interval join(const Interval &rhs) const {
    ...
    Interval r;
    r.lo = (lo && rhs.lo) ? llvm::Optional<int64_t>(std::min(*lo, *rhs.lo)) : llvm::None;
    r.hi = (hi && rhs.hi) ? llvm::Optional<int64_t>(std::max(*hi, *rhs.hi)) : llvm::None;
    return r;
  }
  ...
  Interval intersect(const Interval &rhs) const {
    if (isBottom() || rhs.isBottom()) return bottom();
    llvm::Optional<int64_t> l = (lo && rhs.lo) ? std::max(*lo, *rhs.lo) : (lo ? lo : rhs.lo);
    llvm::Optional<int64_t> h = (hi && rhs.hi) ? std::min(*hi, *rhs.hi) : (hi ? hi : rhs.hi);
  ...
  Interval complement() const {
    ...
    if (hasLower() && !hasUpper()) return {llvm::None, llvm::Optional<int64_t>(*lo - 1), false};
    if (!hasLower() && hasUpper()) return {llvm::Optional<int64_t>(*hi + 1), llvm::None, false};
    return top();
  }
...
inline SatisfyingInputs Interval::satisfyLessOrEqual(const Interval &rhs) const {
  return SatisfyingInputs(
      {lo, hi && rhs.hi ? std::min(*hi, *rhs.hi) : hi},
      {rhs.lo && lo ? std::max(*rhs.lo, *lo) : rhs.lo, rhs.hi});
}
inline SatisfyingInputs Interval::satisfyGreaterOrEqual(const Interval &rhs) const {
...
public:
  ...
  llvm::Optional<Interval> get(const llvm::Value *v) const {
    ...
    if (it == intervals_.end()) return llvm::None;
    return it->second;
  }
...
/// post(Edge) applies real block transfer (instruction semantics) for soundness.
/// When BlockTransferPolicy is set and marks a block as block-wise, post(Edge)
/// uses a fast havoc transfer for that block (less precise, faster).
