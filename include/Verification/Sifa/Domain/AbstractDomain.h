...
struct ResultForAlteredInputs {
  ...
  ResultForAlteredInputs(StateT l, StateT r, bool res, bool abstracted = false)
      : lhs(std::move(l)), rhs(std::move(r)), result(res), wasAbstracted(abstracted) {}
...
public:
  ...
  using EqBottomResult = ResultForAlteredInputs<State>; // lhs = altered pred, rhs unused

  virtual ~AbstractDomain() = default;

  /// Top element (over-approximation of all states). Ultimate: SymbolicTools.top() = true.
  virtual State top() const = 0;
  virtual State bottom() const = 0;
  virtual bool isBottom(const State &s) const = 0;

  /// Ultimate-aligned: cheap check for "state is bottom" (e.g. literal comparison).
  /// Default: isBottom(s). Override if domain has a faster literal check.
  ...
  /// Equality-to-bottom check (may over-approximate for termination). Default: isBottom(s).
  virtual bool isEqBottom(const State &s) const { return isBottom(s); }

  /// Ultimate-aligned: isEqBottom returning altered state (for early exit in DagInterpreter).
  /// Default: returns (s, s, isBottom(s), false); override if domain over-approximates.
  ...
  /// Equality for cache key lookup (e.g. FixpointLoopSummarizer). Default: leq(a,b) && leq(b,a).
  virtual bool equal(const State &a, const State &b) const { return leq(a, b) && leq(b, a); }

  /// Ultimate-aligned: isSubsetEq returning possibly altered lhs/rhs for fixpoint loop.
  /// Default: returns (subset, superset, leq(subset, superset), false).
  virtual SubsetEqResult subsetEq(const State &subset, const State &superset) const {
