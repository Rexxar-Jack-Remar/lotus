...
//
// Ultimate's LogSizeWrapperFluid logs "Predicate has dag size %s and %d disjunct(s).
// Abstraction will/won't be applied." when debug enabled. Optional size/disjunct
// functions and log callback enable the same.
//
//===----------------------------------------------------------------------===//
...
public:
  ...
  using LogFn = std::function<void(const StateT &, std::size_t dagSize, std::size_t disjuncts,
                                   bool abstractionApplied)>;
