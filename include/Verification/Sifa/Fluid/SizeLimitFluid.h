...
//
// Abstract when a state exceeds size/disjunct limits (ported from Ultimate Sifa).
//
// Ultimate: maxDagSize + DAGSize.size(term), maxDisjuncts + numberOfDisjuncts(term).
// Negative limit disables that check; caller provides sizeFn and optionally disjunctFn.
//
//===----------------------------------------------------------------------===//
...
class SizeLimitFluid final : public IFluid<StateT> {
public:
  using SizeFn = std::function<std::size_t(const StateT &)>;

  /// Single limit (dag size). Use sizeFn(state) > limit; negative limit disables.
  SizeLimitFluid(std::ptrdiff_t maxDagSize, SizeFn sizeFn)
      : maxDagSize_(maxDagSize), sizeFn_(std::move(sizeFn)) {}

  /// Ultimate-aligned: both maxDagSize and maxDisjuncts. Negative value disables.
  SizeLimitFluid(std::ptrdiff_t maxDagSize, SizeFn sizeFn, std::ptrdiff_t maxDisjuncts,
  ...
  bool shallBeAbstracted(const StateT &state) const override {
    if (maxDagSize_ >= 0 && sizeFn_ && sizeFn_(state) > static_cast<std::size_t>(maxDagSize_))
      return true;
