...
/// DagInterpreter per procedure, enter-call registration and call summarization.
/// Ultimate-aligned: optional logging hooks (logStartingSifa, logEnterProcedure, etc.).
template <typename StateT>
class IcfgInterpreter {
public:
  ...
  /// for interpret() when targeting error locations.
  ...
  void setOnEnterProcedure(std::function<void(const llvm::Function *, const StateT &)> f) {
  ...
  const ProcedureResourceCache &procedureResourceCache() const { return procResCache_; }

  /// Run interprocedural interpretation; fill \p storage with states at LOIs.
  /// Ultimate-aligned: OVERALL_TIME start/stop, ICFG_INTERPRETER_ENTERED_PROCEDURES.
  void interpret(MapBasedStorage<const llvm::BasicBlock *, StateT> &storage) {
    ...
    PriorityWorklist<const llvm::Function *, StateT> worklist(
        order, [&](const StateT &a, const StateT &b) { return domain_.join(a, b); });
    ...
    while (worklist.advance()) {
      ...
      if (onEnterProcedure_) onEnterProcedure_(F, input);

      const ProcedureResources &res = procResCache_.resourcesOf(*F);
      dagInterpreter.interpretWithCalls(res.getRegexDag(),
                                        res.getDagOverlayPathToLoisAndEnterCalls(),
