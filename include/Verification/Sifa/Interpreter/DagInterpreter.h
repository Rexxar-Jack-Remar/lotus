...
public:
  ...
  DagInterpreter(SifaStats &stats, const Domain &domain, const IFluid<State> &fluid)
  ...
  /// Optional: set a timer that returns false when processing should stop (timeout).
  /// When set, checked each worklist iteration; interpretation stops and returns
  /// current state (over-approximation) if the timer returns false.
  ...
  State interpretForSingleMarker(const Dag &dag, const IDagOverlay<L> &overlay, const State &initialInput) {
    MapBasedStorage<const llvm::BasicBlock *, State> storage;

    auto order = topsortCache_.topsort(dag);
    PriorityWorklist<Node *, State> worklist(order, [&](const State &a, const State &b) {
  ...
  void interpretWithCalls(const Dag &dag, const IDagOverlay<L> &overlay,
    ...
    while (worklist.advance()) {
      ...
      const State curOut = interpretNodeWithCalls(*cur, curIn, storage, enterCallRegistrar);
...
private:
  /// Ultimate-aligned: isBottomLiteral first (cheap), then isEqBottom only at branches.
  bool earlyExitAfterStep(const IDagOverlay<L> &overlay, Node *curNode, const State &curOut) {
    bool earlyExit = domain_.isBottomLiteral(curOut);
    if (!earlyExit && overlay.successorsOf(curNode).size() > 1) {
      stats_.increment(SifaStats::Key::DAG_INTERPRETER_EARLY_EXIT_QUERIES_NONTRIVIAL);
  ...
  State interpretNode(const Node &node, const State &input,
                      MapBasedStorage<const llvm::BasicBlock *, State> &storage) {
    return interpretNodeWithCalls(node, input, storage, *noOpEnterCallRegistrar());
  }

  State interpretNodeWithCalls(const Node &node, const State &input,
                               MapBasedStorage<const llvm::BasicBlock *, State> &storage,
                               IEnterCallRegistrar<State> &enterCallRegistrar) {
    ...
    if (const auto *lit = dynamic_cast<const lotus::pathexpressions::Literal<L> *>(regex.get())) {
      return interpretTransitionWithCalls(lit->getLetter(), input, storage, enterCallRegistrar);
    }
    if (const auto *star = dynamic_cast<const lotus::pathexpressions::Star<L> *>(regex.get())) {
  ...
  State interpretTransition(const L &t, const State &input,
                            MapBasedStorage<const llvm::BasicBlock *, State> &storage) {
    return interpretTransitionWithCalls(t, input, storage, *noOpEnterCallRegistrar());
  }

  State interpretTransitionWithCalls(const L &t, const State &input,
                                     MapBasedStorage<const llvm::BasicBlock *, State> &storage,
                                     IEnterCallRegistrar<State> &enterCallRegistrar) {
    return interpretTransitionImplWithCalls(t, input, storage, enterCallRegistrar);
  }

  template <typename X>
  typename std::enable_if<std::is_same<X, Transition>::value, State>::type
  interpretTransitionImplWithCalls(const X &t, const State &input,
                                  MapBasedStorage<const llvm::BasicBlock *, State> &storage,
                                  IEnterCallRegistrar<State> &enterCallRegistrar) {
  ...
  template <typename X>
  typename std::enable_if<std::is_same<X, Transition>::value, State>::type
  interpretTransitionImpl(const X &t, const State &input,
                          MapBasedStorage<const llvm::BasicBlock *, State> &storage) {
    if (t.kind == TransitionKind::Marker) {
      storage.store(t.target, input);
      return input;
...
extern template class lotus::sifa::DagInterpreter<lotus::sifa::Transition, bool>;
extern template class lotus::sifa::DagInterpreter<lotus::sifa::Transition, lotus::sifa::SymAbsState>;
