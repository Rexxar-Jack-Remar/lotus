...
public:
  ...
  /// Strongest postcondition for transition (Ultimate: post(input, transition)).
  State post(const Label &t, const State &input) const {
    ...
    if (stats_) stats_->stop(SifaStats::Key::TOOLS_POST_TIME);
    return r;
  }
  ...
  /// Ultimate: isBottomLiteral(pred) — cheap literal check; delegates to domain.isBottomLiteral(s).
  bool isBottomLiteral(const State &s) const { return domain_.isBottomLiteral(s); }

  Domain &getDomain() { return domain_; }
  const Domain &getDomain() const { return domain_; }

  // --- Ultimate API not provided in lotus (SMT-only) ---
  // Ultimate: and(operands), andT(operands), or(operands), orT(operands) — combine predicates/terms.
  // Ultimate: predicate(term), dnfDisjuncts(pred), dnfDisjuncts(pred, termTransformer).
  // Ultimate: isFalse(pred), implies(p1, p2) — solver checks.
  // Lotus uses AbstractDomain over LLVM states; use domain.join/meet and domain.leq for relations.
