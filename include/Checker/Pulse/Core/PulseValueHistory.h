...
public:
    ...
    struct Event {
        ...
        AbstractValue* related_value;       // Related abstract value (for allocations, etc.)
...
private:
    struct Node {
        ...
        Node(Event e, std::shared_ptr<const Node> p) : event(std::move(e)), parent(std::move(p)) {}
    };
...
public:
    struct TraceEvent {
        ...
        TraceEvent(const llvm::Instruction* loc, const llvm::Function* func, const std::string& desc)
...
public:
    ...
    void addEvent(const llvm::Instruction* loc, const llvm::Function* func, const std::string& desc) {
    ...
    static Trace fromValueHistory(const ValueHistory& history) {
        ...
        for (const auto& event : history.getEvents()) {
            ...
            std::string desc;
            switch (event.kind) {
                case ValueHistory::EventKind::Allocation: desc = "Allocated"; break;
                case ValueHistory::EventKind::FunctionCall: desc = "Returned from call"; break;
                case ValueHistory::EventKind::Load: desc = "Loaded from memory"; break;
                case ValueHistory::EventKind::Store: desc = "Stored to memory"; break;
                case ValueHistory::EventKind::Parameter: desc = "Parameter"; break;
                case ValueHistory::EventKind::Return: desc = "Return value"; break;
...
public:
    ValueOrigin(AbstractValue* av, OriginKind k, const llvm::Instruction* loc = nullptr)
    ...
    const llvm::Instruction* getOriginLocation() const { return origin_location_; }
    
    void addHistoryEvent(ValueHistory::EventKind kind, const llvm::Instruction* loc, const llvm::Function* func = nullptr) {
