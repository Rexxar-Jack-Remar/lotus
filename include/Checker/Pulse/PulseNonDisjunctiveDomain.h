...
public:
    ...
    NonDisjunctiveSummary(const NonDisjunctiveSummary& other)
        : summary_(other.summary_ ? std::make_unique<AbductiveDomain>(other.summary_->clone()) : nullptr) {}
    ...
    NonDisjunctiveSummary clone() const {
        if (summary_) {
            return NonDisjunctiveSummary(std::make_unique<AbductiveDomain>(summary_->clone()));
        }
     ...
     */
    static NonDisjunctiveSummary join(const NonDisjunctiveSummary& s1, const NonDisjunctiveSummary& s2);
};
