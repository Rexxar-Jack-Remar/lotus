...
public:
     ...
     * Join two summaries.
     */
    static AbductiveDomain joinSummaries(const AbductiveDomain& lhs, const AbductiveDomain& rhs);

private:
    // Join state: tracks substitutions during join
    struct JoinState {
        // Maps (lhs_value, rhs_value) -> joined_value
        std::map<std::pair<llvm::Optional<AbstractValue>, llvm::Optional<AbstractValue>>, AbstractValue> subst;
        
        // Reverse map: joined_value -> (lhs_value, rhs_value)
        std::map<AbstractValue, std::pair<llvm::Optional<AbstractValue>, llvm::Optional<AbstractValue>>> rev_subst;
        
        // Visited pairs to avoid cycles
        std::set<std::pair<llvm::Optional<AbstractValue>, llvm::Optional<AbstractValue>>> visited;
    ...
    joinValuesHists(JoinState& state,
     ...
     */
    static std::pair<JoinState&, std::pair<AbstractValue, ValueHistory>>
    joinValuesHistsOpts(JoinState& state,
     ...
     */
    static llvm::Optional<Attribute> joinTwoSidedAttribute(JoinState& state,
                                                           Attribute attr1,
                                                           Attribute attr2,
     ...
     * Join formulas.
     */
    static PulseFormula joinFormulas(const AbductiveDomain& lhs, const AbductiveDomain& rhs);
};
