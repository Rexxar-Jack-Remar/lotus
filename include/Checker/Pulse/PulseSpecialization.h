...
public:
    ...
    struct Element {
        ...
        Element(PathElement k, const std::string& fname) : kind(k), field_name(fname) {}
        Element(PathElement k, AbstractValue idx) : kind(k), index(idx) {}
        
        bool operator<(const Element& other) const {
            if (kind != other.kind) return kind < other.kind;
            if (kind == PathElement::FieldAccess) return field_name < other.field_name;
            if (kind == PathElement::ArrayIndex) return index < other.index;
            return false;
        }
...
public:
    ...
    bool operator<(const HeapPath& other) const {
...
struct SpecializationKey {
    // Map from heap paths to abstract values that need dynamic type specialization
     ...
     */
    bool isBottom() const {
        return heap_paths_to_values.empty() && aliasing_map.empty() && dynamic_types.empty();
    }
};
...
private:
    ...
    std::map<const llvm::Function*, std::map<SpecializationKey, std::unique_ptr<PulseSummary>>> specialized_summaries_;
    
    // Track which functions need specialization
    std::map<const llvm::Function*, std::set<SpecializationKey>> pending_specializations_;
...
public:
     ...
     */
    void addSpecializedSummary(const llvm::Function* func, 
     ...
     */
    const PulseSummary* getSpecializedSummary(const llvm::Function* func, 
                                              const SpecializationKey& key) const {
        auto it = specialized_summaries_.find(func);
        if (it == specialized_summaries_.end()) return nullptr;
     ...
     */
    bool hasSpecializedSummary(const llvm::Function* func, const SpecializationKey& key) const {
     ...
     */
    void requestSpecialization(const llvm::Function* func, const SpecializationKey& key) {
        if (!key.isBottom()) {
     ...
     */
    const std::set<SpecializationKey>& getPendingSpecializations(const llvm::Function* func) const {
     ...
     */
    static SpecializationKey computeSpecializationKey(
