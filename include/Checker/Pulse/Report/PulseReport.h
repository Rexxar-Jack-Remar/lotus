...
private:
    ...
    struct ReportEntry {
        ...
        ReportEntry(std::unique_ptr<Diagnostic> d) : representative(std::move(d)), count(1) {}
    };

    std::map<size_t, std::vector<ReportEntry>> reports_; // Map hash -> entries (handle collision)
    std::unordered_map<std::string, int> bugTypeIds_; // Map issue type string -> BugReportMgr ID
    std::mutex mutex_;
};
