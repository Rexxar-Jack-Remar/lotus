...
struct ProcedureUnit {
    ...
    ProcedureUnit(const ProcedureMatcher& m, const std::vector<TaintKind>& k, ProcedureTarget t)
...
public:
    ...
    // Add matchers
    void addSourceProcedure(const ProcedureUnit& unit) { source_procedures_.push_back(unit); }
    void addSinkProcedure(const ProcedureUnit& unit) { sink_procedures_.push_back(unit); }
    void addSanitizerProcedure(const ProcedureUnit& unit) { sanitizer_procedures_.push_back(unit); }
    void addPropagatorProcedure(const ProcedureUnit& unit) { propagator_procedures_.push_back(unit); }
    ...
    void addAllocationSource(const std::string& class_name, const std::vector<TaintKind>& kinds) {
    ...
    bool isSink(const llvm::Function* func, std::vector<TaintKind>& sink_kinds) const;
    ...
    const std::vector<SinkPolicy>& getSinkPolicies(const TaintKind& sink_kind) const;
    
    // Get allocation source kinds
    std::vector<TaintKind> getAllocationSourceKinds(const std::string& class_name) const;
