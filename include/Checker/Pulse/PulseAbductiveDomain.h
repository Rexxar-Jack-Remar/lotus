...
private:
    ...
    std::map<AbstractValue, std::pair<InvalidationKind, const llvm::Instruction*>>
        invalidation_info_;
    ...
    // Unknown values flag: did we generate at least one unknown abstract value on this path?
    bool unknown_values_ = false;
    
    // Need dynamic type specialization: set of abstract values that need specialization
    std::set<AbstractValue> need_dynamic_type_specialization_;
    
    // Recursive calls: set of mutually recursive calls (simplified: just function names)
    ...
    struct LoopInvariantUnderInference {
        ...
        LoopInvariantUnderInference(const llvm::BasicBlock* h, AbductiveDomain astate)
            : header(h), entry_astate(std::make_unique<AbductiveDomain>(std::move(astate))) {}
    };
...
public:
    ...
    void setInvalidationInfo(AbstractValue addr,
    ...
    const std::set<std::string>& getSkippedCalls() const { return skipped_calls_; }
    bool hasSkippedCall(const std::string& name) const { return skipped_calls_.count(name) > 0; }
    ...
    void setTransitiveInfo(const TransitiveInfo& info) { transitive_info_ = info; }
    ...
    // Recursive calls
    const std::set<std::string>& getRecursiveCalls() const { return recursive_calls_; }
    void addRecursiveCall(const std::string& name) { recursive_calls_.insert(name); }
    void addRecursiveCalls(const std::set<std::string>& calls) {
        recursive_calls_.insert(calls.begin(), calls.end());
    }
    
    // Loop header info
    const std::map<const llvm::BasicBlock*, unsigned>& getLoopHeaderInfo() const {
    ...
    void startLoopInvariantInference(const llvm::BasicBlock* header, AbductiveDomain entry_astate) {
        loop_invariant_under_inference_ = std::make_unique<LoopInvariantUnderInference>(
            header, std::move(entry_astate));
    }
