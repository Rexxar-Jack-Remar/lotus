...
public:
     ...
     * pred: predecessor block when resolving PHI incoming value (PHI handled in checker).
     */
    llvm::Optional<Address> eval(AbductiveDomain& astate,
    ...
    evalDeref(AbductiveDomain& astate,
              Address ptr,
              const llvm::Instruction* loc);
     ...
     */
    OperationResult checkAddrAccess(AbductiveDomain& astate,
                                     Address addr,
                                     const llvm::Instruction* loc);
     ...
     */
    void allocate(AbductiveDomain& astate,
                  AbstractValue addr,
                  const llvm::Instruction* loc);
     ...
     */
    void invalidate(AbductiveDomain& astate,
                    Address addr,
                    const llvm::Instruction* loc,
     ...
     */
    OperationResult writeDeref(AbductiveDomain& astate,
                               Address ptr,
                               Address value,
                               const llvm::Instruction* loc);
     ...
     */
    std::pair<OperationResult, llvm::Optional<Address>>
    readDeref(AbductiveDomain& astate,
              Address ptr,
              const llvm::Instruction* loc);
     ...
     */
    OperationResult checkNull(AbductiveDomain& astate,
                              Address addr,
                              const llvm::Instruction* loc);
     ...
     */
    std::pair<OperationResult, llvm::Optional<Address>>
    shallowCopy(AbductiveDomain& astate,
                Address source,
                const llvm::Instruction* loc);
     ...
     */
    std::pair<OperationResult, llvm::Optional<Address>>
    deepCopy(AbductiveDomain& astate,
             Address source,
             const llvm::Instruction* loc,
     ...
     */
    void havoc(AbductiveDomain& astate,
               Address addr,
               const llvm::Instruction* loc);
     ...
     */
    OperationResult checkAddressEscape(AbductiveDomain& astate,
                                      Address addr,
...
private:
     ...
     */
    void deepCopyRecursive(AbductiveDomain& astate,
