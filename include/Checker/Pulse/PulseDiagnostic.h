...
namespace IssueType {
    ...
    constexpr const char* StackVariableAddressEscape = "Stack Variable Address Escape";
} // namespace IssueType
...
public:
    AccessToInvalidAddress(const llvm::Instruction* loc,
        ...
        : location_(loc), message_(msg), description_(desc), 
          suggestion_(sugg), issue_type_(type), trace_(std::move(trace)),
...
public:
    ResourceLeak(const llvm::Instruction* loc,
        ...
        : location_(loc), allocation_site_(alloc_site),
          resource_name_(resource), trace_(std::move(trace)) {}

    std::string getMessage() const override;
    std::string getDescription() const override;
    std::string getSuggestion() const override { return "Close the resource before it goes out of scope."; }
...
public:
    TaintFlow(const llvm::Instruction* loc,
              const std::string& source,
              const std::string& sink,
              Trace trace)
        : location_(loc), source_kind_(source), sink_kind_(sink), trace_(std::move(trace)) {}

    std::string getMessage() const override;
    std::string getDescription() const override;
    std::string getSuggestion() const override { return "Validate untrusted input before using it in sensitive operations."; }
...
public:
    UnnecessaryCopy(const llvm::Instruction* loc,
