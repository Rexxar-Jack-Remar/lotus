...
public:
    SummaryEntry(std::unique_ptr<AbductiveDomain> pre,
    ...
    const llvm::Optional<LatentIssueSummary>& getLatentIssue() const { return latent_issue_; }
...
public:
    ...
    const std::vector<SummaryEntry>& getPrePostList() const { return pre_post_list_; }
    ...
    void setNonDisj(NonDisjunctiveSummary non_disj) { non_disj_ = std::move(non_disj); }
     ...
     */
    bool isValid() const { return !pre_post_list_.empty() || !non_disj_.isEmpty(); }
...
private:
    struct SkippedCall {
        ...
        SkippedCall(const llvm::Function* f, const llvm::Instruction* cs, const llvm::Function* c)
...
public:
    ...
    void addSkippedCall(const llvm::Function* func, const llvm::Instruction* call_site, const llvm::Function* caller) {
        skipped_calls_.emplace_back(func, call_site, caller);
    }
    
    const std::vector<SkippedCall>& getSkippedCalls() const { return skipped_calls_; }
...
public:
     ...
     */
    void recordSkippedCall(const llvm::Function* func, const llvm::Instruction* call_site, const llvm::Function* caller) {
