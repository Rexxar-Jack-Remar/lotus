...
public:
    ...
    struct StoppedExecution {
        ...
        std::vector<std::pair<const llvm::Function*, const llvm::Instruction*>> calling_context;
        
        // For LatentSpecializedTypeIssue
        std::string specialized_type;
        Trace type_trace;
        StoppedExecution() : kind(StoppedExecutionKind::ExitProgram),
        ...
        StoppedExecution(const StoppedExecution& other)
            : kind(other.kind),
              astate(other.astate ? std::make_unique<AbductiveDomain>(other.astate->clone()) : nullptr),
              diagnostic(other.diagnostic),
              trace_to_issue(other.trace_to_issue.clone()),
              latent_issue(other.latent_issue),  // Pointer, not owned
              address(other.address),
              must_be_valid(std::make_pair(other.must_be_valid.first.clone(), other.must_be_valid.second)),
...
public:
    ...
    ExecutionDomain(const ExecutionDomain& other)
        ...
        if (other.variant_ == Variant::Stopped) {
            ...
            if (other.stopped_execution_.astate) {
                stopped_execution_.astate = 
                    std::make_unique<AbductiveDomain>(other.stopped_execution_.astate->clone());
            }
            stopped_execution_.diagnostic = other.stopped_execution_.diagnostic;
            stopped_execution_.trace_to_issue = other.stopped_execution_.trace_to_issue.clone();
            stopped_execution_.latent_issue = other.stopped_execution_.latent_issue;  // Pointer, not owned
            stopped_execution_.address = other.stopped_execution_.address;
            stopped_execution_.must_be_valid = std::make_pair(
                other.stopped_execution_.must_be_valid.first.clone(),
                other.stopped_execution_.must_be_valid.second);
            stopped_execution_.calling_context = other.stopped_execution_.calling_context;
            stopped_execution_.specialized_type = other.stopped_execution_.specialized_type;
            stopped_execution_.type_trace = other.stopped_execution_.type_trace.clone();
        } else {
    ...
    static ExecutionDomain continueProgram(std::unique_ptr<AbductiveDomain> astate) {
        ExecutionDomain d;
    ...
    static ExecutionDomain exceptionRaised(std::unique_ptr<AbductiveDomain> astate) {
        ExecutionDomain d;
    ...
    static ExecutionDomain latentAbortProgram(std::unique_ptr<AbductiveDomain> astate,
    ...
    static ExecutionDomain latentInvalidAccess(
        std::unique_ptr<AbductiveDomain> astate,
        AbstractValue address,
        std::pair<Trace, llvm::Optional<InvalidationKind>> must_be_valid,
        std::vector<std::pair<const llvm::Function*, const llvm::Instruction*>> calling_context) {
        ExecutionDomain d;
    ...
    static ExecutionDomain latentSpecializedTypeIssue(
        std::unique_ptr<AbductiveDomain> astate,
        std::string specialized_type,
        Trace type_trace) {
        ExecutionDomain d;
        d.variant_ = Variant::Stopped;
        d.stopped_execution_.kind = StoppedExecutionKind::LatentSpecializedTypeIssue;
        d.stopped_execution_.astate = std::move(astate);
    ...
    // Accessors
    bool isContinueProgram() const { return variant_ == Variant::ContinueProgram; }
    bool isExceptionRaised() const { return variant_ == Variant::ExceptionRaised; }
    bool isStopped() const { return variant_ == Variant::Stopped; }
    
    bool isExitProgram() const {
        return isStopped() && stopped_execution_.kind == StoppedExecutionKind::ExitProgram;
    }
    
    bool isAbortProgram() const {
        return isStopped() && stopped_execution_.kind == StoppedExecutionKind::AbortProgram;
    }
    
    bool isLatentAbortProgram() const {
        return isStopped() && stopped_execution_.kind == StoppedExecutionKind::LatentAbortProgram;
    }
    
    bool isLatentInvalidAccess() const {
        return isStopped() && stopped_execution_.kind == StoppedExecutionKind::LatentInvalidAccess;
    }
    
    bool isLatentSpecializedTypeIssue() const {
        return isStopped() && stopped_execution_.kind == StoppedExecutionKind::LatentSpecializedTypeIssue;
    }
    ...
    StoppedExecution& getStoppedExecution() {
        return stopped_execution_;
    }
    ...
    ExecutionDomain clone() const {
        ...
        c.stopped_execution_.trace_to_issue = stopped_execution_.trace_to_issue.clone();
        c.stopped_execution_.latent_issue = stopped_execution_.latent_issue;  // Pointer, not owned
        c.stopped_execution_.address = stopped_execution_.address;
        c.stopped_execution_.must_be_valid = std::make_pair(
            stopped_execution_.must_be_valid.first.clone(),
