 ...
 * Tracks loop headers, implements widening at loop headers, and infers invariants
 * Production-ready implementation aligned with Infer's PulseLoopHeaderInfo
 */
...
private:
    ...
    static constexpr unsigned kMaxInvariantIterations = 5;  // Max iterations for invariant inference
...
public:
     ...
     */
    ExecutionDomain widen(const llvm::BasicBlock* BB, const ExecutionDomain& current_state);
     ...
     */
    llvm::Optional<ExecutionDomain> inferInvariant(const llvm::BasicBlock* BB,
     ...
     */
    void pushLoopInfo(const llvm::BasicBlock* BB, unsigned timestamp, const PulseFormula& path_condition);
