...
private:
    ...
    Trace trace_;
    std::vector<std::pair<const llvm::Function*, const llvm::Instruction*>> calling_context_;
    
public:
    LatentIssue(OperationResult diagnostic,
                IssueKind k,
                AbstractValue addr,
                const llvm::Instruction* loc,
                Trace trace)
        : diagnostic_(diagnostic), kind_(k), address_(addr), location_(loc), trace_(std::move(trace)) {}
    ...
    const std::vector<std::pair<const llvm::Function*, const llvm::Instruction*>>& getCallingContext() const {
        return calling_context_;
    }
    
    void addCallingContext(const llvm::Function* func, const llvm::Instruction* call_site) {
     ...
     */
    static bool shouldReport(const PulseSummary& summary, const LatentIssue& issue);
