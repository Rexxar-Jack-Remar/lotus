...
public:
    ...
    struct CalleeIndexToVisit {
        ...
        std::pair<AbstractValue, ValueHistory> addr_hist_caller;  // Address/history in caller
        
        CalleeIndexToVisit(AbstractValue addr_pre, const ValueHistory& hist, Access acc,
...
private:
    ...
    // Alias tracking: if alias detected between addr_callee and addr_callee' with paths,
    // aliases[addr_callee] contains addr_callee'
    // Simplified: map from AbstractValue to set of AbstractValues
    std::map<AbstractValue, std::set<AbstractValue>> aliases_;

public:
    CallState(std::unique_ptr<AbductiveDomain> astate) : astate_(std::move(astate)) {}
    ...
    const std::map<AbstractValue, std::pair<AbstractValue, LazyHeapPath>>& getRevSubst() const {
        return rev_subst_;
    }
    
    const std::map<AbstractValue, ValueHistory>& getHistMap() const { return hist_map_; }
    ...
    std::map<AbstractValue, std::set<AbstractValue>>& getAliases() { return aliases_; }
     ...
     */
    void addRevSubst(AbstractValue addr_caller, AbstractValue addr_callee, const LazyHeapPath& path) {
     ...
     */
    llvm::Optional<std::pair<AbstractValue, LazyHeapPath>> toCalleeAddr(AbstractValue addr_caller) const {
     ...
     */
    llvm::Optional<std::pair<AbstractValue, ValueHistory>> toCallerValue(AbstractValue addr_callee) const {
