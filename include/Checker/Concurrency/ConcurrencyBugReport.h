...
struct ConcurrencyBugReport {
    ...
    ConcurrencyBugReport(const ConcurrencyBugReport& other)
        : bugType(other.bugType), steps(other.steps), description(other.description),
          importance(other.importance), classification(other.classification) {
    ...
    ConcurrencyBugReport& operator=(const ConcurrencyBugReport& other) {
        if (this != &other) {
            ...
            dataRaceInfo = other.dataRaceInfo ? std::make_unique<DataRaceInfo>(*other.dataRaceInfo) : nullptr;
        }
        return *this;
    }
