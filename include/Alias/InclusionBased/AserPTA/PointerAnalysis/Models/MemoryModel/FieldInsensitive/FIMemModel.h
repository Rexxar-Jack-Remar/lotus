...
#include "Alias/InclusionBased/AserPTA/PointerAnalysis/Models/MemoryModel/FieldInsensitive/FIObject.h"
...
public:
  FIMemModel(ConsGraphTy &consGraph, const PtrOwner &owner)
  ...
  inline void handleMemCpy(const ctx *C, const llvm::MemCpyInst *memCpy,
    ...
    // a temporal node
    ObjNode *memCpyNode =
        this->template createNode<PT>(C, memCpy, AllocType::Anonymous);
...
template <typename ctx>
struct MemModelTrait<FIMemModel<ctx>>
    : public SingleInstanceOwner<FIObject<ctx>> {
  using CtxTy = ctx;
  ...
  static void getAnalysisUsage(llvm::AnalysisUsage &AU) {
    AU.addRequired<CanonicalizeGEPPass>();
  }
  ...
  template <typename PT>
  inline static CGObjNode<FIMemModel<ctx>> *
  allocateNullObj(FIMemModel<ctx> &model, const llvm::Module *module) {
    return model.template allocNullObj<PT>(module);
  }

  template <typename PT>
  inline static CGObjNode<FIMemModel<ctx>> *
  allocateUniObj(FIMemModel<ctx> &model, const llvm::Module *module) {
    return model.template allocUniObj<PT>(module);
  }

  template <typename PT>
  inline static CGObjNode<FIMemModel<ctx>> *
  allocateFunction(FIMemModel<ctx> &model, const llvm::Function *fun) {
    return model.template allocFunction<PT>(fun);
  }

  template <typename PT>
  inline static CGObjNode<FIMemModel<ctx>> *
  allocateGlobalVariable(FIMemModel<ctx> &model,
  ...
  template <typename PT>
  inline static CGObjNode<FIMemModel<ctx>> *
  allocateStackObj(FIMemModel<ctx> &model, const ctx *context,
  ...
  template <typename PT>
  inline static CGObjNode<FIMemModel<ctx>> *
  allocateHeapObj(FIMemModel<ctx> &model, const ctx *context,
                  const llvm::Instruction *callsite, const llvm::DataLayout &,
                  const llvm::Type *T) {
  ...
  template <typename PT>
  inline static CGObjNode<FIMemModel<ctx>> *
  indexObject(FIMemModel<ctx> &model, const FIObject<ctx> *obj,
              const llvm::GetElementPtrInst *gep) {
    llvm_unreachable(
        "can not index object in field insensitive pointer analysis");
  }

  template <typename PT>
  inline static void handleMemCpy(FIMemModel<ctx> &model, const ctx *C,
  ...
  template <typename PT>
  inline static void initializeGlobal(FIMemModel<ctx> &memModel,
                                      const llvm::GlobalVariable *gVar,
                                      const llvm::DataLayout &DL) {
