...
class DeepPropagation
    : public SolverBase<LangModel, DeepPropagation<LangModel>> {
public:
...
protected:
  void runSolver(LangModel &langModel) {
    ...
    // enlarge the cachedPTS if necessary, after processing offset constraints,
    ...
    while (!copySCCStack.empty()) {
      ...
      if (scc.size() > 1) {
        ...
        cachedPTS[superNode->getNodeID()] =
            PT::getPointsTo(superNode->getNodeID());
      } else {
    ...
    do {
      ...
      for (NodeID id = 0; id < nodeNum; id++) {
        ...
        for (auto it = curNode->succ_load_begin(),
             ...
             it != ie; it++) {
          auto &cachedPts =
              edgeCachedPTS[std::make_tuple(curNode, *it, Constraints::load)];
          // P_new_pts in the paper
          typename PT::PtsTy newPTS;
          // P_new_edges in the paper
          typename PT::PtsTy newEdgePTS;
          newEdgePTS.intersectWithComplement(
              PT::getPointsTo(curNode->getNodeID()), cachedPts);
          cachedPts |= newEdgePTS;
          for (auto v : newEdgePTS) {
            bool newCons = consGraph.addConstraints(consGraph.getNode(v), (*it),
                                                    Constraints::copy);
            if (newCons)
          ...
          typename PT::PtsTy diffPTS;
          diffPTS.intersectWithComplement(newPTS,
                                          PT::getPointsTo((*it)->getNodeID()));
        ...
        for (auto it = curNode->pred_store_begin(),
             ...
             it != ie; it++) {
          auto &cachedPts =
              edgeCachedPTS[std::make_tuple(*it, curNode, Constraints::store)];
          typename PT::PtsTy newEdgePTS;
          newEdgePTS.intersectWithComplement(
              PT::getPointsTo(curNode->getNodeID()), cachedPts);
          cachedPts |= newEdgePTS;
          ...
          for (auto v : newEdgePTS) {
            ...
            bool newCons =
                consGraph.addConstraints((*it), node, Constraints::copy);
            if (newCons) {
              // FIXME: will intersectWithComplement clear the pts first?
              typename PT::PtsTy diff;
              diff.intersectWithComplement(PT::getPointsTo((*it)->getNodeID()),
                                           PT::getPointsTo(v));
        ...
        for (auto it = curNode->succ_offset_begin(),
          ...
          changed |= super::processOffset(
              curNode, *it, [&](CGNodeTy *src, CGNodeTy *dst) {
                auto addrNode = llvm::cast<typename super::ObjNodeTy>(src)
                                    ->getAddrTakenNode();
  ...
  bool deepPropagate(CGNodeTy *start, CGNodeTy *stop, typename PT::PtsTy diff,
                     bool &changed) {
    ...
    typename PT::PtsTy &pts =
        const_cast<typename PT::PtsTy &>(PT::getPointsTo(start->getNodeID()));
    ...
    if (!newPTS.empty()) {
      ...
      changed = true;
      // line 10 of algo 7
      for (auto cit = start->succ_copy_begin(), cie = start->succ_copy_end();
           cit != cie; cit++) {
