...
template <typename ElemType> class FixSizedVector {
private:
...
public:
  ...
  inline explicit FixSizedVector(int size) : capacity(size) {
    vec.reserve(size);
  }

  inline FixSizedVector(const FixSizedVector &v)
      : capacity(v.getCapacity()), vec(v.getVec()) {}
  inline FixSizedVector(FixSizedVector &&v) noexcept
      : capacity(v.getCapacity()), vec(std::move(v.getVec())) {}
