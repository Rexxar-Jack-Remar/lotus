...
struct ConcatIterator : public ConcatIterator<Wrapped, N - 1> {
  ...
  inline bool operator==(const self &rhs) {
    return cur == rhs.cur && ((super *)this)->operator==((super &)rhs);
  }
...
template <typename Wrapped> struct ConcatIterator<Wrapped, 1> {
  ...
  inline bool operator!=(const ConcatIterator<Wrapped, 1> &rhs) {
...
struct ConcatIteratorWithTag : public ConcatIteratorWithTag<Wrapped, N - 1, E> {
  ...
  inline bool operator==(const self &rhs) {
    return cur == rhs.cur &&
           static_cast<super *>(this)->operator==((super &)rhs);
  }
};
