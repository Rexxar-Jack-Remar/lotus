...
class WavePropagation
    : public SolverBase<LangModel, WavePropagation<LangModel>> {
public:
...
protected:
  void runSolver(LangModel &langModel) {
    ...
    do {
      ...
      // enlarge the cachedPTS if necessary, after processing offset
      // constraints, new node can be added
      ...
      for (NodeID id = 0; id < nodeNum; id++) {
        ...
        for (auto it = curNode->succ_load_begin(),
          ...
          diffPTS.intersectWithComplement(PT::getPointsTo(curNode->getNodeID()),
                                          cachedPts);
          cachedPts |= diffPTS;
          changed |= super::processLoad(
              curNode, *it,
              [&](CGNodeTy *src, CGNodeTy *dst) {
        ...
        for (auto it = curNode->pred_store_begin(),
             ...
             it != ie; it++) {
          auto &cachedPts =
              edgeCachedPTS[std::make_tuple(curNode, *it, Constraints::store)];
          typename PT::PtsTy diffPTS;
          diffPTS.intersectWithComplement(PT::getPointsTo(curNode->getNodeID()),
                                          cachedPts);
          cachedPts |= diffPTS;

          changed |= super::processStore(
              *it, curNode,
              [&](CGNodeTy *src, CGNodeTy *dst) {
                super::processCopy(src, dst);
              },
