...
protected:
  MemBlock(const ctx *c, const llvm::Value *v, const AllocType t,
           const MemBlockKind kind)
...
private:
  ...
  FSObject<ctx> *indexMemoryBlock(size_t pOffset) {
    ...
    if (fieldNum >= 0) {
      ...
      if (fieldObjs[fieldNum].get() == nullptr) {
        fieldObjs[fieldNum] = std::unique_ptr<FSObject<ctx>>(
            new FSObject<ctx>(this, pOffset, lOffset));
      }
      ...
      //                                       pOffset,
      //                                       lOffset)).first->second);
      // return &(objectMap.emplace(lOffset, this, pOffset,
      // lOffset).first->second);
    }
  ...
  FSObject<ctx> *indexPtrInMemoryBlock(size_t pOffset) {
    ...
    size_t lOffset = layout->indexPhysicalOffset(pOffset);
    int fieldNum = layout->getLogicalOffset(lOffset);
    if (fieldNum >= 0 && layout->offsetIsPtr(lOffset)) {
      // 2nd, index the memory block, return cached object or create a new
      // object.
      if (fieldObjs[fieldNum].get() == nullptr) {
        fieldObjs[fieldNum] = std::unique_ptr<FSObject<ctx>>(
      ...
      //            // 2nd, index the memory block, return cached object or
      //            create a new object. return
...
public:
  AggregateMemBlock(const ctx *c, const llvm::Value *v, const AllocType t,
                    const MemLayout *layout)
