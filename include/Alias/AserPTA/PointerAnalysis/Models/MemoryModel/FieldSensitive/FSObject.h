...
template <typename ctx> class FSObject {
private:
  ...
  //    inline std::pair<ObjNode *, bool> getCachedResult(const
  //    llvm::GetElementPtrInst *key) const {
  ...
  //    }
  //
  //    inline void cacheIndexResult(const llvm::GetElementPtrInst *key, ObjNode
  //    *value) const {
  //        const_cast<FSObject<ctx>
  //        *>(this)->cachedResult.insert(std::make_pair(key, value));
  //    }
