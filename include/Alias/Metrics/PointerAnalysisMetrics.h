/// Pointer analysis metrics for comparing precision and soundness across analyses
#pragma once
...
struct PointerAnalysisMetrics {
  ...
  double avg_points_to_size = 0.0;      ///< total_points_to_size / num_pointers_tracked
  ...
  uint64_t num_indirect_call_edges = 0; ///< Resolved (call site, callee) for indirect
  uint64_t num_poly_indirect_calls = 0; ///< Indirect sites with >1 target
  double avg_targets_per_indirect = 0.0;///< num_indirect_call_edges / num_indirect_call_sites

  // --- Alias query metrics (use-site pairs, possibly capped) ---
  uint64_t num_alias_pairs_queried = 0; ///< Pairs actually queried (may be capped)
  ...
  double fractionNoAlias() const {
    if (num_alias_pairs_queried == 0) return 0.0;
...
struct CollectMetricsOptions {
  ...
  /// Default 50k keeps cost reasonable; increase for more precision in the sample.
 ...
 * alias-pair metrics over use-site pairs (pointer operands at each instruction),
 * capped by options.max_alias_pairs to avoid O(n^2) cost.
 *
