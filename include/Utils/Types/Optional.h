...
template <typename T>
struct storage_base<T, /*trivial*/ true> {
  bool engaged_;
  typename std::aligned_storage<sizeof(T), alignof(T)>::type buf_;
...
using convertible_t =
    typename std::enable_if<std::is_constructible<To, From>::value &&
                            !std::is_same<typename std::decay<From>::type,
                                          To>::value>::type;
...
template <typename T>
class optional : private storage_base<T> {
...
public:
  ...
  optional(const optional& other) {
    if (other.engaged_) this->construct(*other);
  }

  optional(optional&& other) noexcept(
      std::is_nothrow_move_constructible<T>::value) {
    if (other.engaged_) this->construct(std::move(*other));
    other.destroy();
  }
  ...
  constexpr optional(U&& v) { this->construct(std::forward<U>(v)); }
  ...
  optional(optional<U>&& other) {
    if (other) this->construct(std::move(*other));
    other.reset();
  }
...
//---------------------------------------------------------------------------
template <class T>
void swap(optional<T>& a,
          optional<T>& b) noexcept(noexcept(a.swap(b))) { a.swap(b); }
...
template <class T, class U>
constexpr bool operator==(U const& v, optional<T> const& a) { return a == v; }

template <class T, class U>
constexpr bool operator!=(optional<T> const& a, U const& v) { return !(a == v); }
template <class T, class U>
constexpr bool operator!=(U const& v, optional<T> const& a) { return !(a == v); }
...
namespace util {
template <typename T>
using Optional = optional14::optional<T>;
...
struct hash<optional14::optional<T>> {
  size_t operator()(optional14::optional<T> const& o) const noexcept(
      noexcept(std::declval<hash<T>>()(std::declval<T>()))) {
