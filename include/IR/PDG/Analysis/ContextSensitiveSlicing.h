...
struct SliceOptions {
  ...
  /// Include call-invocation and return edges (required for context
  /// sensitivity).
  bool include_call_return_edges = true;
  /// Optional traversal limits (0 means unlimited).
  size_t max_states = 0;
  size_t max_stack_depth = 0;
  /// Use summary cache (tabulation-style): reuse procedure summaries for same
  /// (entry, call_site).
 ...
 * Optional summary caching avoids re-exploring the same procedure for the same
 * caller context, matching the "summary edges at callee" design used in
 * IFDS-based slicers.
 */
...
public:
  ...
  using VisitedSet =
      std::unordered_set<std::pair<Node *, std::vector<Node *>>, NodeStackHash>;
   ...
   * @param edge_types Optional set of edge types to include (empty means all
   * types)
   * @return Set of nodes in the context-sensitive forward slice
   */
  NodeSet computeForwardSlice(Node &start_node,
   ...
   * @return Set of nodes in the context-sensitive forward slice
   */
  NodeSet computeForwardSlice(const NodeSet &start_nodes,
                              const std::set<EdgeType> &edge_types = {});

  /**
   * @brief Compute context-sensitive forward slice from multiple nodes with
   * traversal limits
   * @param start_nodes Set of starting nodes for the slice
   * @param edge_types Optional set of edge types to include (empty means all
   * types)
   ...
   */
  NodeSet computeBackwardSlice(Node &end_node,
                               const std::set<EdgeType> &edge_types = {});
   ...
   * @return Set of nodes in the context-sensitive backward slice
   */
  NodeSet computeBackwardSlice(const NodeSet &end_nodes,
                               const std::set<EdgeType> &edge_types = {});

  /**
   * @brief Compute context-sensitive backward slice from multiple nodes with
   * traversal limits
   * @param end_nodes Set of ending nodes for the slice
   * @param edge_types Optional set of edge types to include (empty means all
   * types)
  ...
  /**
   * @brief Compute context-sensitive forward slice using options (preferred
   * API).
   */
  NodeSet computeForwardSlice(const NodeSet &start_nodes,
  ...
  /**
   * @brief Compute context-sensitive backward slice using options (preferred
   * API).
   */
  NodeSet computeBackwardSlice(const NodeSet &end_nodes,
                               const SliceOptions &options,
   ...
   */
  bool hasContextSensitivePath(Node &source_node, Node &sink_node,
                               const std::set<EdgeType> &edge_types = {});

private:
  GenericGraph &_pdg;

  /// Summary for one (callee-entry, call_site): nodes reachable within callee
  /// and whether we return to caller.
  struct ProcedureSummary {
    NodeSet reachable;
    bool returns_to_caller = false;
  };
  using SummaryCache = std::unordered_map<std::pair<Node *, Node *>,
                                          ProcedureSummary, NodePairHash>;

  /**
   * @brief Context-sensitive traversal with call stack (unified
   * forward/backward). When summary_cache is non-null and use_summary_cache is
   * true, reuses procedure summaries.
   */
  NodeSet traverseWithStack(const NodeSet &start_nodes,
  ...
  /// Compute and cache procedure summary from (entry_node, call_site); used
  /// when summary cache is enabled.
...
public:
  ...
  compareSlices(const NodeSet &cs_slice, const NodeSet &ci_slice);
   ...
   */
  static void printContextSensitiveSlice(const NodeSet &slice,
                                         const std::string &slice_name);

  /**
   * @brief Get context-sensitive slice statistics
   * @param slice Set of nodes in the slice
   * @return Map of statistics (node types, edge types, context information,
   * etc.)
   */
  static std::unordered_map<std::string, size_t>
  getContextSensitiveSliceStatistics(const NodeSet &slice);

  /**
   * @brief Get CFL-reachability statistics
   * @param slice Set of nodes in the slice
   * @return Map of CFL-specific statistics (call stack depths, matched pairs,
   * etc.)
   */
  static std::unordered_map<std::string, size_t>
  getCFLReachabilityStatistics(const NodeSet &slice);
   ...
   */
  static bool isCFLValidPath(const std::vector<Node *> &path,
                             GenericGraph &pdg);
};
