 ...
 *
 * This file defines the slicing primitives for the Program Dependency Graph
 * (PDG):
 ...
 *
 * These primitives enable various program analysis tasks such as impact
 * analysis, debugging, and program understanding by identifying relevant
 ...
 * slicing criteria (starting nodes) by following the PDG edges in the forward
 * direction. This includes nodes that are data-dependent or control-dependent
 * on the criteria.
 */
...
public:
   ...
   */
  NodeSet computeSlice(Node &start_node,
   ...
   * @return Set of nodes in the forward slice
   */
  NodeSet computeSlice(const NodeSet &start_nodes,
                       const std::set<EdgeType> &edge_types = {});
   ...
   * @return Set of nodes in the forward slice
   */
  NodeSet computeSliceWithDepth(Node &start_node, size_t max_depth,
                                const std::set<EdgeType> &edge_types = {});
...
private:
  ...
  bool isEdgeTypeAllowed(EdgeType edge_type,
                         const std::set<EdgeType> &allowed_types) const;
};
 ...
 * This includes nodes that the criteria are data-dependent or control-dependent
 * on.
 */
...
public:
   ...
   */
  NodeSet computeSlice(Node &end_node,
                       const std::set<EdgeType> &edge_types = {});
   ...
   * @return Set of nodes in the backward slice
   */
  NodeSet computeSlice(const NodeSet &end_nodes,
                       const std::set<EdgeType> &edge_types = {});
   ...
   * @return Set of nodes in the backward slice
   */
  NodeSet computeSliceWithDepth(Node &end_node, size_t max_depth,
                                const std::set<EdgeType> &edge_types = {});
 ...
 *
 * Program chopping computes all nodes that lie on paths between a set of source
 * nodes and a set of sink nodes. This is useful for computing the impact of
 * certain program points on other program points.
 */
...
public:
  ...
  NodeSet computeChop(const NodeSet &source_nodes, const NodeSet &sink_nodes,
                      const std::set<EdgeType> &edge_types = {},
                      size_t max_paths = 0, size_t max_path_length = 0,
   ...
   * @return Set of nodes in the chop
   */
  NodeSet computeChopWithDepth(const NodeSet &source_nodes,
                               const NodeSet &sink_nodes, size_t max_depth,
                               const std::set<EdgeType> &edge_types = {},
                               size_t max_paths = 0, size_t max_path_length = 0,
                               ChopDiagnostics *diagnostics = nullptr);
   ...
   */
  bool hasPath(Node &source_node, Node &sink_node,
               const std::set<EdgeType> &edge_types = {});
  ...
  findAllPaths(Node &source_node, Node &sink_node, size_t max_paths = 0,
               const std::set<EdgeType> &edge_types = {}, size_t max_depth = 0,
...
private:
  ...
  void findPathsDFS(Node &current, Node &sink, VisitedSet &visited,
                    ...
                    std::vector<std::vector<Node *>> &all_paths, size_t depth,
                    size_t max_paths, size_t max_depth, size_t max_path_length,
                    const std::set<EdgeType> &edge_types,
...
public:
  ...
  getSliceStatistics(const NodeSet &slice);
};
